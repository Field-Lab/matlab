...
midgh = plot(crsx(1,1:3)./3, midgpeaksnorm, '-ok',  'MarkerSize', 4);
parah = plot(crsx(1,1:3)./3, parapeaksnorm, '--^k', 'MarkerSize', 5);
axis([-0.45 -0.1 0 50]);
legend([midgh(1) parah(1)], {'OFF Midget' 'OFF Parasol'});

subplot(2, 3, 2); hold on
for k = 1:length(midgetconerun.rgcs);
    plot(midgxscale(:,k), midgpeaksnorm(:,k), '-o',  'Color', [0.25 0.25 0.25], 'MarkerSize', 4);
end
for k = 1:length(parasolconerun.rgcs);
    plot(paraxscale(:,k), parapeaksnorm(:,k), '--^', 'Color', [0.25 0.25 0.25], 'MarkerSize', 5);
end
...
%% Diagnose cone selections

% Midgets, mosaic weight
figure;
spaced = space_voronoi_masks(midgetconerun, 0, 5.5);
...
nrgcs = length(midgetconerun.rgcs);
for i = 1:nrgcs
    sanesubplot(2,nrgcs, {1,i});
    cla;
    conenyc = map2manhattan(index_masks(spaced(midget_selection(:,i))));
    plot_rf_stimmap(midgetconerun, midgetconerun.rgcs(i), conenyc, 'autozoom', false)
end
...
for i = 1:nrgcs
    ...
    conenyc = map2manhattan(index_masks(spaced(weight_selection & radius_selection)));
    plot_rf_stimmap(midgetconerun, midgetconerun.rgcs(i), conenyc, 'autozoom', false)
end


% Parasols, mosaic weight
figure;
spaced = space_voronoi_masks(parasolconerun, 0, 5.5);
...
nrgcs = length(parasolconerun.rgcs);
for i = 1:nrgcs
    sanesubplot(2,nrgcs, {1,i});
    conenyc = map2manhattan(index_masks(spaced(parasol_selection(:,i))));
    plot_rf_stimmap(parasolconerun, parasolconerun.rgcs(i), conenyc, 'autozoom', false)
end
...
for k = 1:nrgcs
    if k <= nM
        plot(crsx(1,1:4)./3, peaksnorm(:,k), '-ok',  'MarkerSize', 4); 
    else
        plot(crsx(1,1:4)./3, peaksnorm(:,k), '--^k', 'MarkerSize', 5); 
    end
end
xlabel('contrast');
...
for k = 1:nrgcs
    if k <= nM
        plot(xscale(:,k), peaksnorm(:,k), '-o',  'Color', [0.25 0.25 0.25], 'MarkerSize', 4);
    else
        plot(xscale(:,k), peaksnorm(:,k), '--^', 'Color', [0.25 0.25 0.25], 'MarkerSize', 5);
    end
end
f = plot(midgfit, 'k');   set(f, 'LineWidth', 3);
f = plot(parafit, '--k'); set(f, 'LineWidth', 3);
...
for i = 1:nrgcs
    sanesubplot(2,nrgcs, {1,i});
    conenyc = map2manhattan(index_masks(spaced(selection(:,i))));
    plot_rf_stimmap(conerun, conerun.rgcs(i), conenyc, 'autozoom', false)
end
...
%% Add histograms
sanesubplot(2, 3, {1 3}); cla; hold on
histedge = 0:0.5:5;
midgkapandshap = [32 8 0 0 0 0 0 0 0 0 0];
parakapandshap = [0  0 0 2 4 1 4 2 2 0 0];
hmks = bar(histedge, midgkapandshap, 'histc');
hpks = bar(histedge, parakapandshap, 'histc');
set(hmks, 'FaceColor', 'k');
set(hpks, 'FaceColor', 'w');
axis([-0.5 5 0 34]);

sanesubplot(2, 3, {2 3}); cla; hold on
histedge = 0:6:60;
