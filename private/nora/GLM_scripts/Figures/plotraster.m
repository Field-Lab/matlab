...
addParameter(p,'raster_length',10, @isnumeric); % length of raster in seconds, default 10
...
if p.Results.separate
    ...
    if ~predict_only
        ...
        for i_trial = 1:trials
            ...
            plot(rec1, i_trial, 'k.','MarkerSize',markersz)
        end
        ylim([1 trials])
        if p.Results.labels
            xlabel('Time (seconds)')
    ...
    for i_trial = 1:trials
        ...
        if length(sim1) < 4*length(rec1)
            plot(sim1, i_trial, 'r.','MarkerSize',markersz)
        else
    ...
    for i_trial = 1:trials
        if ~predict_only
            ...
            sim1 = time(find(sim_rast(i_trial,:)));
            if length(sim1) < 4*length(rec1)
                plot(sim1, i_trial + trials, 'r.','MarkerSize',markersz)
            else
                runawaytrial(i_trial)=0;
            end
            ylim([1 trials*2])
