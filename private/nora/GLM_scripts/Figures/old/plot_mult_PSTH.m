...
for i_fit=1:3
load(['/Volumes/Analysis/nora/NSEM/GLM_Output/fixedSP_rk1_linear_MU_PS_noCP_p8IDp8/fit' num2str(i_fit) '/standardparams/NSEM_mapPRJ/2012-08-09-3/ONPar_1772.mat'])
...
convolve=200;
PSTH_rec{i_fit}=zeros(length(runtrials),bins);
PSTH_sim{i_fit}=zeros(length(runtrials),bins);
PSTH_rec_CBP{i_fit}=zeros(length(runtrials),bins);
PSTH_sim_CBP{i_fit}=zeros(length(runtrials),bins);

for i=1:length(runtrials)
    i_trial=runtrials(i);
    PSTH_rec{i_fit}(i,:)=conv(fittedGLM.xvalperformance.rasters.recorded(i_trial,1:bins),ones(convolve,1),'same');
    PSTH_sim{i_fit}(i,:)=conv(fittedGLM.xvalperformance.rasters.glm_sim(i_trial,1:bins),ones(convolve,1),'same');
    PSTH_rec_CBP{i_fit}(i,:)=conv(fittedGLM_CBP.xvalperformance.rasters.recorded(i_trial,1:bins),ones(convolve,1),'same');
    PSTH_sim_CBP{i_fit}(i,:)=conv(fittedGLM_CBP.xvalperformance.rasters.glm_sim(i_trial,1:bins),ones(convolve,1),'same');
end
...
plot(time,mean(PSTH_sim_CBP{1}))
hold on
for i=1:3
    plot(time,mean(PSTH_sim{i}),'Color',i*[1/3 0 0])
end
legend('GLM fit with CBP','GLM fit with clustering')
