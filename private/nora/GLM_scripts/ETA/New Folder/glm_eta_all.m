            ...
            if 1
                ...
                for i = 1:28
                    ...
                    if abs(temp) > 200
                        figure;
                        subplot(1,2,1)
                        imagesc(testmovie{1}.matrix(x_coord, y_coord, frame_number-100))
                        axis image
                        subplot(1,2,2)
                        imagesc(testmovie{1}.matrix(x_coord, y_coord, frame_number+100))
                        axis image
                    ...
                    SI(1,i) = temp;
                    SI(2,i) = after_stim - before_stim;
                end
%                 plot([buffer+1 buffer+1], [-5, 5], 'k')
%                 xlabel('Bin')
%                 ylabel('GLM PSTH - True PSTH (arb. units)')
%                 xlim([0 2*buffer+1])
%                 text(1010, 3, 'Time of Saccade')
%                 title(cell_savename)
%                 hold off
%                 figure;
%                 scatter(SI(1,:), SI(2,:))
...
%                 pause()
            ...
            if 0
                ...
                for i = 1:28
                    ...
                    saccade_error = saccade_error + sum(glm_error(idx));
                    n_bins = n_bins + length(idx);
                end
                s_e(1,count) = saccade_error/n_bins;
                s_e(2,count) = sum(glm_error)/length(glm_error);
                disp(saccade_error/sum(glm_error))
