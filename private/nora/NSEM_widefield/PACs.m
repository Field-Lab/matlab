...
[NSEM_Corr_AC,locations_AC, avg_profile_AC, PSTHs_AC] = wide_field(datarun, 'Off Amacrine',stim_end, get_cell_ids(datarun{1}, 'Off Amacrine'));
datarun{1}.default_sta_fits = 'vision';

%%
figure(10); hold on; plot(locations, NSEM_Corr(:,3), '.', 'Color', default_colors(1,:), 'MarkerSize', 10)
figure(10); hold on; plot(locations_AC, NSEM_Corr_AC(:,3), '.', 'Color', default_colors(2,:), 'MarkerSize', 10)
...
for i = 1:length(NSEM_Corr_AC)
    plot(PSTHs_AC{i,1}{3}/1000+locations_AC(i), 'g')
end
%%
[~, idx] = sort(locations);
