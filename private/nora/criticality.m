function [start_times, duration] = criticality(exps,stimtypes,celltypes)
...
for i_exp = exps    
    for i_stimtype = stimtypes
        ...
        for i_celltype = celltypes
            ...
            spikes = zeros(length(cellgroup),60,time_bins);
        ...
        pop = sum(sum(spike_rate));
        pop = abs((pop(:) - mean(pop(:)))/std(pop(:)));
        avalanche = diff(pop>cutoff);
        start_times = pop(avalanche>0);
        duration = pop(avalanche<0)-start_times;
        hist(start_times, 100)
        hist(duration, 100)
    end
end
end
