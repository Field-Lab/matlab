...
mask = interleaved_data_prep(datarun, 1800, 40, 'cell_spec', cells, 'visual_check', 1);
LES = interleaved_data_prep(datarun_LES, 1800, 40, 'cell_spec', cells, 'visual_check', 1);
...
%PSTH_LES = conv2(PSTH_LES, gausswin(30), 'same');
%PSTH_mask = conv2(PSTH_mask, gausswin(30), 'same');
...
hFig1=figure;
default_colors = get(gca,'ColorOrder');
set(hFig1, 'Position', [100 100 1000 150])
for i_cell =1:6
    ...
    while time < 15
        plot([120*time 120*time], [0 100], 'Color', default_colors(3,:))
        i = i+1;
...
for i_cell = 1:6
    ...
    while time < 15
        try
            mask_spike_count(i) = sum(PSTH_mask(frame_changes(i):frame_changes(i+1), i_cell));
            LES_spike_count(i) = sum(PSTH_LES(frame_changes(i):frame_changes(i+1), i_cell));
        catch
            mask_spike_count(i) = sum(PSTH_mask(frame_changes(i):end, i_cell));
            LES_spike_count(i) = sum(PSTH_LES(frame_changes(i):end, i_cell));
        end
        i = i+1;
    ...
    switch type{i_cell}
        case 'on'
            on = [on; mask_spike_count', LES_spike_count'];
        case 'off'
            off = [off; mask_spike_count', LES_spike_count'];
        case 'large'
            large = [large; mask_spike_count', LES_spike_count'];  
    end
