...
for subgroup = 1:n_subgroups
    for i_cell = cell_idx{subgroup}
        ...
        [PSTH{1}, time] = IDP_plot_PSTH(reg_data{1},i_cell, 'color', 0, 'smoothing', 10);
            ...
            spatial{1} = spatial{1} - sta_fit.surround_amp_scale*spatial{2};
            ...
            for i = 1%:length(mask_conditions{subgroup})
                ...
                for i_movie_type = 1:3
                    ...
                    data_matrix = [gen_signal{i_movie_type}{1}(1:1166)];
                    responses = PSTH{i_movie_type}(30:end);
                    model = fitnlm(data_matrix, responses, model_arch, init);
                    %plot(predict(model, data_matrix)); hold on; plot(responses); pause()
            ...
            for i_interval = 1:24
               ...
               try
                   mask_gen(i_interval) = sum(gen_signal{2}{1}((start:interval_frame(i_interval)-1)-30));
                   comp_gen(i_interval) = sum(gen_signal{3}{1}((start:interval_frame(i_interval)-1)-30));
               catch
