...
for subgroup = 1:n_subgroups
    for i_cell = cell_idx{subgroup}
        ...
        for i = 1:(length(mask_conditions{subgroup}))
            if i_exp == 3
                NSEM_Corr(i_cell,i) = err( PSTH{1}, PSTH{2}{i});
                model_NSEM_Corr(i_cell,i) = err( PSTH{1}(30:end), model_PSTH{2}{i});
            elseif i_exp == 5
                NSEM_Corr(i_cell,i) = err(second_half_PSTH, PSTH{2}{i});
                model_NSEM_Corr(i_cell,i) = err(model_second_half_PSTH, model_PSTH{2}{i});
            end
        end
        noise_floor = err( PSTH{1}, PSTH_compare);
    end
end
...
mean_corr = mean(NSEM_Corr(good_cells,:));
if i_exp == 5; noise_floor = mean_corr(end); end
mean_corr = mean_corr-noise_floor;
std_corr =  std(NSEM_Corr(good_cells,:))/sqrt(5);
figure(1); hold on
...
for i = 1:2
    mean_corr = mean(NSEM_Corr(good_cells{i},:));
    if i_exp == 5; noise_floor = mean_corr(end); end
    mean_corr = mean_corr-noise_floor;
    std_corr =  std(NSEM_Corr(good_cells{i},:))/sqrt(3);
    figure(1); hold on
    error_bars_y = [std_corr+mean_corr flip(mean_corr-std_corr)];
    error_bars_x = [all_sigmas flip(all_sigmas)];
    fill(error_bars_x, error_bars_y, 0.75*default_colors(i,:), 'FaceAlpha', 0.25, 'EdgeColor', 'none');
    hold on; plot(all_sigmas, mean_corr, 'Color', default_colors(i,:), 'LineWidth', 3)
    set(gcf, 'Position', [2 2 4 4])
    axis square
    xlim([2 10])
end
...
%{
mean_corr = mean(model_NSEM_Corr(good_cells,:));
%noise_floor = mean_corr(end);
%mean_corr = mean_corr - noise_floor;
std_corr =  std(model_NSEM_Corr(good_cells,:))/sqrt(5);
figure(2); hold on;
error_bars_y = [std_corr+mean_corr flip(mean_corr-std_corr)];
error_bars_x = [all_sigmas flip(all_sigmas)];
