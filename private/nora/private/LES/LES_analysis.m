...
datarun_LES = load_data('2015-10-29-2/map-from-data036-stream/data039/data039'); % full 
...
datarun_LES = load_data('2015-10-29-7/mapped-from-data001-streamed/data003/data003'); %full
...
datarun_LES = load_data('2015-11-09-1/map-from-data000-stream/data002/data002'); %full
...
datarun_LES = load_data('2015-11-09-1/map-from-data000-stream/data008/data008'); %full
cells = [182,751,1684, 1756, 2313, 3333, 4201, 4367, 6272, 7276];
type = {'on','off','on','off','on', 'off','on','off', 'on', 'on'};
%}
...
datarun_LES = load_data('2015-11-09-8/map-from-data006-stream/data010/data010'); % LES 
...
datarun_LES = load_data('2015-11-09-8/map-from-data006-stream/data014/data014'); % LES 
cells = [3,1147,1188,2208,2643,2866,3512,4863,5060,5915,6706,7636];
type = {'off','on','on','on','on', 'off','off','off','on','on', 'off', 'off'};
%}
...
%%
figure; 
plot(log(on(:,1)), log(on(:,2)), '.', 'MarkerSize', 10)
axis equal
...
figure; 
plot(log(off(:,1)), log(off(:,2)), '.', 'MarkerSize', 10)
axis equal
hold on; plot([0, 10], [0, 10])
xlim([0 5])
ylim([0 5])
