function map_ei_classification_txtAH(master_datarun, slave_datarun, varargin)
...
p.addParamValue('classification_file_name', 'classification_mapEI_from_')
p.addParamValue('map_file_name', 'cellmatch_mapEI_from_')
p.parse(master_datarun, slave_datarun, varargin{:});

%% for troubleshooting
%p.parse(datarun{1}, datarun{2},'master_cell_type',{1 2 3 4 5},'classification_path', save_path,'corr_threshold',.7);
%%
...
%write file
[fid1,msg]=fopen([p.Results.classification_path p.Results.classification_file_name master_datarun.names.shortname '.txt'], 'w');
...
[fid2,msg]=fopen([p.Results.classification_path p.Results.map_file_name master_datarun.names.shortname '.txt'], 'w');    
