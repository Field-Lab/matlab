...
function [MetricAnal] =  analyze_simtoraster_metrics2(Basepars,SimandMetrics) 
%% Directory Setting  
	...
	for i_trial = 1 : trials
     ...
     %       MA.pertrial.normedsimvar{i_trial}.pairs.millisec(i_pair)     = spkd ( train1 , train2 , 1000) / RetVar.avgmillisec;
        ...
        if round(pctdone/10) > round(pctdonepre/10);
            display(sprintf('SimulationIntraTrialVariability PctDone=%d',round(pctdone)));
        end
        pctdonepre = pctdone;
...
%    MA.avg_pertrial.normedsimvar.avg_millisec  = mean( [atemp.avg_millisec] ); 
    ...
    display('%%% Computing Simulation Raster Variability Cross Trial %%%%')
