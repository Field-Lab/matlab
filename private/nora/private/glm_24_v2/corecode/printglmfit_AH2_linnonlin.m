...
function printglmfit_AH2_linnonlin(Basepars,filename,savedir)
p0    = Basepars.p0;
...
if strcmp(Basepars.k_filtermode, 'STA');
    ...
    if isfield(Basepars.paramind, 'FR')
        ...
        K = reshape(p_opt(Basepars.paramind.FR), 15,15);
    end
...
if ~strcmp(Basepars.k_filtermode , 'STA')
    ...
    for plot_type = 1:3
        ...
        for i_shift = 2:6
            ...
            subplot(321)
            imagesc(reshape(Kplot(:,shift),15,15),col_ax), colormap jet
            title(sprintf('K  %dth frame',shift)); colorbar

            subplot(323)
            imagesc(reshape(Kplot(:,shift+1),15,15),col_ax), colormap jet
            title(sprintf('K  %dth frame',shift+1)); colorbar

            subplot(325)
            imagesc(reshape(Kplot(:,shift+2),15,15),col_ax), colormap jet
            ...
            subplot(322)
            imagesc(reshape(STAplot(:,shift),15,15),col_ax), colormap jet
            title(sprintf('STA  %dth frame',shift)); colorbar

            subplot(324)
            imagesc(reshape(STAplot(:,shift+1),15,15),col_ax), colormap jet
            title(sprintf('STA  %dth frame',shift+1)); colorbar

            subplot(326)
            imagesc(reshape(STAplot(:,shift+2),15,15),col_ax), colormap jet
