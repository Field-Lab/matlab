function init_pars = postspike_coupling_filterparamsAH2(init_pars);
...
%%%% no sinusoidals.. justraw optimization %%%
if isfield(init_pars, 'cp_perbin')
    if init_pars.cp_perbin && init_pars.Coupling
        init_pars.cp_timebins = floor( (((init_pars.cp_ms)/1000) /init_pars.tstim) * init_pars.spikebins_perstimframe); % on fine timescale ~ 6/factor m(s)^-1
        
        cpbins   = init_pars.cp_timebins; 
        x        = linspace( -cpbins, cpbins , 2*cpbins + 1);
        x0_ind   = cpbins + 1;
        cp_basis = zeros(cpbins,cpbins);
        for i_filt = 1:cpbins
            
            shift = (i_filt-1);
            cp_basis(:,i_filt) = normpdf(x( (x0_ind-shift) : (x0_ind-1-shift+ cpbins) ) , 0 , 1 );
        end
            
        
        init_pars.cp_basis    = cp_basis;
    end
end
