function [p_opt,Basepars,f_opt] = glm_fitAH2(Basepars,spikesconcat_Home,spikesconcat_Neighbors,fn_save,gopts,novelROIstim_Concat)
...
if Basepars.GLMPars.debug
    d_sfx = sprintf( 'debug_bin%d_blk%d',Basepars.GLMPars.binning,length(fit_blks) );
else
    d_sfx = sprintf( 'bin%d_blk%d',Basepars.GLMPars.binning,length(fit_blks) );
end
...
if SimandMetrics.MetricPars.debug,save(sprintf('%s/%d_dbugSimulationsandMetrics_%s.mat',MetricAnalysis_dir, Basepars.headid,Basepars.fn_save), 'SimandMetrics');end
...
SolSpace_MetricAnal_thisparset.BW_Test       = MetricAnal.BW;       SolSpace_MetricAnal_thisparset.NSEM_Test     = MetricAnal.NSEM;
...
SolSpace_LogProbAnal_thisparset.NSEM_Test = Raster_LogProb.NSEM;
