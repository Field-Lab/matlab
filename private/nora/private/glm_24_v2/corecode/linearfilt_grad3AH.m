...
% Just replaced stim_design (c code .. sometimes buggy) with
% stimdesign_hackAH
...
function L = linearfilt_grad3AH(p,Basepars,Stimpars,Trainpars,neuron_no)
...
if strcmp(Basepars.k_filtermode , 'fixedSP')
    ...
    L = stimdesign_hackAH(xs,ntime);  
end


if strcmp(Basepars.k_filtermode , 'OnOff_hardrect_fixedSP_STA')
    ntime  = Basepars.k_stimframes;
    xs1 = Stimpars.movie_ROI(1,:);
    L1 = stimdesign_hackAH(xs1,ntime);  
    xs2 = Stimpars.movie_ROI(2,:);
    L2 = stimdesign_hackAH(xs2,ntime);  
...
if strcmp(Basepars.k_filtermode , 'rk2')
    ...
    switch(Basepars.k_filtermode)
        case {'sep_raw','rk2'}
            ...
            L( nspace+1         : nspace+ntime    , 1:end) = stimdesign_hackAH(xs1,ntime);
            L( 2*nspace+ntime+1 : 2*(nspace+ntime), 1:end) = stimdesign_hackAH(xs2,ntime);
         %   for j=1:ntime-1
