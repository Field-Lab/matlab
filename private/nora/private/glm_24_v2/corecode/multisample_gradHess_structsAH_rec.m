...
if isfield(Basepars, 'rect_conv_spSTA')
    R_idx = Basepars.paramind.SR;
end
if isfield(Basepars, 'rect_full')
    R_idx = Basepars.paramind.FR;
end
...
gradient(R_idx) = recgradn * kernel_grad_blocked;
...
if (nargout > 1)    
    ...
    if exist('recgradn', 'var')
         H(b_idx,R_idx)  = ( sum(reckernel_hessweighted,2) )'; % 1 x nLpars
    end
    ...
    if exist('recgradn', 'var')
         H(R_idx,R_idx)  = (reckernel * reckernel_hessweighted'); % 1 x nLpars
    end
  %  HLold = H(L_idx,L_idx);
 %   Hold = H;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    if exist('recgradn', 'var')
        H(R_idx,PS_idx) = (reckernel * pskernel_hw_blocked' );% 1 x nLpars
    end
    ...
    if (cpFlag)
       % H(L_idx,CP_idx) = lgradn*cpkernel_hw_blocked';% nLpars x nCPpars
        for iNeighbor = 1 : (nNeighbors )
            ...
            if exist('recgradn', 'var')
                H(R_idx,index_CP) = ( reckernel * cpkernel_hw_blocked{iNeighbor}' );% 1 x nLpars
            end
        end
    end
