...
% filters. Multiple data set version

% Parameters to train:
% 1. stimulus filter (space x time) for each neuron
% 2. the coupling filters h (for each pair of neurons) - this is represented as cooridinates wrt some finite basis (of exponentials). This
...
% stimpars - structure with the following fields:
    % x - stimulus (space x time x dataset)
...
function [pstar fstar gstar Hstar eflag output] = train_glm2_multi(p0,basepars,stimpars,trainpars,gopts,NITER)
...
if (~isfield(trainpars,'psbasisGrad'))
    ...
    for j=1:nsets
        ...
        [sp_times D negSpikes] = pull_spikes(trainpars.analog_spikes,[],1,basepars.fac*basepars.maxt,stimpars.dt*basepars.maxt,trainpars.dt,(j-1)*stimpars.trialtime);                
        trainpars.psbasisGrad{j} = grad_basis([negSpikes;D],basepars.postspike_basis,size(negSpikes,1));         
    end
    fprintf('done.\n');
...
if (size(trainpars.D,2) > 1 && ~isfield(trainpars,'cpbasisGrad'))
    ...
    for j=1:nsets
        [sp_times D negSpikes] = pull_spikes(trainpars.analog_spikes,[],1,basepars.fac*basepars.maxt,stimpars.dt*basepars.maxt,trainpars.dt,(j-1)*stimpars.trialtime);                
        trainpars.cpbasisGrad{j} = grad_basis([negSpikes;D],basepars.coupling_basis,size(negSpikes,1));         
    end
    fprintf('done.\n');
elseif (length(trainpars.analog_spikes) == 1)
...
if (strcmp(basepars.filtermode,'nonsep') && ~isfield(trainpars,'lgrad'))
    ...
    for j=1:nsets                
        trainpars.lgrad{j} = nonsep_lgrad(basepars,struct('dt',stimpars.dt,'x',stimpars.x(:,:,j)),[]);
    end
    fprintf('done.\n');
end
...
if 0%(isempty(optimget(gopts,'PlotFcns'))) % plotting settings if not already set
...
%check_gradhess(0.1.*randn(size(p0)),@(p) ll_func2_multi(p,basepars,stimpars,trainpars), @(Hinfo,Y) ll_hess_mult(Hinfo,Y,basepars,stimpars,trainpars), vartest);
vartest = [1 5 10 20 25 35 50];
check_gradhess(p0,@(p) ll_func2_multi(p,basepars,stimpars,trainpars), [], vartest);
end
...
if (strcmp(basepars.hessmode ,'mult') && strcmp(basepars.filtermode,'nonsep') && (~isfield(basepars,'normconstr') || basepars.normconstr == 0))
    ...
    for j=1:NITER
        ...
        if (isfield(basepars,'ext_timepts') && ~isempty(basepars.ext_timepts))
            ...
            [pstar fstar eflag output] = fmincon(@(p)ll_func2_multi(p,basepars,stimpars,trainpars),pstar,[],[],A,0,[],[],[],gopts); 
        else
                1;
            [pstar fstar eflag output] = fminunc(@(p)ll_func2_multi(p,basepars,stimpars,trainpars),pstar,gopts);
        end
    end
      
    [fstar gstar] = ll_func2_multi(pstar,basepars,stimpars,trainpars);
    [Hstar] = [];%ll_hess_mult(Hinfo,eye(size(gstar,1)),basepars,stimpars,trainpars);
    
elseif (strcmp(basepars.hessmode ,'mult') && strcmp(basepars.filtermode,'nonsep') && (isfield(basepars,'normconstr') && basepars.normconstr))
    ...
    [pstar fstar] = fmincon(@(p)ll_func2_multi(p,basepars,stimpars,trainpars),p0,[],[],[],[],[],[],@(x) norm_constr(x,basepars,stimpars,trainpars),gopts_constr);    
    [fstar gstar] = ll_func2_multi(pstar,basepars,stimpars,trainpars);
    Hstar = [];
elseif (strcmp(basepars.filtermode,'sep_raw') && (isfield(basepars,'normconstr') && basepars.normconstr))
    ...
    [pstar fstar] = fmincon(@(p)ll_func2_multi(p,basepars,stimpars,trainpars),p02,[],[],[],[],[],[],@(x) norm_constr(x,basepars,stimpars,trainpars),gopts_constr);    
    [fstar gstar Hstar] = ll_func2_multi(pstar,basepars,stimpars,trainpars);
    ...
    while (~outflag)
        ...
        %check_gradhess(0.1.*randn(size(p0)),@(p) ll_func2_multi(p,basepars,stimpars,trainpars), []);
        ...
        for j=1:NITER
            ...
            if (isfield(basepars,'ext_timepts') && ~isempty(basepars.ext_timepts))
                ...
                if (~exist('nc','var'))%
                    [pstar fstar eflag output] = fmincon(@(p) ll_func2_multi(p,basepars,stimpars,trainpars), pstar, [], [], A, 0, [], [], [], gopts);
                    % [pstar fstar eflag output] = fmincon(@(p) ll_func2_multi(p,basepars,stimpars,trainpars), pstar, [], [], A, 0, lb, ub, [], gopts);
                else
                    [pstar fstar eflag output] = fmincon(@(p) ll_func2_multi(p,basepars,stimpars,trainpars), pstar, [], [], A, 0, [], [], nc, gopts_constr);
                end
                
            else
                if (~exist('nc','var'))
                    %[pstar fstar eflag output] = fmincon(@(p) ll_func2_multi(p,basepars,stimpars,trainpars), pstar, [], [], [], [], lb, ub, [], gopts);
                    [pstar fstar eflag output] = fminunc(@(p)ll_func2_multi(p,basepars,stimpars,trainpars),pstar,gopts);
                else
                    [pstar fstar eflag output] = fmincon(@(p) ll_func2_multi(p,basepars,stimpars,trainpars), pstar, [], [], [], [], [], [], nc, gopts_constr);
                end
