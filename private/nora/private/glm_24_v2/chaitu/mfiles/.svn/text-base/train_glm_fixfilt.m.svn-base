...
function [pstar fstar gstar Hstar eflag output] = train_glm_fixfilt(p0,basepars,stimpars,trainpars,gopts,NITER)
...
pstar = p0;
for j=1:NITER
   [pstar fstar eflag output] = fminunc(@(p)ll_func2_fixfilt(p,basepars,stimpars,trainpars),pstar,gopts);
end
[fstar gstar Hstar] = ll_func2_fixfilt(pstar,basepars,stimpars,trainpars);
