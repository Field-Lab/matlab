...
function [master_pstar basepars master_f master_g master_H] = multiple_neuron(cells_totrain,cells_touse,basepars,spikes_lng,tstim,cell_ids,stim_type,save_dir,gopts,X,tag)
...
if (basepars.nofilters_postspike > 0)
   basepars.postspike_basis = create_histbasis(alpha_ps,beta_ps,bstretch,basepars.nofilters_postspike,...
      basepars.Mhist,trainpars.dt,'L2',basepars.ps_spacing);
else
...
for j=1:N
   ...
   if (~strcmp(basepars.filtermode,'fixfilt') && ~isfield(basepars,'p0'))
      ...
      [stas,rstas,cpidces,p0,box_m,box_n,~,par_ini] = find_init_pt(...
         sp_times{trainpars.baseneuron_idx},basepars,stimpars,trainpars,basepars.prelimIter,...
         basepars.prelimFrames,0);
      %-- edoi: check if the cpidces are right
   ...
   %fprintf('check crop index @ multiple_neuron.m')
   %keyboard
   ...
   fprintf('now right before train_glm2...')
   switch filtermode
      case 'sep_basis'
      ...
      case 'sep_raw'
         [pstar fstar gstar] = train_glm2(p0,basepars,stimpars,trainpars,gopts,1);
      case 'nonsep'
         fprintf('multiple_neuron.m')
         keyboard
         [pstar fstar gstar] = train_glm_nonsep(p0,basepars,stimpars,trainpars,gopts,par_ini);
      case 'fixfilt'
...
if (saveFlag)
   ...
   if (~exist('tag','var') || isempty(tag))
      ...
      matfilename = strcat(save_dir,cell_prefix,'_',file_suffix,'.mat');
   else
