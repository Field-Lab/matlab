...
function [lg_p input_term cifs kx lg_p_breakdown] = train_ll3_fixrk2(p,basepars,stimpars,trainpars)
...
knormoffset = get_pars_idx(basepars,1,Neff,'k'); % modify get_pars_idx for fixrk2 -- done.
%knorms = p(knormoffset(1):numpars_perneuron:end);
knorms = p(knormoffset); % for a single neuron for now.  edoi.
% edoi, editing, 2011-01-04.

% kx = trainpars.kx * diag(knorms); modified, edoi, 2012-01-06.
fprintf('\ntrain_ll3_fixrk2.m...\n')
keyboard
kx = trainpars.kx{1}*knorms(1) + trainpars.kx{2}*knorms(2);
