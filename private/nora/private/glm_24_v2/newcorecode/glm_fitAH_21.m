function [Basepars, opt_param] = glm_fitAH_21(Basepars,spikesconcat_Home,spikesconcat_Neighbors,gopts,finalstim)
%
...
microBins = Basepars.spikebins_perstimframe*Basepars.fitframes;  % this is in (1/50)th units per frame
duration  = Basepars.tstim * Basepars.fitframes;
...
%%% PULL SPIKES Convolve with PS filter  %%%%%%%%
...
%%% PULL NHB SPIKES Convolve with CP filters %%%% 
...
if strcmp(Basepars.k_filtermode, 'STA')
    Basepars.kx_STA          = sum(fastconv(Stimpars.movie_ROI,Basepars.STA,Basepars.k_spacepixels,Basepars.fitframes,Basepars.padval),1)';
end
if Basepars.ps_FIX
...
%%
if isfield(Basepars, 'frame_offset')
    frame_idx = (Basepars.frame_offset+1) : Basepars.frame_offset+Basepars.fitframes;   %%% total frames.. again   
else
    frame_idx = 1 : Basepars.fitframes;   %%% total frames.. again   
end
Stimpars.dt = Basepars.tstim;
if strcmp(Basepars.k_filtermode, 'OnOff_hardrect_fixedSP_STA')
    Stimpars.movie_ROI(1,:) = finalstim.pos;
    Stimpars.movie_ROI(2,:) = finalstim.neg;
end
if strcmp(Basepars.k_filtermode, 'rk2')
    Stimpars.movie_ROI = finalstim(:,frame_idx); clear frame_idx  %%% the movie over the ROI
end
...
if strcmp( Basepars.k_filtermode , 'OnOff_hardrect_fixedSP_STA')
    Basepars.crop_idx = 1:2;
end


%[f_opt g_opt H_opt lcifs] = ll_func2AH(p0,Basepars,Stimpars,Trainpars);
