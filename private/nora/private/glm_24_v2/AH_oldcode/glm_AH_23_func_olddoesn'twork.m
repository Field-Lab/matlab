...
function [opt_param , Basepars] = glm_AH_23_func_old(cid, exp_nm, GLMPars,concat_fullfitMovie,inputstats, testmovie)  
BD = NSEM_BaseDirectories;
[StimulusPars DirPars datarun_slv datarun_mas] = Directories_Params_v22(exp_nm, GLMPars.fit_type, GLMPars.map_type)   
...
directory_type = 'GLM_fitandraster'; condition = 'netappsnle';
...
%%% hack to account for disparate location of files %%%
    ...
    tempdir  = NSEM_secondaryDirectories(directory_type, inputs,condition);
    load(sprintf('%s/BlockSpikesandRasters/organizedspikes_%s.mat',tempdir,init_pars.savename));
  ...
  %  init_pars.p0(init_pars.paramind.MU)  = log (spikespersec);
    ...
    for nid = [cid, neighbors_id]
        ...
        inputs.exp_nm   = exp_nm;
        tempdir  = NSEM_secondaryDirectories(directory_type, inputs,condition);
        load(sprintf('%s/BlockSpikesandRasters/organizedspikes_%s.mat',tempdir,savename));
