...
function xvalperformance = eval_xvalperformance_NEW(fittedGLM, SPars, organizedspikes,testmovie)
%%
...
if GLMType.PostSpikeFilter
    ...
    %not sure how to do this part?? NBCoupling 06-23-2014
if GLMType.CouplingFilters
    lcif_cp = fastconvAH(logicalspike , [0; CP]', size(logicalspike,1), size(logicalspike,2) );    
    lcif = lcif + lcif_cp;
end
...
else if GLMType.CouplingFilters
    cif_cpgain = exp(cp);
    cp_bins     = length(cif_cpgain);
    for i_trial = 1 : size(logicalspike,1)
        cif0         = exp(lcif_const);         
        cif_cp       = cif0;
        binary_simulation = zeros(1,params.bins);
        for i = 1 : params.bins- cp_bins;
            ...
            if roll >  exp(-params.bindur*cif_cp(i));
                cif_cp(i+1: i + cp_bins) =  cif_cp(i+1: i + cp_bins) .* (cif_cpgain');
                binary_simulation(i)= 1;
            end
        end
        logical_sim(i_trial,:) = binary_simulation ;
