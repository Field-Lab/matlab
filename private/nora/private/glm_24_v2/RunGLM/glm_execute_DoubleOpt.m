...
function [fittedGLM] = glm_execute_DoubleOpt(GLMType, spikes, fitmovie, glm_cellinfo,troubleshoot)
...
[fstar, pstar] = glm_execute_DoubleOpt_InnerLoop([], '',glm_cellinfo, GLMType_start, GLMPars,fixed_covariates,home_spbins,fitmovie,t_bin,p_init);
...
if isfield(GLMType,'input_pt_nonlinearity')  && GLMType.input_pt_nonlinearity && ...
    ...
    [nonlinstar, fstar eflag output] = fminbnd(@(nonlin_par) glm_execute_DoubleOpt_InnerLoop(nonlin_par,'input_pt_nonlinearity',...
        glm_cellinfo, GLMType, GLMPars,fixed_covariates,home_spbins,fitmovie,t_bin,p_guess,troubleshoot),...
        .05, 2,outer_optim_struct);
    [fstar, pstar] = glm_execute_DoubleOpt_InnerLoop(nonlinstar, 'input_pt_nonlinearity',glm_cellinfo, GLMType, GLMPars,fixed_covariates,home_spbins,fitmovie,t_bin,p_guess,troubleshoot);
    nonlin_pstar = pstar;
    else
        [nonlinstar, fstar eflag output] = fminbnd(@(nonlin_par) glm_execute_DoubleOpt_InnerLoop(nonlin_par,'input_pt_nonlinearity',...
        glm_cellinfo, GLMType, GLMPars,fixed_covariates,home_spbins,fitmovie,t_bin,p_guess),...
        .05,2,outer_optim_struct);
        [fstar, pstar] = glm_execute_DoubleOpt_InnerLoop(nonlinstar, 'input_pt_nonlinearity',glm_cellinfo, GLMType, GLMPars,fixed_covariates,home_spbins,fitmovie,t_bin,p_guess);
        nonlin_pstar = pstar;
    end
end    
