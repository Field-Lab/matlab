...
for i_exp=3:4
    ...
    cells
    for fit_type=1
        GLMType.fit_type=fittyps{fit_type};
        ...
        for i=1:length(blockedmoviecell)
            StimData.FitMovie{i}=blockedmoviecell{i}.matrix;
        end
        StimData.TestMovie=testmovie{1}.matrix;
        eval([GLMType.fit_type 'StimData=StimData;'])
        ...
        for i_cell = 1:length(cells)
            ...
            eval([GLMType.fit_type 'CellData.' cell_savename '.Spikes=organizedspikes.block.t_sp_withinblock;'])
            eval([GLMType.fit_type 'CellData.' cell_savename '.STA=STAandROI.STA;'])
            
        end
        
        % Save data structures
        save([GLMType.fit_type '-' exp_nm '-' 'StimData.mat'],[GLMType.fit_type 'StimData']);
        save([GLMType.fit_type '-' exp_nm '-' 'CellData.mat'],[GLMType.fit_type 'CellData']);
    end
end
