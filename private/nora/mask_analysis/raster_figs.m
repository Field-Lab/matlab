...
for exp = [e];%3]% 5]
    ...
    % 1 = reg, 2 = spot, 3 = gap
    for subgroup = SG
        for i_cell = c%cell_idx{subgroup}
...
figure(1); ylim([1 58]); xlim([1 5]); exportfig(gcf, ['cell' num2str(i_cell) '_raster_full.eps'], 'Bounds', 'loose', 'Color', 'rgb')
                       
figure(2); ylim([2 59]); xlim([1 5]);  exportfig(gcf, ['cell' num2str(i_cell) 'sigma' num2str(i) '_raster_mask.eps'], 'Bounds', 'loose', 'Color', 'rgb')
