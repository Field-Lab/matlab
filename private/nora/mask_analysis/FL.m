...
>>>>>>> 43329a01140ebcb500922a9ecf81e3ed7da014b1
fit_data = 'data012';
...
for i_cell = 1:n_cells
    disp(i_cell)
    load([dsave '2015-12-18-2/WN/' num2str(cells(i_cell)) '.mat']);
    ...
    spikes(1,i_cell) = sum(fittedGLM.xvalperformance.rasters.recorded(:));
    load([dsave piece num2str(cells(i_cell)) 'NSEM.mat']);
    [~, ~, crm_bps, ~] = rastbps_comp_findPS(fittedGLM.xvalperformance.rasters.recorded,fittedGLM.t_bin,fittedGLM.rawfit.ps_basis);
    BPS(2,i_cell) = fittedGLM.xvalperformance.logprob_glm_bpspike/crm_bps;
    spikes(2,i_cell) = sum(fittedGLM.xvalperformance.rasters.recorded(:));
end
