...
%%{
    ...
    for i_cell = 1:size(reg_data{1}.testspikes,2)
        figure; hold on
        for i_reg = 1:n_reg
            IDP_plot_PSTH(reg_data{i_reg}, i_cell, i_reg);
        end
...
for subgroup = 1:n_subgroups
    for i_cell = cell_idx{subgroup}
        for i = 1:length(mask_conditions{subgroup})
            ...
            figure(1); hold on
            IDP_plot_PSTH(condition{mask_conditions{subgroup}(i)},i_cell, color);
            figure(2); hold on
            IDP_plot_PSTH(condition{comp_conditions{subgroup}(i)},i_cell, color);
        end
