...
cell_indices = get_cell_indices(class_datarun, type);

datarun = load_data([ Analysis_Path regA{maskin} '/' regA{maskin}]);
datarun = load_neurons(datarun);
regA_data = interleaved_data_prep(datarun, 1100, 30, 'cell_spec', type, 'datarun_class', class_datarun, 'visual_check', 0);
datarun = load_data([Analysis_Path sigma2{maskin} '/' sigma2{maskin}]);
datarun = load_neurons(datarun);
sigma_data = interleaved_data_prep(datarun, 1100, 120, 'cell_spec', type, 'datarun_class', class_datarun, 'visual_check', 0);
mask_data.testspikes = sigma_data.testspikes(1:30, :);

for i = 1:42
    figure(1)
    IDP_plot_PSTH(regA_data, i);
    hold on; IDP_plot_PSTH(mask_data, i); hold off;
    figure(2)
    imagesc(mask_file.final_mask)
    hold on; plot(class_datarun.vision.sta_fits{cell_indices(i)}.mean(1)*8,160-class_datarun.vision.sta_fits{cell_indices(i)}.mean(2)*8, '*'); hold off
    pause()
end
