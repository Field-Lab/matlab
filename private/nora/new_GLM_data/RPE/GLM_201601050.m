...
EI_map = crossIdentifyNeuronIDs_NB(ref_data, new_data, cells, [] , 0);
cells_test = EI_map(:,2);
good_cell_index = ~isnan(cells_fit) & ~isnan(cells_test);
cells = cells(good_cell_index);
cells_fit = cells_fit(good_cell_index);
cells_test = cells_test(good_cell_index);
...
for i=1:length(cells)
    ...
    class_idx = get_cell_indices(datarun_class, cells_fit(i));
