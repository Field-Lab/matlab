...
%%{
        files = dir(['/Volumes/Lab/Users/Nora/GLMFits/' type '/' folder '/WN/ON*.mat']);
    for i_cell = 1:length(files)
...
%%
for i = 1:8; exp_std(i) = std(cell2mat(Scores(i)')); end
for i = 1:8; exp_means(i) = mean(cell2mat(Scores(i)')); end
b = bar(1:2, exp_means(1:2), 'w', 'EdgeColor', 'r', 'LineWidth', 5);
hold on;
b = bar(3:5, exp_means(3:5), 'w');
b = bar(6:8, exp_means(6:8), 'y');
