function printglmfit_CP(fittedGLM,datarun_slv,printname)
...
if GLMType.CouplingFilters
    ...
    for n_ctype = 1:length(CTYPE)
        [~, parasol_cp_ids{n_ctype}] = celltype_id_AH(CTYPE{n_ctype}, datarun_slv.cell_types);
    end
end
% end NBCoupling
...
if GLMType.CouplingFilters
    ...
    plot(time_msec, oneline, 'k-'); hold on
    for pair=1:6
        plot(time_msec, exp(CP{pair}),'color', 'red','linewidth', 1);
    end
    for pair=7:12
        plot(time_msec, exp(CP{pair}),'color', 'green','linewidth', 1);
    end
    xlim([0, time_msec(end)]);
    ylim([0, 2]);
...
if GLMType.CouplingFilters
    ...
    axis off;
    text(0,0,sprintf('ON-%s: %s %d: %s-Fit', info.celltype))
    bins    = [1:length(CP{1})];
    time_msec = 1000*dt*bins ;
    oneline = ones(1,length(time_msec));
    for pair=1:6
