...
if GLMType.CouplingFilters
    CP = fittedGLM.linearfilters.Coupling.Filter{1};
end
...
if GLMType.CouplingFilters
    ...
    oneline = ones(1,length(time_msec)); 
    plot(time_msec, oneline, 'k-'); hold on
    plot(time_msec, exp(CP),'color', 'b','linewidth', LW); 
    xlim([0, time_msec(end)]);
    ylim([0, max(1.5, max(exp(CP)))]);
    ylabel('gain'); xlabel('msec'); title('Coupling Filter')
end
