...
p.addParameter('stixel_mask', 8)
p.addParameter('mask_type', 'sig') % options are sig: only significant stixels, or est: approx from STA fit
...
p.addParameter('filter_type', 'sig') % options are sig: only significant stixels, or est: approx from STA fit
p.addParameter('thresh', 2.5)
p.addParameter('save_folder', '/Volumes/Lab/Users/bhaishahster/test-tag/')
p.addParameter('save_filename','mask');
...
%% ------------------------------- Load Data ------------------------------------------
datarun.names.rrs_params_path=['/Volumes/Analysis/', classification_file, '.params'];
datarun.names.rrs_sta_path = ['/Volumes/Analysis/', classification_file, '.sta'];
opt=struct('verbose',0,'load_params',1,'load_neurons',0,'load_obvius_sta_fits',false, 'load_sta', 1, 'load_all',false);
...
for i_cell = 1:length(cells)
    ...
    ctr = the_fit.mean;
    rad = p.Results.sigmas*mean(the_fit.sd);
...
new_mask =zeros(size(mask,2),size(mask,1),size(mask,3));
for icell=1:size(mask,3)
new_mask(:,:,icell) = mask(:,:,icell)';
end
mask=new_mask;

final_mask = double(sum(mask,3)>0);
figure; imagesc(final_mask)

 save([save_folder '/' save_filename '.mat'],'final_mask', 'mask','cells')
