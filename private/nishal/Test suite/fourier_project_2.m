function [mov_orig2,mov_new2]=fourier_project_2(stas,mov)
...
tic;
shift_mat=zeros(2*Filtdim1-1,2*Filtdim2-1,movie_len);
for ix=1:Filtdim1*2-1
    for iy=1:Filtdim2*2-1
        for itime=1:movie_len
            shift_mat(ix,iy,itime) = exp(sqrt(-1)*( (2*pi*(ix-1)*(Filtdim1-1)/(2*Filtdim1-1)) +(2*pi*(iy-1)*(Filtdim2-1)/(2*Filtdim2-1)) ) ); 
        end
    end
end
...
ssta=zeros(Filtdim1,Filtdim2,filt_len,n_cell);
sta_mov=zeros(Filtdim1*2-1,Filtdim2*2-1,movie_len,n_cell);
...
sta_mov(1:Filtdim1,1:Filtdim2,1:30,icell)=ssta(:,:,:,icell);
sta_mov_ft(:,:,:,icell)=fftn(squeeze(sta_mov(:,:,:,icell))).*shift_mat;
end
toc;


mov_orig = zeros(Filtdim1*2-1,Filtdim2*2-1,movie_len);
mov_orig (Filtdim1:end,Filtdim2:end,end-movie_time+1:end)= mov;
mov_orig(1:Filtdim1-1,1:Filtdim2-1,:)=0; % make sure its 0 where STAs there ?
...
mov_orig2 = mov_orig(Filtdim1:end,Filtdim2:end,end-movie_time+1:end);
...
% Filter movie 
mov_new_ft_re= zeros(Filtdim1*2-1,Filtdim2*2-1,movie_len);
mov_new_ft_im= zeros(Filtdim1*2-1,Filtdim2*2-1,movie_len);

tic;

parfor iftime=1:movie_len
   %iftime
  [mov_new_ft_re(:,:,iftime), mov_new_ft_im(:,:,iftime)]= proj_null_fourier_2(sta_mov_ft(:,:,iftime,:),n_cell,mov_ft_re(:,:,iftime),mov_ft_im(:,:,iftime));
...
mov_new2 = mov_new(1:Filtdim1,1:Filtdim2,:);
