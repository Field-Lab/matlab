...
totalGen=zeros(length(binsTotal),1);
spksGen=zeros(length(binsTotal),1);
...
for iframe=1:movie_time
    ...
    ibincnt=ibincnt+1;
    totalGen(ibincnt)=exp(gen(iframe) + PostSpikeGen(ibincnt));
    spksGen(ibincnt) = double(rand(1)<totalGen(ibincnt)*binLen);
    mov_frame_number(ibincnt)=iframe;
    
    futureLen=min(postSpikeFilterLen,binsTotal-ibincnt+1);
    if(spksGen(ibincnt)>0)
