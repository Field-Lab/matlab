...
nTrials=1;
% My own STA code 
reSTA=zeros(Filtdim1,Filtdim2,1);
icnt=0;
for itrial=1:nTrials
for iframe=59:1:movie_new_len
    if(binnedResponses(iframe,itrial)>0)
reSTA=reSTA+repmat(mask,[1,1]).*mov_new2(:,:,iframe-4:-1:iframe-4)*binnedResponses(iframe,itrial);
...
reSTC=zeros(Filtdim1*Filtdim2*1);
...
for iframe=framesValid
    iframe
  xx=  reshape(repmat(mask,[1,1]).*mov_new2(:,:,iframe-4:-1:iframe-4),[Filtdim1*Filtdim2*1,1]);
  
reSTC=reSTC+ xx*xx'*binnedResponsesTrial(iframe);
end
end
yy=reshape(reSTA,[Filtdim1*Filtdim2*1,1]);
...
uSq=cell(4,1); 
for isub=1:4
uSq{isub}=reshape(u(:,isub),[Filtdim1,Filtdim2,1]).*repmat(mask,[1,1]);
end


 figure
 
 for itime=1%1:Filtlen
 for isub=1:4
     subplot(2,2,isub);
