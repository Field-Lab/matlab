function [metric,metric_su] = smoothness_metric2(u_spatial_log,mask2,spatial_scale)
...
for isu = 1:size(u_sp_norm,2)
    subplot(1,size(u_sp_norm,2),isu);
     ...
     imagesc(u_spatial);axis square;colormap gray;hold on;
     [r,c] = find(abs(u_spatial) == max(abs(u_spatial(:))));
     u_spatial = u_spatial *sign(u_spatial(r,c));
     
     x=repelem(1:sta_dim1,sta_dim2,1);
     y=repelem([1:sta_dim2]',1,sta_dim1);
