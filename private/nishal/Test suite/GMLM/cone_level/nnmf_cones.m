...
for  cellID=3736%[5251,5476,5701,5851,5911,6076,6151,6241,6332,7036];
    close all
    ...
    for trainFrac = 0.02:-0.002:0.01%0.1:-0.02:0.02 %0.8:-0.2:0.2; % default by Jeremy
        ...
        for   nFilters = nc-1%[nc-6: nc-0] % on average, 2 sub-units!
            if(nFilters<1)
                continue;
            end
            nSU = nFilters;
            ...
            save(sprintf('/Volumes/Lab/Users/bhaishahster/Cone_data_alex/2011-12-13-2/data009_nnmf_diff_amt_data/nnmf_%d_su_%d_data_%0.04f.mat',cellID,nFilters,trainFrac),'su_log','W_log','trainFrac');
