function [fitGLM,f_val,params_iter] = fitGMLM_EM_bias_initPoint(binnedResponses,mov,filteredStimDim2,nFrontEnds2,interval,initialFilters,initialbias)


% initialFilters = 2*(rand(filteredStimDim*nFrontEnds,1)-0.5);
% initalbias = 2*(rand(nFrontEnds,1)-0.5);
...
bias(ifilter)=initialbias(ifilter);%2*(rand(1,1)-0.5);
end
