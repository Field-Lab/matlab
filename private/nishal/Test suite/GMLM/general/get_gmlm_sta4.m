...
function [stas_true,stas_rand] = get_gmlm_sta4(WN_datafile,movie_xml,stim_length,cellID_list,nSU_list,destination,sta_depth,userMov,usrSpks)
...
extract_movie_response6;

%% User input
 mov=userMov;%squeeze(mean(mov,3));
...
for nSU = nSU_list%1:2 %1:10%1:filteredStimDim
 ...
 mov_use=maskedMovdd(:,trainData);
  %mov_use=maskedMovdd2(:,trainData); 
 filteredStimDim=size(mov_use,1); 
 
  [fitGMLM,output] = fitGMLM_EM_bias(binnedResponsesbigd,mov_use,filteredStimDim,nSU,interval);  
...
%[fitGMLM,f_val(nSU)] = fitGMLM_MEL_EM_bias(binnedResponsesbigd,mov_use,filteredStimDim,nSU,interval); 
 %[fitGMLM,output] = fitGMLM_EM_power2(binnedResponsesbigd,mov_use,filteredStimDim,nSU,interval,2);  
