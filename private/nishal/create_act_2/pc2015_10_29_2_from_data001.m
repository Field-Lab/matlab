...
datafile='2015-10-29-2/streamed/data001/data001';%'nishal/2014-08-20-2/data001/data001';

% type_name_inp = 'userCellList' for a list of cells.

no_images_per_movie=10;
start_image=10;
tag='pc2015_10_29_2_from_01';
...
%% Iterated Spatial nulling (low contrast) - has interval parameter - remember to carefully decide the scaling (not stretching) when doing Iterated methods.
cell_params=struct();
cell_params.type_name_inp='OFF Parasol';%'userCellList';
cell_params.cell_list=[];
cell_params.STAlen=14;
...
stim_length=1800;% 
cellID_list = [3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];%1006,1411,2086,2341,2686,4096,4276,4831,5371,5566,5596,5866,7156,7216,7381,7490];
nSU_list= [1:14];
destination= 'pc2015_10_29_2_from_01/sus'
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
nSUs=7*ones(length(cellIDs),1);
[stas_t,stas_r] = get_su_fitted(destination,cellIDs,nSUs)

%% Sub-units correct,7 SU
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
nSUs=11*ones(length(cellIDs),1);
[stas_t,stas_r] = get_su_fitted(destination,cellIDs,nSUs)

%% Sub-units correct,11 SU
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
nSUs=8*ones(length(cellIDs),1);
[stas_t,stas_r] = get_su_fitted(destination,cellIDs,nSUs)

%% Sub-units correct,8 SU
...
cell_params=struct();
cell_params.type_name_inp='userCellList';%'userCellList';
cell_params.cell_list=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
cell_params.STAlen=14;
cell_params.sta_spatial=sprintf('%s/stas_spatial.mat',destination_mat);
cell_params.use_fits=2; % 2, 0,0,2
cell_params.sta_spatial_method=4;%1,2 ,3,4
...
%% 
% save 'totalMaskAccept' and load it below 
load('/Volumes/Lab/Users/bhaishahster/pc2015_10_29_2_from_01/mask.mat');
xx=(movies{42}(repmat((totalMaskAccept>0),[1,1,600])));sd = sqrt(var(xx(:)/255 - 0.5))
...
stim_length=1800;% 
cellID_list = [3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];%1006,1411,2086,2341,2686,4096,4276,4831,5371,5566,5596,5866,7156,7216,7381,7490];
nSU_list= [14,15,16,17,18,19,20,21,22];
destination= 'pc2015_10_29_2_from_01/sus2'
sta_depth=30; % CAREFUL TO CHANGE IT TO LONG .. FOR SLOWER CELLS
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
nSUs=15*ones(length(cellIDs),1);
[stas_t,stas_r] = get_su_fitted(destination,cellIDs,nSUs)

%% Sub-units correct,15 SU
...
movies{49}=mov_orignial;
movies{50}=mov_modify_new;
mov_idx=49;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
mov_idx=50;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);

%% Sub-units wrong, 15 SU
...
movies{51}=mov_orignial;
movies{52}=mov_modify_new;
mov_idx=51;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
mov_idx=52;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
nSUs=17*ones(length(cellIDs),1);
[stas_t,stas_r] = get_su_fitted(destination,cellIDs,nSUs)

%% Sub-units correct,17 SU
...
movies{53}=mov_orignial;
movies{54}=mov_modify_new;
mov_idx=53;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
mov_idx=54;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);

%% Sub-units wrong, 17 SU
...
movies{55}=mov_orignial;
movies{56}=mov_modify_new;
mov_idx=55;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
mov_idx=56;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
...
cellIDs=[3466,544,3227,2135,2312,754, 3287,6841,751,2148,4098,4487,7126];
nSUs=19*ones(length(cellIDs),1);
[stas_t,stas_r] = get_su_fitted(destination,cellIDs,nSUs)

%% Sub-units correct,19 SU
...
movies{57}=mov_orignial;
movies{58}=mov_modify_new;
mov_idx=57;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
mov_idx=58;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);

%% Sub-units wrong, 19 SU
...
movies{59}=mov_orignial;
movies{60}=mov_modify_new;
mov_idx=59;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
mov_idx=60;
write_movie_idx(destination_mat,movies{mov_idx},mov_idx,mov_params.stixel);
