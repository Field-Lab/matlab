function [mov_orig2,mov_new2]=fourier_project128x128(stas,mov)

display('Starting Fourier Computation');
var64=128;%size(stas{1},1);
...
tic;
shift_mat=zeros(2*var64-1,2*128-1,movie_len);
...
%             shift_mat(ix,iy,itime) = exp(sqrt(-1)*( (2*pi*(ix-1)*(128-1)/(2*128-1)) +(2*pi*(iy-1)*31/(2*128-1)) ) ); 
%         end
%     end
% end
...
ix=1:128*2-1;
iy = 1:128*2-1;
shift_mat1 = repmat(( (2*pi*(ix-1)*(128-1)/(2*128-1)))',[1,length(iy)]);
shift_mat2 = repmat((2*pi*(iy-1)*(128-1)/(2*128-1)),[length(ix),1]);
...
ssta=zeros(var64,128,30,n_cell);
sta_mov=zeros(var64*2-1,128*2-1,movie_len,n_cell);
...
sta_mov(1:var64,1:128,1:30,icell)=ssta(:,:,:,icell);
sta_mov_ft(:,:,:,icell)=fftn(squeeze(sta_mov(:,:,:,icell))).*shift_mat;
end
toc;


mov_orig = zeros(var64*2-1,128*2-1,movie_len);
mov_orig (var64:end,128:end,end-movie_time+1:end)= mov;
mov_orig(1:var64-1,1:128-1,:)=0; % make sure its 0 where STAs there ?
...
mov_orig2 = mov_orig(var64:end,128:end,end-movie_time+1:end);
...
% Filter movie 
mov_new_ft_re= zeros(var64*2-1,128*2-1,movie_len);
mov_new_ft_im= zeros(var64*2-1,128*2-1,movie_len);

tic;

parfor iftime=1:movie_len
  %iftime
  [mov_new_ft_re(:,:,iftime), mov_new_ft_im(:,:,iftime)]= proj_null_fourier128x128(sta_mov_ft(:,:,iftime,:),n_cell,mov_ft_re(:,:,iftime),mov_ft_im(:,:,iftime));
...
mov_new2 = mov_new(1:var64,1:128,:);
