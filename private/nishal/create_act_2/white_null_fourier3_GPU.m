...
movie_len=1200;
shift_mat=zeros(2*64-1,2*32-1,movie_len,'gpuArray');
for ix=1:64*2-1
    for iy=1:32*2-1
...
ssta{icell}=gpuArray(squeeze(stas{icell}));
%ssta=zeros(64,32,30);
%ssta(20:40,10:20,1)=1;
ssta{icell}=ssta{icell}(end:-1:1,end:-1:1,:);
sta_mov{icell}=zeros(64*2-1,32*2-1,movie_len,'gpuArray');
sta_mov{icell}(1:64,1:32,1:30)=ssta{icell};
tic;
sta_mov_ft{icell}=gpuArray(fftn(sta_mov{icell}).*shift_mat);
toc;
end
...
mov_orig = zeros(size(sta_mov_ft{1}),'gpuArray');
%mov_orig (64:end,32:end,40:69)= squeeze(stas{3}(:,:,1,:));%randn(64,32,movie_len);
%mov_orig(80:90,40:50,80:160)=10;
mov_orig (64:end,32:end,:)= randn(64,32,movie_len,'gpuArray')*120;
...
mov_orig2 = gather(mov_orig(64:end,32:end,:));
...
mov_ft_re = gpuArray(real(mov_ft));
mov_ft_im = gpuArray(imag(mov_ft));
%mov_orig = ifftn(mov_ft);
%TODO - FFT v/s DFT stuff

% Filter movie 
mov_new_ft_re=zeros(size(sta_mov_ft{1}),'gpuArray');
mov_new_ft_im=zeros(size(sta_mov_ft{1}),'gpuArray');

tic;
A=zeros(2*n_cell,numel(sta_mov_ft{1}(:,:,1))*2,'gpuArray');
for iftime=movie_len:-1:1
   ...
   s_im=cell(2*n_cell,1);
    for icell=1:n_cell*2
        ...
        if(mod(icell,2)==1)
            % s_re and s_im are GPU Arrays?
            s_re{icell} = gpuArray(real(sta_mov_ft{floor((icell-1)/2)+1}(:,:,iftime))); s_re{icell}=s_re{icell}(:);
            s_im{icell} = gpuArray(imag(sta_mov_ft{floor((icell-1)/2)+1}(:,:,iftime))); s_im{icell}=s_im{icell}(:);
            A(icell,:) =[s_re{icell}',-s_im{icell}'];
        else
            A(icell,:)=[s_im{icell-1}',s_re{icell-1}'];
        end
    end
    m_real = gpuArray(mov_ft_re(:,:,iftime));
    m_im = gpuArray(mov_ft_im(:,:,iftime));
    m=gpuArray([m_real(:);m_im(:)]);
...
mov_new2 = gather(mov_new(1:64,1:32,:));
