function [mov]=white_noise(datafile,type_name_inp,destination_mat,movie_white_len)
...
for itime=filt_len:movie_white_len
    ...
    mov_chunk=mov(:,:,itime-filt_len+1:itime-1);
    for icell=1:n_cell
       tot_resp(icell)= sum(sum(sum(squeeze(stas{icell}(:,:,1,end:-1:2)).*mov_chunk)));
    end
    
    init_frame= rand(filt_dim1,filt_dim2);
...
mov(:,:,itime)=reshape(x,[filt_dim1,filt_dim2,1])+init_frame;
end
toc;
