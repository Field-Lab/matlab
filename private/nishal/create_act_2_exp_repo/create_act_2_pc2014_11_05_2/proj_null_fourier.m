...
var64=32;
   ...
   s_im=zeros((var64*2-1)*(32*2-1),2*n_cell);
    for icell=1:n_cell*2
        ...
        if(mod(icell,2)==1)
            s_re(:,icell) = reshape(real(sta_mov_ft_iftime(:,:,1,floor((icell-1)/2)+1)),[(var64*2-1)*(32*2-1),1]); %s_re{icell}=s_re{icell}(:);
            s_im(:,icell) = reshape(imag(sta_mov_ft_iftime(:,:,1,floor((icell-1)/2)+1)),[(var64*2-1)*(32*2-1),1]); %s_im{icell}=s_im{icell}(:);
