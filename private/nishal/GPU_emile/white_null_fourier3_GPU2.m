...
ssta=zeros(64,32,30,n_cell,'gpuArray');
sta_mov=zeros(64*2-1,32*2-1,movie_len,n_cell,'gpuArray');
sta_mov_ft=zeros(size(sta_mov),'gpuArray');
...
mov_orig = zeros(64*2-1,32*2-1,movie_len,'gpuArray');
...
% Filter movie 
mov_new_ft_re= zeros(64*2-1,32*2-1,movie_len,'gpuArray');
mov_new_ft_im= zeros(64*2-1,32*2-1,movie_len,'gpuArray');
...
for iftime=movie_len:-1:1
   ...
   s_re=zeros((64*2-1)*(32*2-1),2*n_cell,'gpuArray');
   s_im=zeros((64*2-1)*(32*2-1),2*n_cell,'gpuArray');
    for icell=1:n_cell*2
   
        if(mod(icell,2)==1)
            s_re(:,icell) = reshape(real(sta_mov_ft(:,:,iftime,floor((icell-1)/2)+1)),[(64*2-1)*(32*2-1),1]); %s_re{icell}=s_re{icell}(:);
            s_im(:,icell) = reshape(imag(sta_mov_ft(:,:,iftime,floor((icell-1)/2)+1)),[(64*2-1)*(32*2-1),1]); %s_im{icell}=s_im{icell}(:);
...
%
mov_new_ft_r=gpuArray(mov_new_ft_r);
