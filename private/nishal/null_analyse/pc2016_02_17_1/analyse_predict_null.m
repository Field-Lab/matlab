...
dataRuns = dataRuns_OFF_null;
  cellType=2;
  ub = 2%0.5; 
  lb= 0%0.4; addivitiy
 movies = movies_OFF_null;
...
rawMovFrames_list=cond_dur*120/4;
icnt=0;
% make pixel histogram
for imov=movies_OFF_null
    %[stim,height,width,header_size] = get_raw_movie(sprintf('/Volumes/Data/2015-10-29-2/Visual/%d.rawMovie',imov),rawMovFrames,1);
    [stim,height,width,header_size] = get_raw_movie(sprintf('/Volumes/Data/2016-02-17-1/Visual/null/pc2016_02_17_1_data028/%d.rawMovie',imov),rawMovFrames_list(icnt+1),1);
    
    subtract_movies{3}=mean(stim,1);
    subtract_movies{3}=mean(stim,1)*0+127.5;
    movie=stim-repmat(subtract_movies{3},[rawMovFrames_list(icnt+1),1,1]);
    movie=movie/255;
...
for itime=3%[1,100]
subplot(3,1,1);
    imagesc(condMov{1}(:,:,itime)');axis image;colormap gray;caxis([0,1]);colorbar
subplot(3,1,2);
    imagesc(condMov{2}(:,:,itime)');axis image;colormap gray;caxis([0,1]);colorbar
subplot (3,1,3);
    imagesc(abs(condMov{1}(:,:,itime)' - condMov{2}(:,:,itime)'));axis image;colormap gray;colorbar;caxis([0,1]);
pause(1/120);
end
...
for cellID =1096% InterestingCell_vis_id;
cols='rkrkrkrkrkrkkrkrkrkr';
...
for icond=[5]%[1,2,3,4,5];
    ...
    [pred1{icnt},lam]= predictGMLM_gamma2_lr(fitGMLM,maskedMov,nTrials,2,1);
    ...
    %PSTH_pred=lam';
    R2_pl = [R2_pl;R_2_value(PSTH_rec(0.2*end:1*end)',PSTH_pred(0.2*end:1*end)')];
    PSTH_rec_log{icnt} = PSTH_rec;PSTH_pred_log{icnt}=PSTH_pred;
end
%close all;
end
