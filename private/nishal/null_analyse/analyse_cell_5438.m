...
a=load('/Volumes/Analysis/nishal/cell5348_2.mat');
b=load('/Volumes/Analysis/nishal/cell5348_small.mat');
c=load('/Volumes/Analysis/nishal/cell5348.mat');
%%
...
waveforms{3}=a.waveforms{3};
spike_times{3}=a.spike_times{3};
spike_amps{3}=a.spike_amps{3};
recon_snippets{3}=a.recon_snippets{3};
...
pltIdx=[1:length(psthData{3})];
figure;
semilogy(timeLog{3}*binSize/(20000),psthData{3}(pltIdx),'b')
hold on
...
%% 
figure;
for itime=1150%1:100:1200
    icnt=0;
for imov=[3,4,6,7,8,9]
    icnt=icnt+1;
    subplot(2,3,icnt);    
    imagesc(reshape(movies{imov}(itime,:,:),[32,32]));
    colormap gray
    colorbar
    title(sprintf('Movie %d',imov));
    axis image
