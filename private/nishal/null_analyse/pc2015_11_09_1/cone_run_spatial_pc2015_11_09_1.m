function [rec_rast_tv,sim_rast,h,x_log]=cone_run_spatial_pc2015_03_09_2(cellID,GLM_fit_link,spkCondColl,n_modelruns,WNdatafile_short)
...
for imodel_run=1:n_modelruns
    imodel_run
    %% Generate response to WN and make a cone+GLM cell model.
    interval=2;
...
%    end
    ...
    x=GLM_predict(fittedGLM, WNmovie, 1); % Generate spikes of normal GLM fitted cell to WN.
    
    % Get Cone signal
     WNmovie255=WNmovie*255;
     WNmovie_cone = movie_cone(WNmovie255,interval);
     
  
     % fit cone WN to response of cell to get GLM fitted after cone NL
     fittedGLM_cone=glm_fit_from_movie_spikes({cellID},WNdatafile_short, 'BW-8-1-0.48-11111-40x40', size(WNmovie_cone,3)/120,'~/Nishal/TS_Data/',WNmovie_cone,find(x.rasters.glm_sim~=0)*(x.rasters.bintime));
      save('~/Nishal/TS_data/ConeCell.mat','WNmovie_cone','WNmovie','fittedGLM_cone');
     % Now, we have fittedGLM_cone, which is the model of a GLM with cone
     % Non-linearity.
     
     %% Generate new WN response to cone+GLM cell.
         interval=2;
    WNtime=120*60*30/interval;
    WNmovie =(0.48/0.5)*(double(rand(32,32,WNtime)>0.5)-0.5)+0.5;
%     if(applyconeNL==1)
...
%    end
    ...
    % Get Cone signal
     WNmovie255=WNmovie*255;
     WNmovie_cone = movie_cone(WNmovie255,interval);
     x=GLM_predict(fittedGLM_cone, WNmovie_cone, 1);
      save('~/Nishal/TS_data/ConeResp.mat','WNmovie_cone','WNmovie','fittedGLM_cone');
...
testmovie255=testmovie*255;
testmovie_cone = movie_cone(testmovie255,interval);
x=GLM_predict(fittedGLM_cone, testmovie_cone, length(spkCondColl(1).spksColl));
