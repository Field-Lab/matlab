function [resp1,resp2] = framework_null_expt_2015_03_09_2(cellID,GIT_link,WN_orig)
...
load(sprintf('%sCellID_%d.mat',GIT_link,cellID));
...
plotSpikeRaster(logical(x.rasters.glm_sim));
    ...
    WNtime=120*15*60;
    WNmovie =double(rand(size(WN_orig,1),size(WN_orig,2),WNtime)>0.5)-0.5;
    WNmovie = repelem(WNmovie,1,1,2);
    x=GLM_predict(fittedGLM, WNmovie+0.5, 1);
    
    %% Calculate STA 
    % WNmovie made 4 dimensional
    WNmov4D = zeros(size(WN_orig,1),size(WN_orig,1),3,WNtime);
