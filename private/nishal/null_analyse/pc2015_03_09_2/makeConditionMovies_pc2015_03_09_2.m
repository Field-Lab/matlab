function makeConditionMovies_pc2015_03_09_2(cond_str,condMovies,datarun,CellID,CondsToWrite)
for icond=1:2
writerObj = VideoWriter(sprintf('/Volumes/Lab/Users/bhaishahster/analyse_2015_03_09_2/Null_Figures_for_EJ/Movie_cell_%d_cond_%d.avi',CellID,icond));
writerObj.FrameRate=120;

scale=20;
%mov=permute(condMovies{CondsToWrite(icond)},[2,3,1]);
mov=condMovies{CondsToWrite(icond)};
...
rowsSelect = abs(rows-centerX)<120;
colsSelect = abs(cols-centerY)<120;
PixelsSelect(flip(colsSelect) ,rowsSelect)=1;
PixelsSelect=logical(PixelsSelect);

for itime=1:size(mov,3)/2

xx = repelem(mov(:,:,itime+130),scale,scale,1); %imresize(mov(:,:,itime+130),scale,'nearest')';
xx(STA_outline)=0;
...
axis image
hgexport(h,sprintf('/Volumes/Lab/Users/bhaishahster/analyse_2015_03_09_2/Null_Figures_for_EJ/STA_CellID_%d.eps',CellID));
