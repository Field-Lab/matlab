...
function [C,U,V,stim,cvx_optval]=cvx_polar_admm_sqrt2(data,data_event,radii_vec,theta_vec,time_sample_len,dict,lam,vec_len,wave_sample_len,stim_avg,mu,y_lag,gam,A_stim)
cvx_begin quiet
...
sum(C)<=20;
minimize (gam*norm((y-data'),2)  + norm(lam.*C,1)+y_lag'*(stim-stim_avg)+ (1/(2*mu))*norm((stim-stim_avg),2))
