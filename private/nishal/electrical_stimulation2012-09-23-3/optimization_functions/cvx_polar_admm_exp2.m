...
function [C,U,V,stim,cvx_optval]=cvx_polar_admm_exp2(data,data_event,radii_vec,theta_vec,time_sample_len,dict,lam,vec_len,wave_sample_len,stim_avg,mu,y_lag,gam,A_stim)
cvx_begin 
...
%norm(C,1) + norm(1-C,1)<=size(C,1); % Fixing the amplitude??
minimize (gam*sum((y-data').^2)  + norm(lam.*C,1) + norm((lam/4).*1-C) +y_lag'*(stim-stim_avg)+(1/(2*mu))*sum((stim-stim_avg).^2))
