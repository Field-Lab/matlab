...
datarun_spikes=datarun.spikes;
spk_coll=cell(nCells,1);

for icell=1:nCells
    icell
    % Next, count spikes in each of these bins.
spk_coll{icell}=binSpks(datarun_spikes{interestingCellIds(icell)},fr);
end
...
% load('/Volumes/Analysis/nishal/recons_2012_08_09_3_data005.mat');
% X=mov;
% 
% % Make movie  
% duration=3600;
...
% mov=mov(:,:,:,1:finalLen);
% for icell=1:nCells
%    spk_coll{icell}=spk_coll{icell}(1:fina lLen); 
% end
% 
% % Play movie ? 
% figure
