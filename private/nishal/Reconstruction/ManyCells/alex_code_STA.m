...
for icell=1:noCells
sta_data(icell).sta=datarun.stas.stas{matlab_ids(icell)};
[sig_stixels, params, rf_strength_out] = significant_stixels(sta_data(icell).sta);
[sig_row,sig_col]=find(sig_stixels==1);
sig_pix=[sig_row,sig_col];
sta_data(icell).sig_stixels=sig_stixels;
sta_data(icell).sig_pix=sig_pix;
end
