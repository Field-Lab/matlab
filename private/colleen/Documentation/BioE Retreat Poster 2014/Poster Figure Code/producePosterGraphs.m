...
parfor j = 1:length(velocity)
    v = velocity(j);
    [strsig1(j)] = -pop_motion_signal_colleen(v, spikes, cell_indices1, cell_indices2, cell_x_pos, tr(1), stop, run_opt.tau, run_opt.tol*.1, datarun);
...
set(gca, 'ytick', []);
set(gca, 'yticklabel', []);
title({'ON Parasol'; 'Motion Signal';'Bright Bar Moving Right'});
hold on
...
legend('Error Function', 'Estimated Speed', 'True Speed')
