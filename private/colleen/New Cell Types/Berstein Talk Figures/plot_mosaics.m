...
% Full path to movie xml
run_opts.movie_spec='/Volumes/Analysis/stimuli/white-noise-xml/RGB-10-2-0.48-11111-64x48.xml';
...
% Use the same spelling/capitalization as the vision params file
% OFF large-2 in vision = OFF large 2
cell_specification = {'ON amacrine amacrine 3'};
...
%     set(fig, 'HandleVisibility', 'on');
 ...
 %%%%%%%%%%%%%%%%%%%%%%% Overlaid Timecourses %%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    y = cell2mat(t_y(logical(index)))';
    % Only plot a certain num_frames
    plot(x((size(x,1)-run_opts.num_frames):size(x,1),:), y((size(x,1)-run_opts.num_frames):size(x,1),:), 'Color', [0 0 0.5])
    ...
    y = cell2mat(t_y(logical(index)))';
    % Only plot a certain num_frames
    plot(x((size(x,1)-run_opts.num_frames):size(x,1), :), y((size(x,1)-run_opts.num_frames):size(x,1),:), 'Color', [1 0 0 0.5])
    ...
    plot(x((size(x,1)-run_opts.num_frames):size(x,1), :), y((size(x,1)-run_opts.num_frames):size(x,1),:),'Color', [0 1 0 0.5])
 ...
 % Get cell array of axes handles to the plots of the stas zoomed in
    ...
    clear
    run_opts.date='2011-08-04-4/'; % one slash at the end
run_opts.concatname='streamed/data003-0'; % Name (or modified name) of run, no slashes

% Sometimes the data has two versions of the concate name
% run_opts.file_name = [run_opts.date, '/', 'data006-nwpca', '/',  'data006/data006'];
run_opts.file_name = [run_opts.date, '/', run_opts.concatname, '/',  'data003-0'];

% Full path to movie xml
run_opts.movie_spec='/Volumes/Analysis/stimuli/white-noise-xml/BW-16-2-0.48-11111-40x30.xml';
...
%     set(fig, 'HandleVisibility', 'on');
 ...
 %%%%%%%%%%%%%%%%%%%%%%% Overlaid Timecourses %%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    y = cell2mat(t_y(logical(index)))';
    % Only plot a certain num_frames
    plot(x((size(x,1)-run_opts.num_frames):size(x,1),:), y((size(x,1)-run_opts.num_frames):size(x,1),:), 'k')
%     
%     hold on
%     % Plot all the red channels together
%     
