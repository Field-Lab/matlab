function plot_sta_fit(sta, fit_params, fixed_params, fit_indices, fixed_indices, sig_stixels)
...
temp_stix = biggestBlob;
   ...
   fit_tc = fit_tc ./ norm_factor;
    subplot(2,1,2)
    if size(sta_fit, 3) == 3
        plot(linspace(1,all_params(20)/3,all_params(20)), fit_tc(:,1), ':r')
        hold on
        plot(linspace(1,all_params(20)/3,all_params(20)), fit_tc(:,2), ':g')
        plot(linspace(1,all_params(20)/3,all_params(20)), fit_tc(:,3), ':b')
    ...
    %real_stix = significant_stixels(sta, 'time', 'max', 'select', 'max', 'thresh', 2.0, 'robust_std_method', 1); % change thres from 3.5 to 3.0
        real_stix = significant_stixels(sta, 'time', 'std', 'select', 'thresh', 'thresh', 2.0, 'robust_std_method', 3); % change thres from 3.5 to 3.0
           real_stix = significant_stixels(sta); % change thres from 3.5 to 3.0
    ...
    tc = tc ./ norm_factor;
    if size(sta_fit, 3) == 3
        plot(linspace(1,all_params(20)/3,all_params(20)),tc(:,1), 'r')
        plot(linspace(1,all_params(20)/3,all_params(20)),tc(:,2), 'g')
        plot(linspace(1,all_params(20)/3,all_params(20)),tc(:,3), 'b')
    elseif size(sta_fit,3) == 1
        plot(tc, 'k')
        hold off
    else
