...
[temp_fit_params{rgc}, sta_new{rgc}, sta_temp{rgc}, sig_stixels{rgc}] = fit_sta(temp_sta, 'fit_n_filters', false, 'fit_surround_sd_scale', false, 'fit_surround', false, 'initial_n_filters', 8, 'interpolate', false, 'frame_number', num_frames);
    % Plot result
    ...
    suptitle({dataset; sprintf('Fit for cell %d', datarun.cell_ids(cellID(rgc)))})
    
    
    if isempty(temp_fit_params)
        temp_id = datarun.cell_ids(cellID(rgc));
        warn_message = ['cell ',num2str(temp_id), ' has no sig stixels and no fit'];
        warning(warn_message)
    end
    
    datarun.matlab.sta_fits{cellID(rgc)} = temp_fit_params{rgc};
    
    
    
    
    parameters(rgc,1) = cell_specification(rgc);
    parameters(rgc,2) = cellID(rgc);
    parameters(rgc,3) = datarun.matlab.sta_fits{cellID(rgc)}.center_point_x;
    parameters(rgc,4) = datarun.matlab.sta_fits{cellID(rgc)}.center_point_y;
    parameters(rgc,5) = datarun.matlab.sta_fits{cellID(rgc)}.center_sd_x;
    parameters(rgc,6) = datarun.matlab.sta_fits{cellID(rgc)}.center_sd_y;
    parameters(rgc,7) = datarun.matlab.sta_fits{cellID(rgc)}.center_rotation_angle;
    parameters(rgc,8) = datarun.matlab.sta_fits{cellID(rgc)}.color_weight_a;
    parameters(rgc,9) = datarun.matlab.sta_fits{cellID(rgc)}.color_weight_b;
    parameters(rgc,10) = datarun.matlab.sta_fits{cellID(rgc)}.color_weight_c;
    parameters(rgc,11) = datarun.matlab.sta_fits{cellID(rgc)}.x_dim;
    parameters(rgc,12) = datarun.matlab.sta_fits{cellID(rgc)}.y_dim;
    parameters(rgc,13) = datarun.matlab.sta_fits{cellID(rgc)}.surround_sd_scale;
    parameters(rgc,14) = datarun.matlab.sta_fits{cellID(rgc)}.surround_amp_scale;
    parameters(rgc,15) = datarun.matlab.sta_fits{cellID(rgc)}.scale_one;
    parameters(rgc,16) = datarun.matlab.sta_fits{cellID(rgc)}.scale_two;
    parameters(rgc,17) = datarun.matlab.sta_fits{cellID(rgc)}.tau_one;
    parameters(rgc,18) = datarun.matlab.sta_fits{cellID(rgc)}.tau_two;
    parameters(rgc,19) = datarun.matlab.sta_fits{cellID(rgc)}.n_filters;
    parameters(rgc,20) = datarun.matlab.sta_fits{cellID(rgc)}.frame_number;
    parameters(rgc,21) = datarun.matlab.sta_fits{cellID(rgc)}.rmse;
...
for q = 1:num_rgcs
    temp_sta = sta_new{q};
    temp_fit_params = datarun.matlab.sta_fits{cellID(q)};
    fit_tc{q} =  plot_fit_timecourses(temp_sta, temp_fit_params.fit_indices, temp_fit_params.fixed_indices, temp_fit_params.fit_params, temp_fit_params.fixed_params, sig_stixels{q}, 1); % plot_raw = 1
    hold on
end
title({['Fits of ' num2str(length(cell_specification))]})
xlabel('Time')
ylabel('Amplitude')
...
%     fit_indices = temp_fit_params{q}.fit_indices;
%     fixed_indices = temp_fit_params{q}.fixed_indices;
%     fit_params = temp_fit_params{q}.fit_params;
%     fixed_params = temp_fit_params{q}.fixed_params;
%     
