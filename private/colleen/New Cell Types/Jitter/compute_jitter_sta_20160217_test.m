...
function [sta] = compute_jitter_sta_20160217_test(datarun, mdf_file, num_frames, spikes, jitter_x, jitter_y,  stixel_size, num_colors,save_path)
...
image_width = 5;
image_height = 5;
real_frame = zeros(image_width, image_height, 1);
real_frame(:,:,:,1) = reshape(inputs(1:image_width*image_height)',image_width, image_height);
    pointer = image_width*image_height+1;
%     pointer = 2+25+2;
    i =2;
while ((pointer+image_height+2)+image_height*(image_width-1))<size(inputs,2)*size(inputs,1)
    temp = inputs(pointer+2:pointer+2+image_height*image_width-1);
    real_frame(:,:,:,i) = reshape(temp, image_width, image_height)';
    pointer = pointer+2+image_height*image_width;
    i = i+1;
end
...
%% Compute movie
movie = zeros(20*stixel_size, 40*stixel_size, 3,duration);
