...
for j = 1:size(spikes,2)
    ...
    frames_needed{j} = zeros(3,upsampled_num_frames ); %% assume triggers every 100 frames
    a = kron(1:ceil(upsampled_num_frames/upsample_factor), ones(1,upsample_factor));
    frames_needed{j}(1,:) = a(1:upsampled_num_frames);
    ...
    for i= 1:length(triggers)-1
        ...
        frames_needed{j}(2, (i-1)*100+1:(i-1)*100+100)= spacing(1:end-1); %% assume triggers every 100 frames
    end
