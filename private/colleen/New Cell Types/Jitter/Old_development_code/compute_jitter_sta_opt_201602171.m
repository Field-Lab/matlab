...
% triggers = [triggers(1:4167); triggers(4168:end) - mean(diff(triggers(1:4167)))];
% triggers = [triggers(1:3450); triggers(3451:end) - mean(diff(triggers(1:3451)))];
% triggers = [triggers(1:2015); triggers(2016:end) - mean(diff(triggers(1:2015)))];
%
% jitter_x= [jitter_x(1:208351); jitter_x(208400:end)]; %1857*100/2 (refresh is 2)+1 : 1857*100/2 (refresh is 2)+50
% jitter_y = [jitter_y(1:208351); jitter_y(208400:end)];
%
% jitter_x= [jitter_x(1:172501); jitter_x(172550:end)]; %1857*100/2 (refresh is 2)+1 : 1857*100/2 (refresh is 2)+50
% jitter_y = [jitter_y(1:172501); jitter_y(172550:end)];
%
% jitter_x= [jitter_x(1:100751); jitter_x(100800:end)]; %1857*100/2 (refresh is 2)+1 : 1857*100/2 (refresh is 2)+50
% jitter_y = [jitter_y(1:100751); jitter_y(100800:end)];
%
% inputs = [inputs(:,1:208351), inputs(:,208400:end)];
% inputs = [inputs(:,1:172501), inputs(:,172550:end)];
% inputs = [inputs(:,1:100751), inputs(:,100800:end)];
%
% for i = 1:size(spikes,2)
%     ind = find(spikes{i} > 208351/60 & spikes{i} <= 208400/60);
%     if ~isempty(ind)
%         spikes{i}(ind(end)+1:end) = spikes{i}(ind(end)+1:end) - (208400/60-208351/60);
%         spikes{i} = [spikes{i}(1:ind(1)-1); spikes{i}(ind(end)+1:end)];
%     end
%
%        ind = find(spikes{i} > 172501/60 & spikes{i} <= 172550/60);
%     if ~isempty(ind)
%         spikes{i}(ind(end)+1:end) = spikes{i}(ind(end)+1:end) - (172550/60-172501/60);
%         spikes{i} = [spikes{i}(1:ind(1)-1); spikes{i}(ind(end)+1:end)];
%     end
%
%        ind = find(spikes{i} > 100751/60 & spikes{i} <= 100800/60);
%     if ~isempty(ind)
%         spikes{i}(ind(end)+1:end) = spikes{i}(ind(end)+1:end) - (100800/60-100751/60);
%         spikes{i} = [spikes{i}(1:ind(1)-1); spikes{i}(ind(end)+1:end)];
%     end
%
%
