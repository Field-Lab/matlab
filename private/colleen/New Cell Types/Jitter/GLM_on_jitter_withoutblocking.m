...
if select_cells == 1
    ...
    dataparam.cell_specification = 4280;
    dataparam.cell_specification_ref = 4326;
...
% --uncomment
% movie = zeros(640,320, size(real_frame,4));
...
num_frames = 30;
movie = zeros(80,80,size(frames_needed{1},2));
  height = 20;
    width = 40;
start_points = [1:10000:size(frames_needed{1},2) size(frames_needed{1},2)];
    try
    for j = 1:length(start_points)-1
        for i = 1:start_points(j+1) - start_points(j)
...
%                 movie(:,:,i+start_points(j) - 1) = sized_frame;
                ...
                movie(:,:,i+start_points(j) - 1) = true_frame(left:right, bottom:top);
...
[STA_full, center] = STA_Test(spikes{1}, movie(:,:,1:2:end), 0, 1/60)
frames{1} = frames_needed_new;
[STA, center] = STA_Test_201602176(spikes{1}, movie(1:2:end), 1, 1/60, frames);
[fittedGLM] = glm_fit(spikes{1}, movie(:,:,1:3:end), center);
