...
% nmruns=['data016'; 'data024']; % NSEM runs
% staruns=['data018'; 'data026']; % long WN runs for STA
% stacoarseruns=['data015'; 'data023']; % coarse long WN runs for STA
% 
% wnruns=['data010'; 'data013']; % WN repeats
% cell_specification = [1833];
...
%     
    ...
    % plot NSEM
    h=subplot('position',[0.05 0.55, 0.9/2,0.3]);
    rasterplot(nmrasters,(num_repeats-1) * size(nmruns,1),movie_length,h)
    for k=1:size(wnruns,1)-1
        line([0,20000], [(num_repeats-1),(num_repeats-1)]*k*1.5,'color','r','linewidth',0.7)
    end
%     for k=1:29
