...
plot_axes = cell(length(cell_indices),1);
for i = 1:length(cell_indices)
        plot_axes{i} = set_up_fig_or_axes(params.foa,params.clear);
    ...
    if size(sta, 3) ~= 3
        ...
        image('Parent',plot_axes{i},'CData',sta(:,:,start_index))
...
%     end
    ...
    hold on
    plot( X,Y,'Color','k', 'LineWidth',1, 'Parent',plot_axes{i})

    
    
    set(plot_axes{i}, 'XLim', bounds(1:2))
        set(plot_axes{i}, 'YLim', bounds(3:4))
    ...
    plot([bounds(1)+1; bounds(1)+stix_scale_bar+1], [bounds(4)-1; bounds(4)-1], '-k', 'LineWidth', 2, 'Parent', plot_axes{i})
