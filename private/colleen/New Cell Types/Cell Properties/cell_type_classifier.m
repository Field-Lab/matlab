...
date = {'2007-01-23-5';'2005-01-21-3';'2007-08-21-4';'2009-04-13-1';'2008-04-08-2';'2005-05-24-1';'2005-07-26-0';'2005-04-26-2'}%'2007-05-01-2'};
 
 

concate = {'data001';'data000';'data000';'data005'; 'data003';'data000'; 'data000';'data002'}%'data000'};
    data= [];
episolon = 0.0001;
for t = 1:size(date,1)
    clear match
...
data_large = data(data(:,1) == 2 | data(:,1) == 3 | data(:,1) == 5 | data(:,1) == 6, :);
data_rows = boolean(floor(sum(~isnan(data_large),2)/size(data_large,2)));
norm_data = data_large(:,2:end)./(repmat(std(data_large(data_rows,2:end)), size(data_large,1),1)+episolon);
norm_data = [data_large(:,1), norm_data];


[COEFF, SCORE, LATENT, TSQUARED, EXPLAINED, MU] = pca(norm_data(:,2:end), 'NumComponents',3);
...
figure;
gscatter(data_large(:,2), data_large(:,5),data_large(:,1))
