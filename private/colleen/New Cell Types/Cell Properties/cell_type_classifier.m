...
for t= 1:size(txt,1)
    ...
    classes = {'ON Parasol', 'ON midget', 'ON large 1', 'ON large 2',  'OFF parasol', 'OFF Large 1', 'OFF Large 2', 'ON LBC'};
    ...
    for c = 1:size(classes,2)
        ...
        if sum(match(c,:))>0
            ...
            elseif c == 2
                ref_on_midget = new_data;
                new_data(:,8) = 0.25;
                new_data = [repmat(t, size(new_data,1),1), new_data];
                
                data = [data; new_data(:, 1:9)];
                real_ind_on_parasol = boolean(floor(sum(~isnan(ref_on_parasol),2)/size(ref_on_parasol,2)));
                real_ind_on_midget = boolean(floor(sum(~isnan(ref_on_midget),2)/size(ref_on_midget,2)));
                midget_parasol_ratio = median(ref_on_parasol(real_ind_on_parasol, [2:7 8:10]))./median(ref_on_midget(real_ind_on_midget, [2:7 8:10]));
            ...
            if c == 3 || c == 8
                ...
                data = [data; new_data(:, 1:9)];
            elseif c == 4
                real_ind = boolean(floor(sum(~isnan(ref_on_midget),2)/size(ref_on_midget,2)));
                new_data(:,[2:7 8:10]) = new_data(:,[2:7 8:10])./repmat(median(ref_on_midget(real_ind, [2:7 8:10])), size(new_data,1),1).*repmat(midget_parasol_ratio, size(new_data,1),1);
                mean_amp = median(ref_on_midget(real_ind, 8));
                if mean_amp < 0
                    new_data(:,8) = -0.25;
                else
                    new_data(:,8) =0.25;
...
data_large = data(data(:,2) == 3 | data(:,2) == 4 | data(:,2) == 6 | data(:,2) == 7| data(:,2) == 8 | data(:,2) == 9, :);
