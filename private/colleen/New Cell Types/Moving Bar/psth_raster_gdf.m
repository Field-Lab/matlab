function [raster_times, raster_marks] = psth_raster_gdf(spikes, triggers, varargin)
...
duration = 3%round(triggers(2) - triggers(1));

for cycle = 1:num_cycles
    begin_window = triggers(cycle) %- duration%triggers(1);
    end_window = triggers(cycle) + duration%triggers(1);
    temp_indices = find(spikes >= begin_window & spikes <= end_window);
    temp_raster_times = spikes(temp_indices);
    temp_raster_times = temp_raster_times - triggers(cycle)%(duration *(cycle-1));
