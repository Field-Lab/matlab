function plot_axes = plot_rf_summaries_colleen(datarun, cell_specification, varargin)
...
%            
            % plot cell ID there
            if i == 17
                cell_num = cell_indices(i);
            text(mean(X)+2,mean(Y)+45, 'A', 'Parent', gca,'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'color', opts.label_color, ...
                'FontSize', opts.label_size, 'FontWeight', 'Bold')
            elseif i == 56
              
                cell_num = cell_indices(i);
             text(mean(X)+3,mean(Y)-15, 'B', 'Parent', gca,'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'color', opts.label_color, ...
                'FontSize', opts.label_size, 'FontWeight', 'Bold')
            hold(opts.axes, 'on');
        end
        end
