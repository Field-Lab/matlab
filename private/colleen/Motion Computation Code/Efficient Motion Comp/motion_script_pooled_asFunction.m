...
function [estimates] = motion_script_pooled_asFunction(varargin)
% USER SET PARAMETERS
    ...
    if run_opt.trial_estimate
        ...
        for i =1:length(tr)
            parfor j = 1:length(velocity)
                v = velocity(j);
                [strsig1(j)] = -pop_motion_signal_colleen(v, spikes, cell_indices1, cell_indices2, cell_x_pos, tr(i), stop, run_opt.tau, run_opt.tol, datarun, run_opt.direction, run_opt.stx);
            end
        ...
        parfor i =1:length(tr)
            ...
            [estimates(i)] = fminunc(@(v) -pop_motion_signal_colleen(v, spikes, cell_indices1, cell_indices2, cell_x_pos, tr(i), stop, run_opt.tau, run_opt.tol, datarun, run_opt.direction, run_opt.stx), run_opt.trial_estimate_start(i), options);
