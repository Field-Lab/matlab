...
chirpPath=dir(['S:\data\alexandra\MEA_data\',date,'\easy_formatted_units\*chirp*']);
load(['S:\data\alexandra\MEA_data\',date,'\easy_formatted_units\',chirpPath(1).name])
...
for unit=1:length(fits)
    ...
    set(gcf,'position',[1  31 1680 946],'color',[1 1 1]*0.8);
     ...
     maxx=max(max([white_flash(2:8,1:4500) black_flash(2:8,1:4500)]));
    for i=1:7
    ...
    load(['S:\data\alexandra\MEA_data\',date,'\easy_formatted_units\',name,'___spike_info_chirp'])
