...
%vector for ISI
isi_all=zeros(1,size(spikes,2));
k=1;
for i=1:length(spikeTimes)
    isi_all(k:k+length(spikeTimes{i})-1)=spikeTimes{i}+isi_all(max(k-1,1))+1;
...
subsetPCAUI=uicontrol('style','pushbutton','string','sub pca');
set(subsetPCAUI,'Units','normalized','position',[0.91 0.92 0.083 0.025],'fontsize',12, 'Callback','manipulate_units(11,get(getXdata,''value''),get(getYdata,''value''))');
