...
for datesCNT=1:15
    ...
    load(['/mnt/muench_data/data/alexandra/MEA_data/analysis/summary_',date],'codeWord','path2save','LinearFilter','dim','units','per','dur')
    path2take=['/mnt/muench_data/data/alexandra/MEA_data/analysis/',date,'/'];
    load([path2take,'protocols_',codeWord])
        
    load([path2save,date,'_nonlinear'],'nonLinear','names')
    nonlin=zeros(61,dim,min(size(LinearFilter,3),24*8),length(units));    
...
for datesCNT=15
    ...
    a=reshape(nonLinear(4,:,:),size(nonLinear,2),size(nonLinear,3));
    a(abs(a)>0.05)=nan;
    a(a==0)=nan;
    tr=size(nonLinear,2)/8;
    cc=1;
    if datesCNT==14
        tr=size(nonLinear,2)/7;
        cc=2;
    end
%     a(:,onOff>0)=[];
    for i=1:tr:size(nonLinear,2)
        subplot(2,4,cc)
%         pt{cc}=[pt{cc} nanmean(a(i:i+tr-1,:))];
        plot(nanmean(a(i:i+tr-1,:)),'.')
% plot(a(i+tr-1,:),'.')
        hold on
        cc=cc+1;
        axis([0,59,-0.,0.05])
...
for i=1:8
    subplot(2,4,i)
    plot(nanmean(pt{i}),'.c','markersize',20)
end
...
for i=1:8
    subplot(2,4,i)
    plot(nanmean(pt{i}),'.m','markersize',20)
end
