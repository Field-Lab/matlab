...
for units_cnt=1:length(units)
    ...
    for trial_counter=1:trialsToTake
        ...
        save(['F:\',date,'\processing\linear_filters_raw_normal_',int2str(filter_length),'\',units(units_cnt).name(1:end-27),'_FFFlicker_linear_filter.mat'],'HighFilters','HighSpikeCount','LowFilters','LowSpikeCount')
    end
end
...
filterspath=['F:\',date,'\processing\linear_filters_raw_normal_',int2str(filter_length),'\'];
