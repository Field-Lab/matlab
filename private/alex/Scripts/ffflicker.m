...
trialsToTake=length(spike_info.spike_times);
...
for units_cnt=1:length(units)
    ...
    for trial_counter=1:trialsToTake
        ...
        HighFilters(1:filter_length,trial_counter)=mean(n);
    end
    save(['F:\',date,'\processing\linear_filters_raw_',int2str(filter_length),'\',units(units_cnt).name(1:end-27),'_FFFlicker_linear_filter.mat'],'HighFilters','HighSpikeCount')
...
filterspath=['F:\',date,'\processing\linear_filters_raw_',int2str(filter_length),'\'];
...
for filters_cnt=1:length(filters)
    ...
    a=regexp(filters(filters_cnt).name,'unit');
    tit=filters(filters_cnt).name(a+5:a+8)
    subplot(4,6,filters_cnt)
    plot(HighFilters)
    axis tight
    title(tit)   
%     
