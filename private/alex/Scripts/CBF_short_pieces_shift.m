...
interval=120; % 1 minutes of data
for pieces=60000:120000:360000 % starting from 2nd minute, 
    for units_cnt=1:length(units)
        a=regexpi(units(units_cnt).name,'unit_(\d+)','tokens');
        a=str2num(char(a{1}));
        if sum(units2take==a)
            ...
            for trial_counter=1:trials
                ...
                spikes(spikes>(interval*1000+pieces))=[];
                spikes(spikes<pieces)=[];
                length(spikes)
            ...
            toc
            save(['/mnt/muench_data/user/alexandra/MEA_data/',date,'/CBF_1000_pieces/',units(units_cnt).name(1:end-27),'_CBFlicker_LF_',int2str(interval),'s_from',int2str(pieces/1000),'s.mat'],'CBFfilter','SpikeCount')
        end
    end
end