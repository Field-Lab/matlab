...
for datesCNT=1:2
    ...
    for i=1:7        
        tmp=white_flash(:,i+st,:);
        tmp=reshape(tmp,4500,length(onOff));
        wf{t}=[wf{t} tmp];
        
        tmp=black_flash(:,i+st,:);
        tmp=reshape(tmp,4500,length(onOff));
        bf{t}=[bf{t} tmp];
    ...
    for i=1:24:24*7
        HC(:,p,tt)=reshape(mean(LinearFilter(:,1,(i+24*st):2:(i+24*st)+23,:),3),500,size(LinearFilter,4));
        LC(:,p,tt)=reshape(mean(LinearFilter(:,1,((i+24*st)+1):2:(i+24*st)+23,:),3),500,size(LinearFilter,4));
        p=p+1;
    end
...
save('/mnt/muench_data/data/alexandra/MEA_data/analysis/summary_rodOnly','peak_HC','peak_LC','HC','LC','bothContr','bf','wf','names','onOff','zc_HC','zc_LC','frMean_HC','frMean_LC','frSTD_HC','frSTD_LC','frMean_spont','frSTD_spont','dates','exp_codes')
clear all

%% Temporal development
load('/mnt/muench_data/data/alexandra/MEA_data/analysis/summary_rodOnly')
...
for datesCNT=1:2
    ...
    HCon(:,:,tt)=reshape(LinearFilter(:,1,k,onOff>0),500,63,sum(onOff>0));
    HCoff(:,:,pp)=reshape(LinearFilter(:,1,k,onOff<0),500,63,sum(onOff<0));
    
end
names=names_all;
onOff=onOff_all;
save('/mnt/muench_data/data/alexandra/MEA_data/analysis/temporal_develop_LF_rodOnly.mat','HCon','HCoff','names','onOff','exp_codes')



%% Plot like M2
clear
load('/mnt/muench_data/data/alexandra/MEA_data/analysis/temporal_develop_LF_rodOnly.mat','HCon','HCoff','names','onOff','exp_codes')
...
for i=1:7
    ...
    for m=1:9
        a=find(nanmean(HCoff(mean(peak_off(i,:)):end,c+m-1,:),3)>0,1)+mean(peak_off(i,:))-1;
        if ~isempty(a)
            zc_off(i,m)=a;
        end
        a=find(nanmean(HCon(mean(peak_on(i,:)):end,c+m-1,:),3)<0,1)+mean(peak_on(i,:))-1;
        if ~isempty(a)
            zc_on(i,m)=a;
        end        
    end
...
axis([0 140 0 450])
set(gca,'xtick',6:20:140,'xticklabel',{'7','6','5','4','3','2','1'})
xlabel('ND')
ylabel('latency, ms')
