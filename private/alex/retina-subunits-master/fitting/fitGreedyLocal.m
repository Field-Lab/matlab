...
%% try fixed subunits
if 0 % ath code
    load('/Volumes/Analysis/2011-10-25-5/subunits/data001-0/fixed_fit_results.mat','all_res')
    found_subs=all_res(3).fit_SUB.I_sc; % FIND WHICH CELL IS WHICH!
    clear combos
    cnt=1;
    for i=1:size(found_subs,1)
        tmp=find(found_subs(i,:));
        if length(tmp)>1
            combos{cnt}=tmp;
            cnt=cnt+1;
        end
    ...
    if length(combos)>1
        ...
        parfor i=1:size(combos,1)
            init_try{i} = mergeRows(init_orig,combos{i});
            [bestfit_try{i} negloglik_try(i) r2_try(i)] = fitABFG(dat,train,test,init_try{i},bestfit);
        end
       
    end
    save('/Volumes/Analysis/2011-10-25-5/subunits/data001-0/test_each_sub/test_','negloglik_orig','r2','best_fit',...
        'combos','bestfit_try','negloglik_try','r2_try','init_try{i}');

else %% try pairs Jeremy code
