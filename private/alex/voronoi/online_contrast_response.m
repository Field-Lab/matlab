function online_contrast_response(inputs, spikes, nbins_cone1, nbins_cone2, center_cones, map, cones, comb, datarunID, cellID,  save_path)
...
%                 marker='x';
%             end
            t = plot(1:2, length(center_cones)+1-[i i], 'linewidth',3, 'color', colors(i,:));
            cnt = cnt+1;
...
%     end
    ...
    saveas(gcf,[path2save,'/', int2str(center_cones(cone1)),'_ID_',int2str(cellID),'.tiff'])
end
