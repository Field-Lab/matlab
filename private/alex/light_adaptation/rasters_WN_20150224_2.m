...
date='2015-02-24-2';
concatname='d00-24-norefit';
finalname='-from-d00-d24';
runs=['data004'; 'data009'; 'data013'; 'data018'];
staruns=['data002'; 'data007'; 'data010'; 'data016'];
% NDF2
% data002 WN
% data003 NSEM
% data004 WN repeats
...
filepath=['/Users/alexth/Desktop/Light_adaptation/WN_rasters_fr_comb/',date,'/',concatname,'/'];
...
figure
plot(diff(trigs))
myTrigs=find(diff(trigs)>0.84&diff(trigs)<2);
...
for i=1:ncells
    ...
    set(gcf,'Visible','off','PaperPosition',[0 0 12 8],'PaperSize',[12 8]);
    set(gcf,'color','white');
    
    h=subplot('position',[0.05 0.55, 0.6,0.4]);
    rasterplot(splitRasters,cnt(4),31000,h)
    for k=1:3
        line([0,31000], [28.25,28.25]+31*(k-1),'color','r','linewidth',0.7)
    end

    set(gca,'ytick',14:28:98,'yticklabel',{'NDF2.0','NDF3.0'})
    set(gca,'xtick',0:5000:30000,'xticklabel',{'0','5','10','15','20','25','30'})
    axis([0 30000 0 Inf])
    ylabel('light level')
    xlabel('WN,s')
    title(['2015-02-24-2, cell ',int2str(datarun.cell_ids(i))])
    ...
    axis([0 30000 0 Inf])
    set(gca,'xtick',0:5000:30000,'xticklabel',{'0','5','10','15','20','25','30'})
    legend('NDF 2.0', 'NDF 3.0','NDF 2.0', 'NDF 3.0');
    ...
    for j=1:4
        ...
        set(h(j), 'XLim',[0.5 16.5],'YLim',[0.5 16.5])
    end
    ...
    plot(sta_tc);
    legend('2.0 first','3.0 first','2.0 second','3.0 second','location','northwest')
    hold on
    ...
    t=['/cell_',int2str(datarun.cell_ids(i))];
    print(gcf,'-dpdf',sprintf('%s%s%s.pdf',[filepath, folder],t));
