...
%   timeWindowStart, a scalar defining the beginning of the time window (in seconds)
%   timeWindowEnd, a scalar defining the end of the time window (in seconds)
%
% returns:
...
%%
[spikeTimes extras] = load_neurons(pathToNeuronsFile, cellIDs);

spikeTimesInWindow = cell(1, length(cellIDs));

for i = 1:length(cellIDs)
    firstSpikeIndex = find(spikeTimes{i} >= timeWindowStart/20000, 1, 'first');
