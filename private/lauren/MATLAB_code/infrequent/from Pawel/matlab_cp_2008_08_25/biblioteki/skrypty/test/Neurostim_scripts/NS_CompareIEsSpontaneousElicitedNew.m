...
Pulse2=[Pulse];
[Amplitude,PlotPointer]=NS_PlotStimulationPulse(Pulse2,Status,Channel,33,X,'b-',NS_GlobalConstants);
set(PlotPointer,'LineWidth',2);
...
FigureNumber=225;
FigureProperties=struct('FigureNumber',FigureNumber,'TimeRange',TimeRange,'AmplitudeRange',[-100 350],'FontSize',FontSize,'Colors',['k' 'r' 'b' 'y']);
Delay=0;
...
signal1=NS_AverageTraces(FileName,Timings(a)-1,Channels,TimeRange,NS_GlobalConstants);
a=find(WaveformTypes==0);
signal2=NS_AverageTraces(FileName,Timings(a)-1,Channels,TimeRange,NS_GlobalConstants);
clear T;
T(1,:,:)=signal1'-signal2';
...
signal=NS_AverageTraces(FileName,Timings1-1,Channels,TimeRange,NS_GlobalConstants);
T(2,:,:)=signal';
