...
figure(51);
subplot(4,1,1);
Amplitude=NS_PlotStimulationPulsesTrain(Pulse,Status,11,100,400,4,20,'m-',NS_GlobalConstants);
gca;
axis([-5 75 -1 1]);
%grid off;
[Pulse,Status]=NS_FindPulseShapeForMovie(FileName,16,MovieNumber,NS_GlobalConstants);
subplot(4,1,2);
Amplitude=NS_PlotStimulationPulsesTrain(Pulse,Status,16,100,400,4,20,'r-',NS_GlobalConstants);
gca;
axis([-15 65 -1 1]);
%grid off;
...
plot(t,(SignalTrace-mean(SignalTrace(60:90))));
axis([490 570 -70 50]);
grid on;
h=gca;
set(h,'FontSize',11);
