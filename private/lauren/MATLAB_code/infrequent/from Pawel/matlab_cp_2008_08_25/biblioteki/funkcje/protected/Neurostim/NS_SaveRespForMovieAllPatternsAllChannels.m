function [PDChunkNumber,MovieBegin,RepetNumber,RepetPeriod]=NS_SaveRespForMovieAllPatternsAllChannels(FileName,WritePath,FigureProperties,NS_GlobalConstants);
...
for i=1:NumberOfMovies-1 %unless specific movie numbers defined!! minus 1 because the last movie is expected to be empty
    ...
    Traces=zeros(NumberOfPatterns,RepetNumber,64,TraceLength);  % always 7 electrodes will be shown - should be more flexible !!!!!!
    ...
    for l=1:NumberOfPatterns % this loop should be running only over patterns used in this movie !!!!
        ...
        a=reshape(TracesToShow,STTS(1)*STTS(2)*STTS(3),1);
        b=zeros(1000,1);
        b(1)=STTS(1);
        b(2)=STTS(2);
        b(3)=STTS(3);
        ...
        o=[b' a'];
        FullName=[WritePath '\' 'p' num2str(l) '_m' num2str(i)]
