...
wzmoc=wzmoc*mnoznik;
korekta=1.11; %tylko na potrzeby wykresu na mixdes!!!!!!
wzmoc=wzmoc*korekta;
wzmoc_dB=dB(wzmoc);
...
semilogx(f(1,3:28)/2,wzmoc_dB(1,3:28)); %bo osobny pomiar;
hold on;
for i=2:64
	if i~=8	& i~=61 & i~=1
		semilogx(f,wzmoc_dB(i,:));
		hold on;
	end
end
...
tx=0.33;
ty=18;
text(tx,ty,['mean:' num2str(mean(offsets),4)]);
text(tx,ty-2,['sigma:' num2str(std(offsets)/mean(offsets)*100,'%3.1f') '%']);
grid on;
