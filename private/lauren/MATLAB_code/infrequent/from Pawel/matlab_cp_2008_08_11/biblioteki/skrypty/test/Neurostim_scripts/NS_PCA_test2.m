function huj=NS_PCA_Test_function(Channel,MovieNumber);
% 1. Global constants
ChipAddresses=[30 31];
...
FigureNumber=15;
FigureProperties=struct('FigureNumber',15,'Subplot',[2 3 3],'TimeRange',[20 50],'AmplitudeRange',[-200 200],'FontSize',10,'Colors',['g' 'r' 'b' 'm' 'k']);
...
text(xstart,ystart-1.5*ystep,['Cluster 2: ' num2str(length(find(TypesNew==2))) '/' num2str(length(find(Types==2))) ' - ' num2str(length(find(TypesNew==2))/length(find(Types==2))*100,3) '%']);
...
%figure(15);
huj=gcf;
set(huj, 'PaperOrientation', 'portrait');
name=['D:\analysis\2008-03-21-0\PCA\Channel' num2str(Channel) 'Movie' num2str(Channel)];
print( huj, '-dtiff', name);