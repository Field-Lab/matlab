...
for kanal=1:6
	...
	for i=1:length(wynik);
		...
		if (start>marg_left & stop<length(s)-marg_right)
			...
			widmo1=abs(fft(spike1))/length(spike1)*2;
			lw1=length(widmo1);
	...
	grid on;
	axis([0 round(fs/2) 0.01 400]);
	xlabel('frequency [Hz]');
	ylabel('fourier coefficient');
	text(7000,320,'2000/12/12');
	text(7000,300,'Data009');
	tekst=[' channel ' num2str(channels-1)];
	text(7000,280,tekst);
	hold off;
end
