function y=NS_PlotClustersOfSignaturesOnArrayLayout(Traces,Channels,WaveformTypes,ArrayID,FigureProperties,NS_GlobalConstants,t);
...
iAnnotation = 0;
%---------------------%
for c=min(WaveformTypes):1:max(WaveformTypes) %%% my version
%for c=max(WaveformTypes):-1:min(WaveformTypes) %%% Pawel's version
    c;
    ...
    %------added--------%
    uicontrol(gcf,'Style','text','String',['cluster ' num2str(c) ', color ' Color ', ' num2str(ll) ' traces'],...
        'Units','normalized','Position',[0.30 0.95-iAnnotation*0.02 0.4 0.02]);
    iAnnotation = iAnnotation + 1;
    %-------------------%
    for i=1:NumberOfChannels
        X=(Coordinates(1,i)-X0)/Xspread+0.05; %0.15;
        Y=(Coordinates(2,i)-Y0)/Yspread+0.05; %0.1;
        XSize=Xstep/Xspread;
        YSize=Ystep/Yspread;
        subplot('Position',[X Y XSize*0.9 YSize*0.8]);
