function varargout = StimScanWatch3(varargin)
% STIMSCANWATCH3 M-file for StimScanWatch3.fig
%      STIMSCANWATCH3, by itself, creates a new STIMSCANWATCH3 or raises the existing
%      singleton*.
%
%      H = STIMSCANWATCH3 returns the handle to a new STIMSCANWATCH3 or the
%      handle to
%      the existing singleton*.
%
%      STIMSCANWATCH3('CALLBACK',hObject,eventData,handles,...) calls the
%      local
%      function named CALLBACK in STIMSCANWATCH3.M with the given input arguments.
%
%      STIMSCANWATCH3('Property','Value',...) creates a new STIMSCANWATCH3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before StimScanWatch3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to StimScanWatch3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help StimScanWatch3

% Last Modified by GUIDE v2.5 20-Oct-2008 23:39:46

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @StimScanWatch3_OpeningFcn, ...
                   'gui_OutputFcn',  @StimScanWatch3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before StimScanWatch3 is made visible.
function StimScanWatch3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to StimScanWatch3 (see VARARGIN)

% Choose default command line output for StimScanWatch3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes StimScanWatch3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = StimScanWatch3_OutputFcn(hObject, eventdata, handles)
...
%% this part is different from Pawel's version!



% remove all the samples when given channel was disconnected!!
% WaveformTypes = manualPCACluster(OutputTraces);
meanDataTraces = mean(DataTraces);
DataTracesWOMean = zeros(size(DataTraces));
for i = 1:size(DataTraces,1)
    DataTracesWOMean(i,:,:) = DataTraces(i,:,:) - meanDataTraces;
end

WaveformTypes = manualClusteringNew(DataTracesWOMean, Channels);
size(WaveformTypes)
...
Radius=str2num(get(handles.edit15,'String'));
Channels=electrodeMap.getAdjacentsTo(CenterChannel,Radius)';
DataTraces=DataTraces(:,Channels,1:35);
...
LowLimit=str2num(get(handles.edit8,'String'));
HighLimit=str2num(get(handles.edit9,'String'));
FigureProperties=struct('FigureNumber',12,'Subplot',[2 3 3],'TimeRange',[5 35],'AmplitudeRange',[LowLimit HighLimit],'FontSize',16,'Colors',['k' 'r' 'b' 'm' 'g' 'c' 'y'],'LineWidth',1,'YLabel','signal [mV]');
...
if SubtractCluster==1
    ...
    TraceToSubtract=mean(DataTraces(a,:,:),1);
    ST=size(DataTraces);
    for i=1:ST(1)
...
LowLimit=str2num(get(handles.edit8,'String'));
HighLimit=str2num(get(handles.edit9,'String'));
FigureProperties=struct('FigureNumber',4,'Subplot',[2 3 3],'TimeRange',[5 65],'AmplitudeRange',[LowLimit HighLimit],'FontSize',16,'Colors',['k' 'r' 'b' 'm' 'g' 'c' 'y'],'LineWidth',1,'YLabel','signal [mV]');
NS_GlobalConstants=NS_GenerateGlobalConstants(61);
...
y=NS_PlotClustersOfSignaturesOnArrayLayout(TracesToShow,Channels,WaveformTypes,1,FigureProperties,NS_GlobalConstants);
