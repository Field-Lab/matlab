function clusterIndeces = manualCluster2D(traces)
...
%
% returns
%   clusterIndeces: a vector (length = number of traces) of ones and twos, denoting which traces
%   are in the category "containing target spikes" (left plot, denoted by a two) or "other traces"
%   (right plot, denoted by a one)
%
% requirements:
...
% 2008-10-24
% edited 2008-11-25: changed clusterIndeces to be 1 for artifact and 2 for spikes (vs. 0 and 1)
...
%% Callbacks for gui
	...
	function moveTracesDummy(hObject, eventdata, H)
		Data = guidata(H.singElecGui);
		Data = moveTraces(hObject, H, Data);
        updateManualClusterAxes(H, Data)
		guidata(H.singElecGui, Data)
    end

    function undoPreviousMove(hObject, eventdata)
        Data = guidata(H.singElecGui);
	...
	function acceptClusters(hObject, eventdata)
		Data = guidata(H.singElecGui);
		Data.clusterIndeces = ones(1,Data.nTraces);
		Data.clusterIndeces(Data.includedIndeces) = 2;
		guidata(H.singElecGui, Data)
		close(H.singElecGui)
	end 
...
clusterIndeces = Data.clusterIndeces;
