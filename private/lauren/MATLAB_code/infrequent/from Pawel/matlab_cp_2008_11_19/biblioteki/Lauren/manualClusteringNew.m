function clusterIndeces = manualClusteringNew(traces, channels)

%Traces - 3-dimensional array: traces*electrodes*samples.
%Channels - includes the number of electrodes to which data in
%"Traces" array correspond.
...
%% Callbacks for gui
    ...
    function electrodeCB(hObject, eventdata, index) %#ok<INUSL>
        ...
        Data.clusterIndeces = manualCluster2D(squeeze(Data.traces(:,H.electrodeChoice,:)));
        guidata(H.mainGui,Data); %so that clusterIndeces can be returned
        close(H.mainGui);
    end
