function [PDChunkNumber,MovieBegin,RepetNumber,RepetPeriod,y]=NS_OverlaidResponseFigureGenerator(FileName,WritePath,FigureProperties,NS_GlobalConstants);
...
% 'period' (period of repetition for each movie, set to 0 if periods are to be definedMovieData
...
for i=1:NumberOfMovies %unless specific movie numbers defined!!
    ...
    elseif ID==[114 -69 27 -4 99 66 -12 -123] %MD (movie data) chunk
        ...
        %scaling factor
        [PDChunkNumber,MovieBegin,nRepeats,RepetPeriod,MovieData]=NS_DecodeMovieDataChunk(ChunkData);

%         keyboard
%         RepetPeriod = 20000;
%         MovieBegin = 220000;
%         if i > 1
%             MovieBegin = (10+50*(i-1))*RepetPeriod;
%         end
    ...
    Traces=zeros(NumberOfPatterns,nRepeats,7,TraceLength);  % always 7 electrodes will be shown - should be more flexible !!!!!!
    ...
    for l=1:NumberOfPatterns % this loop should be running only over patterns used in this movie !!!!
        ...
        TracesToShow=reshape(Traces(l,:,1:length(Channels),:),nRepeats,length(Channels),TraceLength);% might be <7         
        y=NS_PlotManySignaturesOnArrayLayoutNew(TracesToShow,Channels,ones(1,nRepeats),ArrayID,FigureProperties,NS_GlobalConstants);        
        hj=gcf;
