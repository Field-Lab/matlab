...
%     end
    ...
    if ~isempty(elecResp.analysis.type{movieIndex})
        ...
        if sum(failuresBinForArtifact) %some traces have no spikes
            if size(elecResp.cells.mainEI,1) == 512  %LG 1/24/2014 
        ...
        if strcmp('/', filesep) %mac
            if size(elecResp.cells.mainEI,1) == 512  %LG 1/24/2014 mod for 512 electrode array
                channelsToUseBottom = getCluster512(centerElecBottomRow);
            else
