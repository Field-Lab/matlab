...
if analysisMode
    ...
    if get(main.fixedLimsCB, 'value') && ~isempty(elecResp.analysis.details.residCalcWindow{movieIndex}) %update edit boxes to reflect elecResp values
        ...
        for i = 1:length(neuronIDs)
            templates{i} = elecResp.cells.allEIs{elecResp.cells.all == neuronIDs(i)}(elecResp.cells.goodElecs, :);
            templateMinPos(i) = find(squeeze(templates{i}(centInd,:)) == min(squeeze(templates{i}(centInd,:))));
        end
        shiftStartSpikeMin = elecResp.analysis.details.tempOffsetWindow{movieIndex}(1) + max(templateMinPos);
        shiftEndSpikeMin = elecResp.analysis.details.tempOffsetWindow{movieIndex}(2) + min(templateMinPos);
        ...
        if ~isempty(elecResp.analysis.details.residCalcWindow{movieIndex})
            ...
            set(main.offsetLimitLow, 'string', num2str(shiftStartSpikeMin))
            set(main.offsetLimitHigh, 'string', num2str(shiftEndSpikeMin))
            set(main.offsetStep, 'string', num2str(elecResp.analysis.details.tempOffsetStep{movieIndex}))
        end
    end
    
    %latSorted = sort(elecResp.analysis.latencies{movieIndex}(elecResp.analysis.latencies{movieIndex}~=0));
    %disp(latSorted(1:3))
...
if any(plotType == [2 3]) %subtract mean or estimated artifact
    ...
    if analysisMode && analysisAnalyzed
        ...
        if strcmp(elecResp.analysis.type{movieIndex}, 'otherOccurrences')
            otherOccArtTraceToPlot = elecResp.analysis.details.otherOArtifact{movieIndex}(elecResp.cells.recElec, :)' - subtractionVector;
        end
    ...
    if analysisMode && analysisAnalyzed
        ...
        if strcmp(elecResp.analysis.type{movieIndex}, 'otherOccurrences')
            otherOccArtTraceToPlot = elecResp.analysis.details.otherOArtifact{movieIndex}(elecResp.cells.recElec, :)';
        end
    end
end
...
if get(main.lockAxisCheckBox, 'value')
    prevYLim1 = get(main.a1, 'ylim');
    if prevYLim1(2) ~= 1 %default upper limit after using cla
        ymin = prevYLim1(1);
        ymax = prevYLim1(2);
    else
        prevYLim2 = get(main.a2, 'ylim');
        ymin = prevYLim2(1);
        ymax = prevYLim2(2);
    end
end
...
elseif ~analysisMode && exploreAnalyzed %% temporarily analyzed in explore mode (by hand-clustering)
    ...
    text(xmin+1, ymin+0.05*(ymax-ymin), num2str(sum(tempSuccesses~=0)), 'color', [1 0 0])
    hold off
    ...
    markFlagged = get(main.markFlaggedCheckBox, 'Value');
    if markFlagged
        flagged = find(elecResp.analysis.details.analysisFlags{movieIndex});
    else
        flagged = [];
    end
    ...
    if ~isempty(flagged)
        for i = 1:length(flagged)
            if any(latencies(flagged(i),:))
                plot(dataToPlot(flagged(i), :), 'color', [1 0.5 0], 'linewidth', 2)
            end
        end
    end
    ...
    text(xmin+1, ymin+0.05*(ymax-ymin), num2str(sum(latencies(:,1)~=0)), 'color', [1 0 0])
    ...
    %plot TTX artifact if available
    if ~isempty(elecResp.names.artifact_path)
        plot(meanArtTraceToPlot, '--', 'Color', [0.3 0.8 0.3], 'LineWidth', 2)
    end
    
    %plot "other occurences" artifact if available
    if strcmp(elecResp.analysis.type{movieIndex}, 'otherOccurrences')
        plot(otherOccArtTraceToPlot, '--', 'Color', [1 0.6 0.3], 'LineWidth', 2)
    end
...
if analysisMode && analysisAnalyzed
    ...
    set(main.markFlaggedCheckBox, 'Visible', 'on')
    set(main.artFromSubsetCheckBox, 'Visible', 'on')
    set(main.artFromSubsetText, 'Visible', 'on')
    set(main.showTemplatesCheckbox, 'visible', 'on');
    set(main.eiRefreshCheckbox, 'visible', 'off', 'enable', 'off')
    set(main.displayOtherTemplatesButton, 'visible', 'on', 'enable', 'on')
else
    for i = 1:3
        set(eval(['main.legendCheckBox' num2str(i)]), 'Visible', 'off')
    end
    
    set(main.markFlaggedCheckBox, 'Visible', 'off')
    set(main.artFromSubsetCheckBox, 'Visible', 'off')
    set(main.artFromSubsetText, 'Visible', 'off')
    set(main.showTemplatesCheckbox, 'visible', 'off')
    set(main.displayOtherTemplatesButton, 'visible', 'off', 'enable', 'off')
