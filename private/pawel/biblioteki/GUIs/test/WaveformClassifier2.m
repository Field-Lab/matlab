function varargout = WaveformClassifier2(varargin)
% WAVEFORMCLASSIFIER2 M-file for WaveformClassifier2.fig
%      WAVEFORMCLASSIFIER2, by itself, creates a new WAVEFORMCLASSIFIER2 or
%      raises the existing
%      singleton*.
%
%      H = WAVEFORMCLASSIFIER2 returns the handle to a new WAVEFORMCLASSIFIER2 or the handle to
%      the existing singleton*.
%
%      WAVEFORMCLASSIFIER2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WAVEFORMCLASSIFIER2.M with the given input arguments.
%
%      WAVEFORMCLASSIFIER2('Property','Value',...) creates a new WAVEFORMCLASSIFIER2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WaveformClassifier2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WaveformClassifier2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WaveformClassifier2

% Last Modified by GUIDE v2.5 17-Apr-2014 17:42:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WaveformClassifier2_OpeningFcn, ...
                   'gui_OutputFcn',  @WaveformClassifier2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WaveformClassifier2 is made visible.
function WaveformClassifier2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WaveformClassifier2 (see VARARGIN)

% Choose default command line output for WaveformClassifier2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes WaveformClassifier2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = WaveformClassifier2_OutputFcn(hObject, eventdata, handles) 
...
EventID=str2num(get(handles.edit2,'String'))-1
set(handles.edit2,'String',EventID);
Data=ReadData(handles);
NeuronID=Data(1,EventID);
ElectrodeID=Data(2,EventID);
set(handles.edit3,'String',NeuronID);
