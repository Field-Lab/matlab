function varargout = StimScanWatch6_SliceAnalysis_2014_04_01(varargin)
% STIMSCANWATCH6_SLICEANALYSIS_2014_04_01 M-file for StimScanWatch6_SliceAnalysis_2014_04_01.fig
%      STIMSCANWATCH6_SLICEANALYSIS_2014_04_01, by itself, creates a new STIMSCANWATCH6_SLICEANALYSIS_2014_04_01 or raises
%      the existing
%      singleton*.
%
%      H = STIMSCANWATCH6_SLICEANALYSIS_2014_04_01 returns the handle to a new STIMSCANWATCH6_SLICEANALYSIS_2014_04_01 or the
%      handle to
%      the existing singleton*.
%
%      STIMSCANWATCH6_SLICEANALYSIS_2014_04_01('CALLBACK',hObject,eventData,handles,...) calls the
%      local
%      function named CALLBACK in STIMSCANWATCH6_SLICEANALYSIS_2014_04_01.M with the given input
%      arguments.
%
%      STIMSCANWATCH6_SLICEANALYSIS_2014_04_01('Property','Value',...) creates a new STIMSCANWATCH6_SLICEANALYSIS_2014_04_01
%      or raises the
%      existing singleton*.  Starting from the left, property value pairs
%      are
%      applied to the GUI before StimScanWatch6_SliceAnalysis_2014_04_01_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property
%      application
%      stop.  All inputs are passed to StimScanWatch6_SliceAnalysis_2014_04_01_OpeningFcn via
%      varargin.
%
...
% Edit the above text to modify the response to help StimScanWatch6_SliceAnalysis_2014_04_01

% Last Modified by GUIDE v2.5 07-Apr-2014 15:21:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @StimScanWatch6_SliceAnalysis_2014_04_01_OpeningFcn, ...
                   'gui_OutputFcn',  @StimScanWatch6_SliceAnalysis_2014_04_01_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before StimScanWatch6_SliceAnalysis_2014_04_01 is made visible.
function StimScanWatch6_SliceAnalysis_2014_04_01_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to StimScanWatch6_SliceAnalysis_2014_04_01 (see VARARGIN)

% Choose default command line output for StimScanWatch6_SliceAnalysis_2014_04_01
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes StimScanWatch6_SliceAnalysis_2014_04_01 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = StimScanWatch6_SliceAnalysis_2014_04_01_OutputFcn(hObject, eventdata, handles)
...
Radius=str2num(get(handles.edit15,'String'));
Channels=electrodeMap.getAdjacentsTo(CenterChannel,Radius)';
nnnn=size(DataTraces)
FirstSample=str2num(get(handles.edit38,'String'));
OutputTraces=NS_CreateVectorForPCA2(DataTraces(:,:,FirstSample:nnnn(3)),Channels,Pattern,NS_GlobalConstants);
figure(2)
plot(OutputTraces');
% remove all the samples when given channel was disconnected!!
WaveformTypes = manualPCACluster(OutputTraces)';
%bulbul=size(WaveformTypes)
...
SDT=size(DataTraces)
fddfdfdfdf=size(WaveformTypes)
if get(handles.radiobutton8,'Value')
    EI=NS512Read_EI_File(get(handles.edit20,'String'),str2num(get(handles.edit18,'String')),Channels);
    DataTracesOld=DataTraces;
    DataTracesNew=zeros(SDT(1)+1,SDT(2),SDT(3));
    DataTracesNew(1:SDT(1),:,:)=DataTracesOld;
    DataTracesNew(SDT(1)+1,:,:)=EI(:,1:SDT(3));    
    khjkhjkjhkhj=[WaveformTypes' 0]
    size(DataTracesNew)
    y=NS_PlotClustersOfSignaturesOnArrayLayoutPlusEI(DataTracesNew,Channels,[WaveformTypes' 0]',ArrayID,FigureProperties,NS_GlobalConstants);
else
...
switch a
    ...
    otherwise
        'number of electrodes not correct'
end
...
function CrosstalkInfo=ReadCrosstalkFile(CrosstalkFilePath,EventNumber);
fid2=fopen(CrosstalkFilePath,'r');
a=fread(fid2,'integer*2'); %dane 1-8: pierwszy przypadek 
fclose(fid2);
CrosstalkInfoFull=reshape(a,length(a)/8,8);
unique(CrosstalkInfoFull(187:214,1))
length(unique(CrosstalkInfoFull(100:136,1)))
CrosstalkInfo=CrosstalkInfoFull(EventNumber,:);

function EventData=ReadSPPEfile(SPPEfilePath,EventNumber);
fid = fopen(SPPEfilePath,'r');
Data0=fread(fid,'int32');
fclose(fid);
Data=reshape(Data0,3,length(Data0)/3);
EventData=Data(:,EventNumber);

function FirstMovieNumber=FindFirstMovieForPattern(MoviesPatternsDataFile,PatternNumber);
fid = fopen(MoviesPatternsDataFile,'r','ieee-le.l64');
MoviesPatterns=fread(fid,'int32')
fclose(fid);
FirstMovieNumber=MoviesPatterns(PatternNumber)

function ProcessEventNumber(EventNumberChange,handles);
EventNumberOld=str2num(get(handles.edit37,'String'));
EventNumber=EventNumberOld+EventNumberChange
set(handles.edit37,'String',num2str(EventNumber));
%CrosstalkFilePath=get(handles.edit35,'String');
SPPEfilePath=get(handles.edit35,'String');
%CrosstalkInfo=ReadCrosstalkFile(CrosstalkFilePath,EventNumber);
EventData=ReadSPPEfile(SPPEfilePath,EventNumber);
%PrimaryElectrode=

if get(handles.radiobutton18,'Value')
    set(handles.edit2,'String',num2str(EventData(2))); % Pattern Number
    FirstMovieNumber=FindFirstMovieForPattern('D:\Home\Pawel\analysis\slices\2013\2013-12-12-3-PH\data001preproc\MoviesPatterns',EventData(2));
    set(handles.edit1,'String',num2str(FirstMovieNumber)); % Movie Number
    set(handles.edit18,'String',num2str(EventData(1))); % ID of neuron with suspected crosstalk
    
    set(handles.edit14,'String',num2str(EventData(3))); %Primary rec. el. for neuron with crosstalk
    %set(handles.edit30,'String',num2str(140));
end
if get(handles.radiobutton20,'Value')
    ClusterFileNameOld=get(handles.edit10,'String');
    location=findstr(ClusterFileNameOld,'ClusterFile_');
    ClusterFileNameNew=[ClusterFileNameOld(1:location+11) num2str(EventData(1,1))]      
    set(handles.edit10,'String',ClusterFileNameNew);
end

% --- Executes on button press in pushbutton18.
function pushbutton18_Callback(hObject, eventdata, handles)
EventNumberChange=1;
ProcessEventNumber(EventNumberChange,handles);
