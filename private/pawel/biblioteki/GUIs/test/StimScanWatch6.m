function varargout = StimScanWatch6(varargin)
% STIMSCANWATCH6 M-file for StimScanWatch6.fig
%      STIMSCANWATCH6, by itself, creates a new STIMSCANWATCH6 or raises
%      the existing
%      singleton*.
%
%      H = STIMSCANWATCH6 returns the handle to a new STIMSCANWATCH6 or the
%      handle to
%      the existing singleton*.
%
%      STIMSCANWATCH6('CALLBACK',hObject,eventData,handles,...) calls the
%      local
%      function named CALLBACK in STIMSCANWATCH6.M with the given input
%      arguments.
%
%      STIMSCANWATCH6('Property','Value',...) creates a new STIMSCANWATCH6
%      or raises the
%      existing singleton*.  Starting from the left, property value pairs
%      are
%      applied to the GUI before StimScanWatch6_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property
%      application
%      stop.  All inputs are passed to StimScanWatch6_OpeningFcn via
%      varargin.
%
...
% Edit the above text to modify the response to help StimScanWatch6

% Last Modified by GUIDE v2.5 01-Feb-2013 14:23:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @StimScanWatch6_OpeningFcn, ...
                   'gui_OutputFcn',  @StimScanWatch6_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before StimScanWatch6 is made visible.
function StimScanWatch6_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to StimScanWatch6 (see VARARGIN)

% Choose default command line output for StimScanWatch6
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes StimScanWatch6 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = StimScanWatch6_OutputFcn(hObject, eventdata, handles)
...
if get(handles.radiobutton18,'Value')
    set(handles.edit2,'String',num2str(CrosstalkInfo(1))); % Pattern Number
    set(handles.edit1,'String',num2str(CrosstalkInfo(2))); % Movie Number
    set(handles.edit18,'String',num2str(CrosstalkInfo(6))); % ID of neuron with suspected crosstalk
    set(handles.edit14,'String',num2str(CrosstalkInfo(7))); %Primary rec. el. for neuron with crosstalk
    set(handles.edit30,'String',num2str(140));
end
if get(handles.radiobutton20,'Value')
    ClusterFileNameOld=get(handles.edit10,'String');
    location=findstr(ClusterFileNameOld,'ID');
    ClusterFileNameNew=[ClusterFileNameOld(1:location+1) num2str(CrosstalkInfo(6))];            
