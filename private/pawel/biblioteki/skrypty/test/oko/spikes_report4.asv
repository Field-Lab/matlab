...
text(162,655,'Data009');
text(162,550,'channel 64');
grid on;
...
subplot(2,2,3);
plot(time,s);
axis([spike_start spike_stop -1200 800]);
%axis([sp_loc-4 sp_loc+4 -1200 800]);
xlabel('time [miliseconds]');
ylabel('signal value');
grid on;
...
figure(1);
subplot(2,2,4);
s0=s(1,(sp_loc-3.3)*20:(sp_loc+4.7)*20);
dl0=length(s0);
...
for i=1:6
   ...
   subplot(2,3,i);
   plot(t,s,'bd-',t,y0,'g*-');
   axis([sp_loc-0.4 sp_loc+0.2 -1200 0]);
   xlabel('time [miliseconds]');
...
ylabel('signal value');
grid on;
title('Fig. 7: original and 10kHz->20kHz interpolated signal');
...
figure(7);
subplot(2,2,3);
y1=y0(1,(sp_loc-3)*20:(sp_loc+5)*20);
f1=fft(y1)*2/90;
plot(cz0,abs(f0),'b-',cz0,abs(f1),'g-');
axis([0 10000 0 300]);
xlabel('frequency [Hz]');
ylabel('abs. value of Fourier coefficient');
legend('original signal','interpolated signal');
...
xlabel('frequency [Hz]');
ylabel('abs. value of Fourier coefficient');
title('Fig 10: Fourier Transform of the interpolation filter');
grid on;
