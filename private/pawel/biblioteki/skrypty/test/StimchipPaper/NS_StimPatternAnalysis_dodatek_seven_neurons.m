...
for i=1:length(NeuronsIDs)
    ...
    spikes=spikes/N;
    subplot('position',[0.003,0.95-i*0.127,0.09,0.12]);
    for j=1:length(Channels)
        spikes(j,:)=spikes(j,:)-mean(spikes(j,1:2));
    end
    % Normalization:
    spikes=spikes/max(max(abs(spikes)))*70;
    h=NS512_ShowEIAsCircles2(spikes,1,Channels,StimElectrodes(i),MarkedElectrodes(i),[-305 305],[-305 305]);    
