...
% histogramy:
for i=1:8
    h=subplot('position',[0.885,0.965-i*0.1,0.10,0.085]);    
    ...
    %h=ylabel('p [%]');
    set(h,'FontSize',FontSize);
    set(h,'XLim',[0.2 1.2]);       % bylo do 1.0 
    set(h,'XTick',[0.2:0.05:1.2]);
    if i==8
        %set(h,'XTickLabel',{'0.2' '' '' '' '' '' '0.5' '' '' '' '' '' '0.8' '' '' '' '' '' '1.1'});
        set(h,'XTickLabel',{'' '' '0.0' '' '' '' '' '' '' '' '0.4' '' '' '' '' '' '' '' '0.8' '' ''});
    ...
    if i==8
        ...
        set(h,'FontSize',FontSize);    
        h=text(0.45,52,['\sigma=' num2str(round(std(T2)*50)) '/' num2str(round(std(T_crosstalk8)*50)) '\mus']);
        set(h,'FontSize',FontSize);    
        p=length(T1)/IloscImpulsow(i)*100;
        p2=length(T_crosstalk8)/(IloscImpulsow(7)-40)*100;
        h=text(0.45,30,['\epsilon=' num2str(p, '%10.0f') '/' num2str(p2, '%10.1f') '%']);
        set(h,'FontSize',FontSize);
    else
