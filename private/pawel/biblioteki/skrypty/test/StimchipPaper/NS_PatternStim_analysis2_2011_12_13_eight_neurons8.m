...
StimElectrodes=[28 27 37 45 54 51 60 16 3];
RecElectrodes=[28 27 37 45 47 51 58 16 19];
NeuronsIDs=[406 391 541 616 691 736 856 227 271];
SpikesThresholds=[25 45 40 28 50 30 40 100 45 15]; 
ThresholdsForJitter=[25 30 30 40 10 30 40 40 15];
SpikeWidths=[3 4 3 3 8 3 6 7 6];
ThresholdsForJitter2=[50 45 30 40 20 70 60 40 15];
...
subplot('position',[0.11 0.05 0.72 0.91]);
hold on;
...
Croostalk_to_neuron9=zeros(11,30);
T_crosstalk8=[];
T_crosstalk9=[];
...
for i=Elektrody
    ...
    for j=1:length(AllStimElectrodes(1:11))                  
        ...
        for k=1:PulsesNumber                        
            ...
            Croostalk_to_neuron9(j,k)=IloscSpikow;
            if IloscSpikow>0 % jesli                            
                for l=1:length(Types)
                    if Types(l)==2                       
                       ...
                       if AllStimElectrodes(j)==60 && i==8 && k~=8 && k~=19
                           T_crosstalk8=[T_crosstalk8 SpikeTimesPos(kl(1))];
                       end
                       if j==11 && i==8 && k==11
                           PulseTime=PulseTime+12;                                                      
                       end
                       if i==9 && (j==1 | j==11)
                           if SpikeTimesPos(kl(1))>16
                                T_crosstalk9=[T_crosstalk9 SpikeTimesPos(kl(1))];
                           end
                       end
...
set(h,'YTickLabel','');
h=xlabel('Time [ms]');
NS_StimPatternAnalysis_dodatek_eight_neurons3;

%break;
% histogramy:
for i=1:8
    h=subplot('position',[0.88,0.965-i*0.1125,0.10,0.095]);    
    ...
    if i==8
        hold on;
        po2=hist((T_crosstalk8-SpikeWidths(i)-4)/20,[1:1:25]/20);
    ...
    if i==9
        hold on;
        po2=hist((T_crosstalk9-12)/20,[1:1:25]/20);
        bar([1:25]/20,po2/sum(po2)*100,1);
        h = findobj(gca,'Type','patch');
        set(h(1),'FaceColor','g','EdgeColor','k');
    end
    ...
    %h=ylabel('p [%]');
    set(h,'FontSize',FontSize);
    set(h,'XLim',[0.2 1]);       % bylo do 1.0 
    set(h,'XTick',[0.2:0.05:1]);
    if i==8
        %set(h,'XTickLabel',{'0.2' '' '' '' '' '' '0.5' '' '' '' '' '' '0.8' '' '' '' '' '' '1.1'});
        set(h,'XTickLabel',{'' '' '0.0' '' '' '' '' '' '0.3' '' '' '' '' '' '0.8' '' ''});
    ...
    if i==8
        b1=find(T_crosstalk<20);
        T_crosstalk8(b1)=T_crosstalk8(b1)+10;        
        T_crosstalk8=T_crosstalk8-4; %korekta ze wzglêdu na innaszerokoœæ impulsu
        h=text(0.45,74,['\tau=' num2str(round(mean(T2)*50-300)) '/' num2str(round(mean(T_crosstalk8)*50-650)) '\mus']);
        set(h,'FontSize',FontSize);    
        h=text(0.45,53.5,['\sigma=' num2str(round(std(T2)*50)) '/' num2str(round(std(T_crosstalk8)*50)) '\mus']);
        set(h,'FontSize',FontSize);    
        p=length(T1)/IloscImpulsow(i)*100;
        p2=length(T_crosstalk8)/(IloscImpulsow(7)-40)*100;
        h=text(0.45,33,['\epsilon=' num2str(p, '%10.0f') '/' num2str(p2, '%10.1f') '%']);
        set(h,'FontSize',FontSize);
    else
        if i==9
            h=text(0.5,74,['\tau=' num2str(round(mean(T_crosstalk9)*50-900)) '\mus']);
            set(h,'FontSize',FontSize);    
            h=text(0.5,52,['\sigma=' num2str(round(std(T_crosstalk9)*50)) '\mus']);
            set(h,'FontSize',FontSize);    
            p=length(T_crosstalk9)/700*100;
...
%auiergheriugh2
%break
FullName=['C:\pawel\nauka\Stimchip_paper\2012grudzien\dyskusje\figure5_8neurons_proba.tif'];            
h=gcf;
set(h,'PaperUnits','inches');
set(h,'PaperSize',[18.7 15.7]);
set(h,'PaperPosition',[0 0 18.7 15.7]); 
%set(h,'PaperPosition',[0 0 10.7 13.5]); 
print(h, '-dtiff', '-r227', FullName);