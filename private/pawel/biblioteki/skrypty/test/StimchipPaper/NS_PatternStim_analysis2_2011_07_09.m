...
% * * modyfikacja 2011-07-09
SpikeWidths=[0 0 0 0 0 0];
...
DataPath='E:\2010-09-21-0\data017';
DataMovieFile='E:\2010-09-21-0\movie017';
ArtifactDataPath='E:\2010-09-21-0\data020';
ArtifactMovieFile='E:\2010-09-21-0\movie020';

ClusterFilesPath='E:\analysis\2010-09-21-0\ClusterFiles_data017_copies';
        ...
        for k=1:PulsesNumber                        
            ...
            if IloscSpikow>0 % jesli                            
                for l=1:length(Types)
                    if Types(l)==2                       
                       Waveform=DataForNeuron(l,Times0(k)+4:Times0(k)+50)+ThresholdsForJitter(i);
                       %plot(Waveform)
                       %hold on;
                       znak=sign(Waveform);
                       ...
                       % * * * modyfikacja 2011-07-09 - blok 1
                       SpikeTimesPos=find(Waveform==min(Waveform));
                       
                       kl=find(SpikeTimesPos>6);
                       
                       % * * * modyfikacja 2011-07-09 - blok 1
                       kl=find(SpikeTimesPos>3);
...
for i=1:6
    ...
    h=text(0.55,55,['\tau=' num2str(round(mean(T2))*50) '\mus']);
    set(h,'FontSize',FontSize);
    
    h=text(0.55,40,['\sigma=' num2str(round(std(T2)*50)) '\mus']);
    set(h,'FontSize',FontSize);
    
    p=length(T1)/IloscImpulsow(i)*100;
    h=text(0.55,25,['\epsilon=' num2str(p, '%10.1f') '%']);
    set(h,'FontSize',FontSize);
end
%auiergheriugh
break;
FullName=['C:\home\pawel\nauka\Stimchip_paper\obrazki\figure5_sameSF.tif'];            
h=gcf;
