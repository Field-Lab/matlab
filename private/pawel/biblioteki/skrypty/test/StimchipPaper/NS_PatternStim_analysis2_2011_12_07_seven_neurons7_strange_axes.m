...
subplot('position',[0.1 0.17 0.73 0.78]);
%subplot('position',[0.1 0.86 0.73 0.11]);
hold on;
for i=Elektrody([1:7])  
    if i==8
        subplot('position',[0.1 0.02 0.73 0.11]);
        hold on;
    end
    T=[];
    T_crosstalk=[];
    ...
    for k=1:PulsesNumber
        PulseTime=Times0(k);
        if i==8            
            h=plot([PulseTime PulseTime]/20,[100-ElectrodeOrder(i)*100 110-ElectrodeOrder(i)*100],'r-');
        else
...
%auiergheriugh2
break
FullName=['C:\pawel\nauka\Stimchip_paper\2012grudzien\dyskusje\figure5_8neurons2.tif'];            
h=gcf;
