...
NeuronsIDs=[227 391 541 616 691 736 856 76 256 271 406 901];

StimElectrodes=[60 27 37 45 54 51 16 3 6 18 28 61];
%StimElectrodes=[60 27];
RecElectrodes=[58 27 37 45 47 51 16 3 6 18 28 61];
...
mnoznik=8;
Amplitudes=ones(1,12)*mnoznik;
%SpikesThresholds=[100 45 40 28 50 30 40 45]; 
ThresholdsForJitter=[40 30 30 40 10 30 40 20 20 20 20 20];

ThresholdsForJitter2=[60 45 30 40 20 70 40 20 20 20 20 20];
SpikeWidths=[6 4 3 3 8 3 4 4 4 4 4 4];
...
DataPath='K:\2010-09-21-0\data017';
DataMovieFile='K:\2010-09-21-0\movie017';
ArtifactDataPath='K:\2010-09-21-0\data020';
ArtifactMovieFile='K:\2010-09-21-0\movie020';

ClusterFilesPath='K:\analysis\2010-09-21-0\ClusterFiles_data017_copies';
...
for i=Elektrody    
    ...
    for k=1:PulsesNumber
        PulseTime=Times0(k);
        h=plot([PulseTime PulseTime]/20,[740-ElectrodeOrder(i)*50 750-ElectrodeOrder(i)*50],'r-');
        set(h,'LineWidth',2);
    end
            ...
            if IloscSpikow>0 % jesli                            
                for l=1:length(Types)
                    if Types(l)==2                       
                       ...
                       %end
                       h=plot(PulseTime/20,700-ElectrodeOrder(i)*50+l*1.8,'bd');
...
% histogramy:
for i=1:6
    h=subplot('position',[0.785,0.95-i*0.145,0.20,0.11]);    
    ...
    h=text(0.5,74,['\tau=' num2str(round(mean(T2)*50)) '\mus']);
    set(h,'FontSize',FontSize);
...
%auiergheriugh
break
FullName=['C:\home\pawel\nauka\StimchipPaper\figure5c.tif'];            
h=gcf;
