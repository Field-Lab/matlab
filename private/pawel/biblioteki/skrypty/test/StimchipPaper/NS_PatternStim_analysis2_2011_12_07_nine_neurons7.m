...
for i=1:8
    ...
    if i==8
        h=text(0.35,74,['\tau=' num2str(round(mean(T2)*50-300)) '/' num2str(round(mean(T_crosstalk)*50-650)) '\mus']);
        set(h,'FontSize',FontSize);    
        h=text(0.35,52,['\sigma=' num2str(round(std(T2)*50)) '/' num2str(round(std(T_crosstalk)*50)) '\mus']);
        set(h,'FontSize',FontSize);    
        p=length(T1)/IloscImpulsow(i)*100;
        p2=length(T_crosstalk)/IloscImpulsow(7)*100;
        h=text(0.35,30,['\epsilon=' num2str(p, '%10.1f') '/' num2str(p2, '%10.1f') '%']);
        set(h,'FontSize',FontSize);
    else
