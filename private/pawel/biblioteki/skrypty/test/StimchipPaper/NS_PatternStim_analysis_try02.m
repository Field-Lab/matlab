...
StimElectrodes=[16 27 28 37 45 51 61 60 3 6 18 54];
RecElectrodes=[16 27 28 37 45 50 61 58  3 6 18 54];
NeuronsIDs=[227 391 406 541 616 736 901 856 271 76 256 691];

%Amplitudes=[1 7 4 8 9 3 1 5]; %[1 6 3 7 8 3 1];
Amplitudes=ones(1,length(StimElectrodes))*1;
SpikesThresholds=[100 45 40 28 50 30 40 45 1000 1000 1000 1000]; 
%SpikesThresholds=[80 45 40 28 50 30 40]; 

SpikeWidths=[4 3 3 2 4 4 2 4 5 5 5 5];
...
for i=Elektrody(7) %1:NumberOfElectrodes
    ...
    if i==8
        Dane(:,16821:16880)=Dane(:,16821:16880)-20;
    end
    ...
    for j=1:N
        ...
        % 2. Przypadek szczegolny: elektroda 4 oraz 8 - false alarms od artefaktow
        % od innych elektrod
        if i==4
        ...
        if i==8
            FalseTimes=[730 6210 10290 10650 13690 16854];
            for w=1:length(FalseTimes)
                j
                l=find(SpikeTimes>FalseTimes(w)-2 & SpikeTimes<FalseTimes(w)+4)
                SpikeTimes(l)=-100;
        ...
        % 3. Przypadek szczegolny: Wywal amplitudy z okreslonego zakresu,
...
for i=1:NumbersOfElectrodes
    subplot('position',[0.885,0.982-i*0.114,0.10,0.09]);
...
FullName=['C:\home\pawel\nauka\Stimchip_paper\obrazki\figure3c_' num2str(mnoznik) '.tif'];            
h=gcf;
