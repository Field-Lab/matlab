...
for i=1:6
    ...
    h=text(0.55,74,['\tau=' num2str(round(mean(T2)*50)) '\mus']);
    set(h,'FontSize',FontSize);
    
    h=text(0.55,52,['\sigma=' num2str(round(std(T2)*50)) '\mus']);
    set(h,'FontSize',FontSize);
    
    p=length(T1)/IloscImpulsow(i)*100;
    h=text(0.55,30,['\epsilon=' num2str(p, '%10.1f') '%']);
    set(h,'FontSize',FontSize);
end
%auiergheriugh
%break
