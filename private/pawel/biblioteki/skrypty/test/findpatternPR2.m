...
for j=1:6
    ...
    AllMovies = Spikes(:,1);
    subplot(6,1,j);
        fo_ = fitoptions('method','NonlinearLeastSquares','Normalize','on','Robust','On');
...
%         if j<6
%             cf = SigmoidalFit_PR(Spikes(:,1),Efficiency,'Efficiency [%]','');
%         else
%             cf = SigmoidalFit_PR(Spikes(:,1),Efficiency,'Efficiency [%]','Amplitude');
%         end
    plot(cf,'fit',0.95);
    %axis([0 160 0 100]);
