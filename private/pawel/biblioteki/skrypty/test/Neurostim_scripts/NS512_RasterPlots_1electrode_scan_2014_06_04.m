...
if piece==12123
    ...
    FigurePath='D:\Home\Pawel\analysis\slices\2013\2013-12-12-3-PH\data001_Matlab\figures3';
end
...
if piece==12150
    ...
    SpikesSummaryFilePath='D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\data004_Matlab\SpikeTimesCombined\';
    FigurePath='D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\data004_Matlab\figures';
end

if piece==12150008
    full_path='I:\analysis\slices\2013-12-15-0\data008'; %define path to raw data file
    rawFile=edu.ucsc.neurobiology.vision.io.RawDataFile(full_path); 

    MovieFilePath='I:\analysis\slices\2013-12-15-0\movie008';
    neuronFile = edu.ucsc.neurobiology.vision.io.NeuronFile('D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\data008\data008.neurons');
    idList = neuronFile.getIDList(); %this imports list of neurons IDs from the file that is defined above as 'neuronFile';

    NeuronFilePath='D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\data008\data008.neurons';
    DuplicatesFilePath='D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\Duplicates_2013_12_15_0_data008.txt';

    SpikesSummaryFilePath='D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\data008_Matlab\SpikeTimesCombined\';
    FigurePath='D:\Home\Pawel\analysis\slices\2013\2013-12-15-0\data008_Matlab\figures';
end
...
if piece==201009140002
    full_path='G:\analysis\slices\2010-09-14-0\TTX_sub\data002'; %define path to raw data file
    rawFile=edu.ucsc.neurobiology.vision.io.RawDataFile(full_path);        
    
    MovieFilePath='G:\backup1\dane\2010-09-14-0\movie002';
    ...
    SpikesSummaryFilePath='D:\Home\Pawel\analysis\slices\2013\2010-09-14-0\data002_Matlab\SpikeTimesCombined\';
    FigurePath='D:\Home\Pawel\analysis\slices\2013\2010-09-14-0\data002_Matlab\figures';
    
    NumberOfAmplitudes=17;
    LastMovie=136;
    MoviesStep=8;
    ExcludeMovies=[];%sort([17:MoviesStep:LastMovie 18:MoviesStep:LastMovie]);
    MoviesOfInterest=[1:LastMovie];
    RepetitionNumber=50;
end
...
for neuron=[1:length(PrimaryNeurons)]
    ...
    % 1) Find the primary recroding electrode and define the axis ranges
    % for
    % the plot
    ...
    AverageNumberOfSpikesPerPatternAndAmplitude=NumberOfSpikesOfInterest/(length(MoviesOfInterest)*8); %dlaczego dzielone przez 8?????
    ...
    for p=1:length(SuspectedPatterns)% for each pattern tha was associated with a least one spike
        ...
        if Stimulation==1 %jesli flaga jest 1, czyli jesli ten pattern stymulowal ten neuron dla przynajmniej jednej amplitudy...
            ...
            if ceil(MostEfficientMovieID/2)*2==MostEfficientMovieID
                MoviesToShowIDs=[2:2:NumberOfAmplitudes]; %jesli najbardziej efektywna jest amplituda o parzystym ID (czyli druga lub czwartal ub...) to wyswietl na obrazku parzyste amplitudy
            else
                MoviesToShowIDs=[1:2:NumberOfAmplitudes];
            end
            ...
            for m=1:length(MoviesToShowIDs)
                ...
                for PulseID=1:NumberOfPatternUses
                    ...
                    for repetition=1:RepetitionNumber
                        ...
                        spikes(repetition+(PulseID-1)*RepetitionNumber,:)=d1;
                    end
                end
                subplot(6,2,m);
                h=plot(spikes');
            ...
            print(h, '-dtiff', '-r90', FullImageName);
        end
        
    end
    
end             
fid = fopen([FigurePath '\NeuronsPatterns.bin'],'wb');
