...
for i=1:4
    ...
    SamplesToAvoid=(min(x)+3:max(x))+T*20+10000;
    %SignalTrace(1,SamplesToAvoid)=0;
    x=x*0.05-0.35+T;
    y=[0 0 2 2 -3 -3 1 1 0 0];
    Skalowanie=4;
    h=plot(x,y*Skalowanie+60);
    set(h,'LineWidth',2);
    set(h,'Color','r');    
end
...
plot(t-490.5,(SignalTrace-mean(SignalTrace(60:90))-5),'k-');
axis([0 80 -70 80]);
h=gca;
set(h,'YTick',[-60:30:60]);
set(h,'Box','on');

name='C:\home\pawel\2010\Stimchip_paper\figures_two_neurons\TwoNeurons_Channel13';
hc=figure(52);
