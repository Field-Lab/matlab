...
subplot('position',[0.125 0.07 0.72 0.89]);
...
for i=1:NumbersOfElectrodes
    subplot('position',[0.88,0.98-i*0.13,0.10,0.11]);
    ...
    set(h,'XTick',[0:0.1:1]);
    if i==7
        set(h,'XTickLabel',{'0' '' '0.2' '' '0.4' '' '0.6' '' '0.8'});
        xlabel('Spike delay [ms]');
    else
        set(h,'XTickLabel','');
    end
    m=mean(p);
    h=text(0.45,270,['\tau=' num2str(round(m*100/20)*10,'%10.0f') ' \mus']);
    set(h,'FontSize',FontSize2);
    jit=std(p);
    h=text(0.45,225,['\sigma=' num2str(round(jit*50),'%d') ' \mus']);
    set(h,'FontSize',FontSize2);
    h=text(0.45,180,['\epsilon=' num2str(TimesForHist(i,1)/IloscImpulsow(i)/N*100, '%10.1f') '%']);
    set(h,'FontSize',FontSize2);
end
...
break;
FullName=['D:\Home\Pawel\Stimchip_paper\ScatterPlot'];            
h=gcf;
