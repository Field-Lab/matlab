...
Amplitudes=[1 6 3 8 8 3 1]; %[1 6 3 7 8 3 1];
SpikesThresholds=[100 45 40 28 70 30 40]; 
...
for i=Elektrody %1:NumberOfElectrodes
    ...
    for j=1:N
        ...
        if i==4
            ...
            for w=1:length(FalseTimes)
                l=find(SpikeTimes>FalseTimes(w)-2 & SpikeTimes<FalseTimes(w)+2);
                SpikeTimes(l)=-100;
            end                        
        end
    ...
    %A=[2 -3 1];
    %=[0 0 A(1) A(1) A(2) A(2) A(3) A(3) 0 0]+40+6*shift;
    %h=plot(x,y);            
...
set(h,'PaperSize',[11 11]);
set(h,'PaperPosition',[0 0 11 11]); 
