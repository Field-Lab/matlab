...
AmplitudesPerFigure=2;
N0=ceil(N/AmplitudesPerFigure);
T=[1:Length]/20;
for SE=1:length(StimElectrodes);
    Info=NS512_MovieNumberAndTimingForPattern('G:\uncompressed\2010-09-14-0\movie008',StimElectrodes(SE));
    MovieStart=Info(1,1);
    ...
    for M=1:N
        ...
        s3=reshape(b(5,:,:),NumberOfRepetitions,Length);    
        ...
        grid on
        %set(h,'Color','b')
        axis([0 TimeLimit -360 -260]);    
        xlabel('Time [ms]');
        ylabel('Output signal [mV]');
    end
    for f=1:FigureNumber
        figure(f);
        FullName=['D:\Home\Pawel\analysis\2010-09-14-0\figures\data008\stim' num2str(StimElectrodes(SE)) '_' num2str(f)];            
        h=gcf;
