...
figure(6)
for p=49%2:length(PatternsUsed)
    Pattern=PatternsUsed(p);
    Spikes=find(dane(3,:)==Pattern);
    s(p)=length(Spikes);
    dane(4,Spikes);
    HistogramsAll=hist(dane(4,Spikes),[1:60]);
    hist(dane(4,Spikes),[1:60]);
    hold on
    [CFs,leg,moje]=NS512_FitWithMultiGauss_2015_05_26([1:60],HistogramsAll);    
    for i=1:length(CFs)
        h1=plot(CFs{i}([1:60]),'rd-');
    end
end
