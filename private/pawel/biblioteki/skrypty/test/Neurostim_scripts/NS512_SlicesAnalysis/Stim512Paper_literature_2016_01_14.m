...
for i=1:length(lit)
    ...
    if i==length(lit)
        h1=text(str2num(current)*0.7,str2num(f)*1.45,label)
    else
        h1=text(str2num(current)*0.9,str2num(f)*1.45,label)
    end
    ...
    if i==length(lit)
        %h1=text(str2num(charge)*0.7,str2num(charge)/str2num(area)*1.33e-8,label)
        h1=text(str2num(charge)*0.7,str2num(charge)/str2num(area)*1.45e-8,label)
    else
        h1=text(str2num(charge)*0.9,str2num(charge)/str2num(area)*1.45e-8,label)
    end
    set(h1,'FontSize',FontSize1);
    xlabel('Charge [pC]');
...
FullImageName=['C:\pawel\nauka\512paper\figure6.tif'];
h=gcf;
set(h,'PaperUnits','inches');
set(h,'PaperSize',[10 4.8]);
set(h,'PaperPosition',[0 0 10 4.8]);
