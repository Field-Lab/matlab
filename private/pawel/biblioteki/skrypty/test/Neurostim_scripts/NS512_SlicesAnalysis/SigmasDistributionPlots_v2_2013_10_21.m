...
for i=1:length(DirectStimulation)
    ...
    if distance~=0
        ...
        %h17=NS512_ConnectivityLine([x1 x2],[y1 y2],[1 0 0],[0 0 1],NumberOfSteps,[NumberOfSteps-StepsForShortLine:NumberOfSteps]);
    end
end
for i=1:512
...
%grid on
axis([-1000 1000 -500 500]);
FullImageName=[PathForImages 'connectivity_direct.tif'];
h=gcf;
set(h,'PaperUnits','inches');
...
for i=length(InDirectStimulation):-1:1
    ...
    distance=sqrt((x1-x2)^2+(y1-y2)^2);
    if distance~=0
        StepsForShortLine=ceil(70/distance*NumberOfSteps);
        h17=NS512_ConnectivityLine([x1 x2],[y1 y2],[1 0 0],[0 0 1],NumberOfSteps,[1:NumberOfSteps]);
        %h17=NS512_ConnectivityLine([x1 x2],[y1 y2],[1 0 0],[0 0 1],NumberOfSteps,[1:StepsForShortLine]);
        %h17=NS512_ConnectivityLine([x1 x2],[y1 y2],[1 0 0],[0 0 1],NumberOfSteps,[NumberOfSteps-StepsForShortLine:NumberOfSteps]);
    end
...
%grid on
axis([-1000 1000 -500 500]);
FullImageName=[PathForImages 'connectivity_indirect.tif'];
h=gcf;
set(h,'PaperUnits','inches');
set(h,'PaperSize',[10 5]);
set(h,'PaperPosition',[0 0 10 5]); 
