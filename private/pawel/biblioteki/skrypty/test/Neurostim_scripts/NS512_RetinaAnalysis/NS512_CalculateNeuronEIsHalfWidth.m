...
for i=1:length(NeuronIDs)
    EI=NS512Read_EI_File(EIsFilePath,NeuronIDs(i),[1:512]);
    PP=max(EI,[],2)-min(EI,[],2);
    [a,b]=max(PP);
    Waveform=Upsample(EI(b,:),10);
    h(i)=FindNegativeHalfAmplitudeWidth(Waveform-mean(Waveform))/10
end