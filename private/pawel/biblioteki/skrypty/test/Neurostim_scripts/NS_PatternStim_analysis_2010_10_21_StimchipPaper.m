...
for i=Elektrody %1:NumberOfElectrodes
    ...
    for j=1:N
        ...
        % 2. Przypadek szczegolny: elektroda 4 - false alarms od artefakto
        % od innych elektrod
        if i==4
        ...
        for k=1:length(Times0)  
            ...
            l=find(delays>0 & delays<30); % ktore spiki sa opoznione w stosunku do tego konkretnego impulsu o maksymalnie 1.5 ms
        ...
        % 5. Znajdz spikes wywolane przez stymulacje
        for k=1:length(Times0)  
            delays=SpikeTimesPos-Times0(k);
            l=find(delays>0 & delays<30); % ktore spiki sa opoznione w stosunku do tego konkretnego impulsu o maksymalnie 1.5 ms
            if l
                SpikeIndex=SpikeIndex+1;
                %SpikeTimes(l)
                TimesForHist(i,SpikeIndex)=delays(l(1))-SpikeWidths(i)-PulseWidth;  
                if delays(l(1))<8
                    [i j k delays(l(1))];                                 
                end
            end      
