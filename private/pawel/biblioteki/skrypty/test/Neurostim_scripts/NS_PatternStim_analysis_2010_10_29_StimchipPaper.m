...
for i=1:NumbersOfElectrodes
    ...
    m=mean(p);
    text(0.37,270,['\Deltat=' num2str(m/20,'%10.2f') 'ms']);
    jit=std(p);
    text(0.37,225,['\sigma=' num2str(round(jit*50),'%d') '\mus']);
    text(0.37,180,['\epsilon=' num2str(TimesForHist(i,1)/IloscImpulsow(i)/N*100, '%10.1f') '%']);
end
