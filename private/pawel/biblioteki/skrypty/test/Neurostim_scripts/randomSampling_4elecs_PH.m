...
% assignments on each electrode in the pattern. (corrected by PH,
% 2014-06-23)
...
TimesLength      = size(Times,2);
Pattern     = [1:1:nPatterns 1:(TimesLength*MovieChunks-nPatterns)]; 
Pattern     = reshape(Pattern, MovieChunks, []);

MovieChunksToWrite=[];
for i =1:MovieChunks
    Patterns = Pattern(i,1:TimesLength);
    Chunk=NS_MovieChunkGenerationForExperiment(Times, NumberOfSamples, Patterns);
    MovieChunksToWrite = [MovieChunksToWrite Chunk]; %#ok<AGROW> %first value indicates number of chunks
end
