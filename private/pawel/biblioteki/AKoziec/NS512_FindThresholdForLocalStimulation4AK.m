function [AmplitudesVsChannels,RMSVsChannelsAll,SamplesOutput]=NS512_FindThresholdForLocalStimulation4AK_2012_03_08(DataPath,WritePathFigsGood,WritePathFigsBad,PatternNumber,Movies,Channels,ArrayID,FigureProperties,NS_GlobalConstants,MarkedChannels,Samples,SamplesNumber);
        ...
        for j=1:size(TracesWithSpikesInd)
            ...
            for k=1:length(Samples)
                ...
                if mod(SamplesOutput,NumberOfSpikesInFile)==0        %maksymalna ilosc probek to 2 400 000                    
                    nazwa = ['data_05000'  num2str(floor((SamplesOutput-1)/NumberOfSpikesInFile))  '.bin']
                    fid=fopen(nazwa,'a');
            ...
            parametry=cat(1,parametry,[PatternNumber,mn,TracesWithSpikesInd(j)]); %dodac numer elekrtody ze spikiem
            samples512x35xN=cat(2,samples512x35xN,samples512x35);               
        end      
                
                nazwa = ['data_05000'  num2str(floor(SamplesOutput/NumberOfSpikesInFile))  '.bin'];  
