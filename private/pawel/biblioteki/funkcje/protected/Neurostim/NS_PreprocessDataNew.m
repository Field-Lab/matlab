function [PDChunkNumber,MovieBegin,RepetNumber,RepetPeriod,ClusterFileName]=NS_PreprocessDataNew(FileName,WritePath,WritePathFigs,FigureProperties,NS_GlobalConstants,makePlots,MoviesToRead,PatternsToRead,ChannelsToRead);
...
%MoviesToRead,PatternsToRead,ChannelsToRead

for i=1:length(MoviesToRead) %1:NumberOfMovies-1 %unless specific movie numbers defined!! minus 1 because the last movie is expected to be empty. This loop repeats once for each amplitude (in case of scan data).
    ...
    Traces=int16(zeros(NumberOfEvents,RepetNumber,length(ChannelsToRead),TraceLength));  % always 7 electrodes will be shown - should be more flexible !!!!!!    
