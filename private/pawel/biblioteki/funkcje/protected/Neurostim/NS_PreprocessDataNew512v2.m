function [PDChunkNumber,MovieBegin,RepetNumber,RepetPeriod,ClusterFileName]=NS_PreprocessDataNew512v2(FileName,WritePath,WritePathFigs,ArrayID,FigureProperties,NS_GlobalConstants,makePlots,MoviesToRead,PatternsToRead,ChannelsToRead);
...
for i=MoviesToRead %1:8:NumberOfMovies-1 %unless specific movie numbers defined!! minus 1 because the last movie is expected to be empty. This loop repeats once for each amplitude (in case of scan data).    
    ...
    PatternsList=sort(unique(PatternsUsed))
    PatternsToUse=PatternsList(PatternsToRead);  % this will not work when some patterns are used more than once in the movie!!!
              
    NumberOfEvents=length(PatternsToUse);  % this will not work when some patterns are used more than once in the movie!!!
    ...
    for j=1:length(PatternsToUse) %NumberOfPatterns
        [Name,ChannelsStim]=NS_PatternAmplitudes(Patterns,PatternsIndexes,Status,PatternsToUse(j),NS_GlobalConstants);
        Names{j}=Name;
    end            
    ...
    for j=1:RepetNumber % one iteration of this loop takes less than 0.5s for 64-channel data (122 events)                
        ...
        for k=1:NumberOfEvents              
            PatternNumber=PatternsToUse(k);
            index=find(MovieData==PatternNumber);
            %index=(k-1)*3;
            t=MovieData(index-1);
    ...
    ggg=length(PatternsToUse)        
    %f) save output files
    for l=1:length(PatternsToUse) %1:NumberOfPatterns % this loop should be running only over patterns used in this movie !!!!
        ThisPattern=PatternsToUse(l)        
        ...
        if length(WhichEvents)>0            
            ...
            FullName=[WritePath filesep 'p' num2str(ThisPattern) '_m' num2str(i)];
            fid=fopen(FullName,'wb','ieee-le');
                                    
            fwrite(fid,o,'int16');
            fclose(fid);                    
            
            FullName_pattern=[WritePath filesep 'pattern' num2str(ThisPattern) '_m' num2str(i)];            
            ...
            if makePlots==1
                ...
                FullName=[WritePathFigs '\' 'p' num2str(ThisPattern) '_m' num2str(i) Names{l}];
