function [Artifact]=ArtifactEstimation_PR(Traces,N);
%
...
c=min(Traces,[],3);
[d,e]=sort(c,1); %sort traces for each channel independently, from smallest to largest maximum value to idnetify "artifact only" traces

Artifact=zeros(1,ST(2),ST(3));
for i=1:ST(2)
    Artifact(1,i,:)=mean(Traces0(e(ST(1)-N:ST(1),i),i,:));
end