function y=stim_analyze_bez_usr2(filename,dlugosc,okres,stim_channels,figura,marg,zakres,plots);
...
s=readconv(filename,206,65,stim_channels(1,1),[(start) (start+dlugosc-1)]);
sc=s-mean(s);
smin=max(max(abs(sc)));
