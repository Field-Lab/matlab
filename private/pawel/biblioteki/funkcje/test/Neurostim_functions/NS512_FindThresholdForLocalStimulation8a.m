function AmplitudesVsChannels=NS512_FindThresholdForLocalStimulation7a(DataPath,WritePathFigs,PatternNumber,Movies,Channels,ArrayID,FigureProperties,NS_GlobalConstants,MarkedChannels,Samples);
...
%     EventsBeyondTimeWindow = sum(EventsBeyondTimeWindow==2);    %znajduje, ile wystapilo przekroczen progu zbyt wczesnie lub zbyt pozno
    ...
    for i=1:length(ChannelsWithSpikes) % dla wszystkich elektrod z wiecej niz 50 spikami...
        ...
        for i = 1:length(ChannelIndex)           
            ...
            for m=1:size(SpikeUnif, 1)
            TracesMinusMeanSpike(m,:) = SpikeUnif(m,:) - MeanSpike;
            TracesRMS(m) = std(TracesMinusMeanSpike(m,:));
            end
            
            MeanTracesRMS = mean(TracesRMS);    %Srednia wartosc odchylen od przebiegu usrednionego
            MeanSpikePeakToPeak = max(MeanSpike) - min(MeanSpike);
            RMSPercentOfMeanSpike = 100*MeanTracesRMS/MeanSpikePeakToPeak;
        ...
        %sfdgdfg=size(AmplitudeVsChannel)
