function y=NS512_PlotEIOnArrayLayoutStimchipPaper2011_12_06(Traces,Channels,WaveformTypes,ArrayID,FigureProperties,NS_GlobalConstants,MarkedChannels,ElectrodeMap,t);
...
for c=max(WaveformTypes):-1:min(WaveformTypes)
    ...
    for i=1:NumberOfChannels
        X=(Coordinates(1,i)-X0)/Xspread+0.01; %0.15;
        Y=(Coordinates(2,i)-Y0)/Yspread+0.07; %0.1;
        XSize=Xstep/Xspread;
        YSize=Ystep/Yspread;        
        subplot('Position',[X Y XSize*0.92 YSize*0.92]);
        Data=reshape(Traces(TracesToPlot,i,:),length(TracesToPlot),TraceLength);
        h=plot(t,Data');
        numerek=find(MarkedChannels==Channels(i));
        if numerek
            set(h,'Color',Colors(2));  
            h11=text(0.13,83,num2str(ElectrodeMap(Channels(i))));
            set(h11,'FontSize',FontSize);
            set(h11,'BackgroundColor','w');
        else
        ...
        if i==SE
            ...
            %h=text(0.8,2500,num2str(Channels(i)));
            set(h,'FontSize',FontSize);
            %h=xlabel(num2str(Channels(i)));           
        end   
