...
for MovieNumber=Movies        
 ...
 %Wykrywanie spikow metoda dwoch progow:
    ...
    for i=1:length(ChannelsWithSpikes)
        ...
        %plot((ChannelTraces2D(TimeCorelSpikes==1, :))');
        
        %figure(3);
        %ddd = plot((ChannelTraces2D((UniSpikesIndic~=0) & (TimeCorelSpikes==2), :))');
        %TracesWithoutArtefacts = ChannelTraces2D(TimeCorelSpikes==1,:);
...
%             grid on;
        ...
        %fff=size(Traces)
        display('rozmiary macierzy Traces, ChannelsToRead, WaveformTypes');
        size(Traces)
        size(ChannelsToRead)
        ChannelsToRead
        size(WaveformTypes)
