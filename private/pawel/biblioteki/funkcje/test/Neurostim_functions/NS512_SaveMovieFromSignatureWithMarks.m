function M=NS512_SaveMovieFromSignatureWithMarks(Traces,Channels,ElectrodeToShow,ArrayID,FigureProperties,NS_GlobalConstants,MarkedChannels);
...
for frame=1:NumberOfFrames %for each frame...
    ...
    for i=1:ChannelsNumber
        ...
        ElToShow=find(MarkedChannels==Channels(i));
        if ElToShow
            text(X,Y,num2str(MarkedChannels(ElToShow)));
        end
