function [GX,GY,GZ]=NS_EstimatePotentialGradient(Amplitudes,elX,elY,elZ,x0,y0,z0);
...
for i=1:length(Amplitudes)
    G(i)=Amplitudes(i)/((elX(i)-x0)^2+(elY(i)-y0)^2+(elZ(i)-z0^2)); %absolute value    
    Glength=sqrt((elX(i)-x0)^2+(elY(i)-y0)^2+(elZ(i)-z0)^2);
    Gx(i)=G(i)*(elX(i)-x0)/Glength;
    Gy(i)=G(i)*(elY(i)-y0)/Glength;
    Gz(i)=G(i)*z0/Glength;
    GX=GX+Gx(i);
    GY=GY+Gy(i);
    GZ=GZ+Gz(i);
