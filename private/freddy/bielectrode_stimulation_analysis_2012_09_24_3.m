...
[~,~,singElecVals] =find(activationThresh_singleElec(:));
[~,~,biElecVals] =find(activationThresh_bielec1(:));
tag1 = 'Single electrode';
tag2 = 'Bi - electrode  ';
values = [singElecVals; biElecVals]; 
tags = [repmat(tag1,length(singElecVals),1); repmat(tag2,length(biElecVals), 1)]; 
figure; boxplot(values,tags); set(gcf,'Color','white'); 
ylabel('|activation threshold (uA)|');
xlabel('stimulation type'); 
title('somatic activation thresholds'); 

biThresh = axonBundleThresholds(1:2:240); 
singThresh = axonBundleThresholds(241:end); 
values = [singThresh; biThresh]; 
tags = [repmat(tag1,length(singThresh),1); repmat(tag2,length(biThresh), 1)]; 
figure; boxplot(values,tags); set(gcf,'Color','white'); 
ylabel('|activation threshold (uA)|');
xlabel('stimulation type'); 
title('axon bundle activation thresholds'); 
...
biThresh = axonBundleThresholds(1:2:240); 
singThresh = axonBundleThresholds(241:end); 
tag1 = 'Single electrode somatic';
tag2 = 'Bi - electrode   somatic';
tag3 = 'Single electrode  axonal';
tag4 = 'Bi - electrode    axonal';

tags = [repmat(tag1,length(singElecVals),1); ...
    repmat(tag2,length(biElecVals), 1); ...
    repmat(tag3,length(singThresh), 1); ...
    repmat(tag4,length(biThresh), 1)]; 

values = [singElecVals; biElecVals; singThresh; biThresh]; 
figure; boxplot(values,tags); set(gcf,'Color','white'); 
ylabel('|activation threshold (uA)|');
xlabel('stimulation type'); 
