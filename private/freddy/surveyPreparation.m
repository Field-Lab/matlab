...
% Last Modified by GUIDE v2.5 09-Sep-2014 15:09:28
...
handles.overlaps = findOverlappingNeurons(datarun, handles.spikeThresh, cellIdsToCheck);
...
data = [data goodCol goodCol num2cell(handles.overlaps(:,size(handles.overlaps,2)))]; 
...
for n = 1:1:size(table_data,1)
    ...
    if table_data{n,6}
        for nn = 1:(size(handles.overlaps, 2)-1)
            if handles.overlaps(n, nn)
                cellID = handles.overlaps(n, nn); 
                cellIndex = get_cell_indices(handles.datarun, cellID);
                ei = handles.datarun.ei.eis{cellIndex}'; % squeeze(ei(1,2:end,:))';
                eiAmps = max(ei)-min(ei);
                scatter(positions(:,1),positions(:,2),3*eiAmps+0.1,'filled');
                elec = table_data{cell2mat(table_data(:,1)) == cellID,2}; 
                text(positions(elec,1),positions(elec,2),num2str(cellID)); 
            end
        end
    end
...
figure; axis image; contour(ei2matrix(sum),24); axis ij; 
[EIm, EIview] = ei2matrix(sum);
figure; axis image; contourf(EIview,24); axis ij; 