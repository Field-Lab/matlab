...
% mgf10 KO Animal
datamb{1} = load_data('/Volumes/lab/Experiments/Array/Analysis/2015-09-02-0/data001/data001', opt);
datamb{1}.names.stimulus_path = '/Volumes/lab/Experiments/Array/Analysis/2015-09-02-0/stimuli/s01.mat';
datamb{1} = load_stim_matlab(datamb{1});

% mgf10 HET Animal
datamb{1} = load_data('/Volumes/lab/Experiments/Array/Analysis/2015-06-09-0/data001/data001', opt);
datamb{1}.names.stimulus_path = '/Volumes/lab/Experiments/Array/Analysis/2015-06-09-0/stimuli/s01.mat';
datamb{1} = load_stim_matlab(datamb{1});
...
% get the triggers associated with the slow speed gratings
% epoch logic is a little fucked up, have an ad hoc way to generate trigger
% look up


num_conditions = length

slow_bar_triggers = [];
for rep = 1:num_reps
    temp_triggers = slow_indices + rep + ((rep-1) * num_conditions);
    slow_bar_triggers = [slow_bar_triggers; temp_triggers];
end

direction_list = datamb{1}.stimulus.params.DIRECTION;
for dr = 1:length(direction_list)
    spikes_by_trial = get_raster(datamb{1}.spikes{temp_index}, datamb{1}.triggers(slow_bar_triggers));
