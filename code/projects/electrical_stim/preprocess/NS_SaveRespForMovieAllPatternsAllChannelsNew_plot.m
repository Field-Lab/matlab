function [PDChunkNumber, MovieBegin, nRepeats, repeatPeriod]=NS_SaveRespForMovieAllPatternsAllChannelsNew_plot(FileName, WritePath, FigureProperties, NS_GlobalConstants, traceLength)
...
for i=1:NumberOfMovies-1 %unless specific movie numbers defined!! minus 1 because the last movie is expected to be empty
    ...
    for j=1:nRepeats %iterates through number of times movie is played
        ...
        for k = 1:nEvents
            ...
            Name=NS_PatternAmplitudes(Patterns,PatternsIndexes,Status,PatternNumber,NS_GlobalConstants);
    ...
    for l=1:NumberOfPatterns % this loop should be running only over patterns used in this movie !!!!
        ...
        if ~isempty(WhichEvents)
            ...
            TracesToShow=reshape(Traces(WhichEvents,:,1:length(Channels),:),nRepeats*length(WhichEvents),length(Channels),traceLength);
            %STTS=size(TracesToShow);
