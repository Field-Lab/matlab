...
%
% This function preprocesses raw electrical stimulation data as it is being
% written
% 
% NS_SaveRespForMovieAllPatternsAllChannels_stream(...) will determine the 
...
% Read through the header. (reading the header also properly advances the
% pointer)
header_m=readMHchunk(fid0);
% Number of movie chunks x number of amplitudes.
nMovies=header_m.number_of_movies; 

if nMovies == 0
    disp('nMovies is 0 ... set to 500 until I think of something better'); 
    nMovies = 500; 
end
...
for i=1:nMovies-1 % the last movie is expected to be empty.
    ...
    % Update total number of samples continually for analyzing
    % streaming data
    ...
    elseif ID==[114 -69 27 -4 99 66 -12 -123] % movie data chunk ID.
        ...
        % Read in the movie data, which gives the times at which patterns are played
    ...
    for j=1:numRepetitions 
        ...
        % Checks that the raw data file has all of the samples that the stimulus files specify
        if totalSamples <= movieBegin+repetitionPeriod*j+traceLength
            if streaming
                while streaming && totalSamples <= movieBegin+repetitionPeriod*j+traceLength
                    %Check to see if more samples have been written
                    rawFile=edu.ucsc.neurobiology.vision.io.RawDataFile(rawDataPath);
                    header = rawFile.getHeader();
                    totalSamples = header.getNumberOfSamples();
                    if previousTotal == totalSamples
                    ...
                    previousTotal = totalSamples;
                    disp('pausing for data to arrive...'); pause(30);
                end
            else
                err = MException('MATLAB:rawDataMissingSamples', ...
                    ['Raw data file ' rawDataPath ' does not have as many samples as expected by stimulus files']);
