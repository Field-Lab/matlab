...
while(flag==1&&cont<=maxIter)
   ...
   if(elExtra(i)==stimElec)
   ArtF(:,stimElec,:)=Art(1,stimElec,1:Tmax);
   else
     ArtF(:,stimElec,:)=FilterArtifactLocalStim(KersStim,squeeze(Art(1,stimElec,1:Tmax))',[xStim log(var0)],i,QStim,QtStim,krondiaginvStim);
   end
    ...
    ArtF(1,stimElec,:)=FilterArtifactLocalStim(KersStim,squeeze(Art(1,stimElec,1:Tmax))',[xStim log(var0)],1,QStim,QtStim,krondiaginvStim);
    else
        ArtF(1,stimElec,:)=Art(1,stimElec,:);
    end
...
i
    if(i>=ionset&&ionset>1)
        ...
        if(i==ionset&&ionset>1)
            ...
            DiagsPre{3}=DiagsPre{3}(ionset-1);

            DifsPre{3}=DifsPre{3}(ionset-1,ionset-1);



            f11=@(Art,x)logDetKron(Art(ionset-1,ind,:),[xold(1:9)  x log(var0)],DifsPre,[10],[1 1 1],DiagsPre,[3 3 3]);
             g11=@(x)f11(Art,x);
             factor= fminunc(g11,[xold(end)],options);
            x(end)=0;
        
        
    [Res0]=ResidualsElectrodeSimple(Art(1:ionset-1,:,:),stimElec,[1:Tmax]);
     %factor=log(nansum(Res0(end,:)')/trace(KersOld{2}))
   
            [Res]=ResidualsElectrodeSimple(Art(ionset:end,:,:),stimElec,[1:Tmax]);
