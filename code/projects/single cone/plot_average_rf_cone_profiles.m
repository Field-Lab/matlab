function plot_fig = plot_average_rf_cone_profiles(datarun,cell_types, varargin)
% plot_average_rf_cone_profiles     plot average rf profile for several cell types
%
% usage:  plot_fig = plot_average_rf_cone_profiles(datarun,cell_types, varargin)
%
...
if ~isempty(plot_axes)
    ...
    for cc=1:length(cell_types)
        ...
        [all_x, all_y] = get_rf_cone_profiles(datarun,{cell_type},...
            'radius',max(params.bin_edges),'by_cone_type',params.by_cone_type,profiles_params);
        
        % interpolate a curve
        [average_x, average_y] = curve_from_binning(all_x,all_y,curve_params);
