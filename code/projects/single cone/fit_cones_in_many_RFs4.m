function [fit_cone_info,rf_fits] = fit_cones_in_many_RFs4(rfs,cone_info,varargin)
% FIT_CONES     fit ideal cone shapes to cones in several RFs
%
% #4 changed color parameter to RGB
%
...
[cone_weights,cone_rgb] = find_best_weights(rfs,cone_info,params.cone_kernel);
cone_info = [cone_info cone_rgb];
...
lb = repmat([min_x min_y 0 0 0],size(cone_info,1),1);
ub = repmat([max_x max_y 1 1 1],size(cone_info,1),1);
...
% get matrix
M = compute_rf_from_cone_fits_matrix(cone_centers,cone_rgb,'x_size',size(rfs,2),'y_size',size(rfs,1),...
...
rf_fits = reshape(full(M*best_weights),size(rfs));
...
if 0
    ...
    for rr = 1:size(rfs,4)
        ...
        hold(pa{rr},'on')
        % over RFs
        plot(pa{rr},cone_info(:,1),cone_info(:,2),'.k')
        % over fits
        plot(pa{rr},cone_info(:,1),cone_info(:,2)+size(rfs,1),'.k')
    end
    ...
    figure(12);clf
    imagesc(cone_rgb)
    %pa = subplot_axes(12,[0 0 1 1],0.1,0.1,size(cone_rgb,1),1);
    %for rr =1:size(cone_rgb,1)
    %   cla(pa{rr})
    %   bar(pa{rr},cone_rgb(rr,:))
    %end
...
if mod(optimValues.iteration,1)==0 && ~isempty(plot_axes)
    ...
    pa = subplot_axes(11,[0 0 1 1],0.1,0.1,size(x,1),1);
    for rr =1:size(x,1)
        cla(pa{rr})
        bar(pa{rr},x(rr,3:5));%/sum(x(rr,3:5)))
        set(pa{rr},'ylim',[0 1])
    end
...
function [cone_weights,cone_rgb] = find_best_weights(rfs,cone_info,kernel_params)
...
for cc = 1:num_cones
    cone_rgb(cc,:) = sum(reshape(reshape(rfs,[],size(rfs,3)*size(rfs,4))'*...
        reshape(bw_kernel(:,:,cc),[],1),size(rfs,3),size(rfs,4)),2);
    % normalize
    cone_rgb(cc,:) = cone_rgb(cc,:)/sum(cone_rgb(cc,:));
end
