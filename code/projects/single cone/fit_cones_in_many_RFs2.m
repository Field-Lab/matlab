function [fit_cone_info,rf_fits] = fit_cones_in_many_RFs2(rfs,cone_info,varargin)
% FIT_CONES     fit ideal cone shapes to cones in several RFs
%
% #2 introduced color parameter
%
...
%            cone_info - Nx(M+3) matrix, initial guesses of cone locations, N = # cones
%                           columns 1 to M: strength for each cell
%                           column M+1: x coord
%                           column M+2: y coord
%                           column M+3: cone identity
%
...
% start with approximate cone weights
[cone_weights,cone_colors] = find_best_weights(rfs,cone_info,params.cone_kernel);
cone_info(:,1:end-3) = cone_weights;
cone_info(:,end) = cone_colors;
%get_rf_fits(rfs,cone_info,params.cone_kernel);
...
lb = repmat([repmat(-Inf,1,size(rfs,4)) min_x min_y 0],size(cone_info,1),1);
ub = repmat([repmat(Inf,1,size(rfs,4)) max_x max_y 1],size(cone_info,1),1);
...
switch params.algorithm
    ...
    case 'lsqcurvefit'
        ...
        fit_fcn = @(xx,junk)get_rf_fits(rfs,xx,params.cone_kernel);
        plot_fcn = @(xx,oo,ss)plot_current_fits(xx,oo,ss,rfs,params.cone_kernel,plot_axes);
    ...
    case 'fmincon'
        ...
        fit_cone_info = fmincon(obj_fcn,...
            ...
            optimset(params.optim{:},...  optional user parameters
             'OutputFcn',plot_fcn,... plotting function
             'RelLineSrchBnd',0.5,'RelLineSrchBndDuration',Inf,'algorithm','active-set')); % 
...
% compute error
err = sqrt(mean(reshape((rf_fits - rfs).^2,[],1)));
...
cone_weights = reshape(cone_info(:,1:end-3),[],1);
cone_centers = cone_info(:,end-2:end-1);
cone_colors = cone_info(:,end);

num_cones = size(cone_info,1);
cone_fit_size = [size(rfs,1) size(rfs,2)];


fprintf('%0.01f\n',mean(mean(cone_weights)))
...
switch 2
    case 1
        ...
        for cc = 1:size(cone_info,1)
            ...
            % generate cone fit
            cone_fit = make_gaussian(struct('center',cone_info(cc,end-2:end-1),...
            ...
            rgb = cone_rgb(cone_info(cc,end),kernel_params.cone_rgb);

            % identify weights for each cell
            cone_weights = cone_info(cc,1:end-3);

            % scale to height, and add to each rf in rf_fits for each color
            %rf_fits = rf_fits + reshape(kron(cone_weights,cone_fit),size(cone_fit,1),size(cone_fit,2),[]);
            for dd = 1:size(rfs,3)
                % get cone weights for each cell in THIS color
                cone_weights_c = cone_weights*rgb(dd);

                % add the weighted fits for this cone to the complete fit
                rf_fits(:,:,dd,:) = rf_fits(:,:,dd,:) +...
                    reshape(kron(cone_weights_c,cone_fit),size(cone_fit,1),size(cone_fit,2),1,[]);
            end
        end
    ...
    case 2
        ...
        rf_fits = reshape(M*reshape(cone_weights',[],1),size(rfs));
    ...
    case 3
        ...
        for cc = 1:num_cones
            ...
            % set norm to 1
            rgb_kernel = rgb_kernel/norm(reshape(rgb_kernel,[],1));

            % add to accumulating matrix
            clear temp
            for rr=1:size(rfs,4)
                temp{rr}=sparse(reshape(rgb_kernel,[],1));
            end
...
%rf_fits = rf_fits + dist_penalty/numel(rf_fits);


% plot RFs and fits
if 1
    figure(10);clf
    plot_axes = subplot_axes(10,[0 0 1 1],0.1,0.1,size(rfs,4),1);
    ...
    for rr = 1:size(rfs,4)
        ...
        imagesc(norm_image([rfs(:,:,:,rr);rf_fits(:,:,:,rr)]),'Parent',pa{rr});
        % plot cones
        hold(pa{rr},'on')
        % over RFs
...
% compute fits based on these weights
rf_fits = reshape(M*best_weights,size(rfs));

% reshape to return in standard format
%cone_weights = reshape(best_weights,size(rfs,4),num_cones)';
cone_weights = reshape(best_weights,num_cones,size(rfs,4));

mean(mean(best_weights))
...
if 1
    ...
    figure(11);clf
    plot_axes = subplot_axes(11,[0 0 1 1],0.1,0.1,size(rfs,4),1);
