function [class_id, mean_record, covariance_record, K_record, lP_record, alpha_record, bestIndex, bestIndices] = sampler(training_data, num_sweeps, a_0, b_0, mu_0, k_0, v_0, lambda_0, graphics, nBest, verbose, burnin, outputPath)
...
% save indices of max n steps from the posterior
if(nargin < 10)
    nBest = 1;
end
...
if(nargin < 8)
    ...
    lambda_0 = eye(size(training_data(:,1),1))* 1000;
end
...
for sweep = 2:num_sweeps
    try
        ...
        if verbose == true && mod(sweep, 10) == 0
        ...
        for(k=1:K_plus)
            ...
            if(NIW)
                ...
                % if covariance goes bad, this is where it fails
            else
        ...
        lP_record(sweep) = lp;
        if(GRAPHICS && (sweep == num_sweeps-1))
            figure(333)
            clf(333)
            subplot(3,1,1)

            plot(lP_record(lP_record(1:sweep) ~= 0));
            title('Log P')
            subplot(3,1,2)
            plot(K_record(K_record(1:sweep) ~= 0));
            title('K');
            subplot(3,1,3)
            plot(alpha_record(alpha_record(1:sweep) ~= 0));
            title('alpha');
            
            if savePlot == true
                %print(333, '-depsc2', outputPath);
                print(333, '-dpdf', outputPath)
            end
...
% else
%    burnin = round(num_sweeps / 2);
% end

%kill off log likelihoods of 0 which indicate errors
lP_record(lP_record == 0) = NaN;
maxv   = max(lP_record(burnin:end));
sorted = sort(lP_record(burnin:end), 'descend');



%should always pick maximum, since lowest -log likelihood value is best
bestValue = maxv;
bestIndices = zeros(nBest,1);

offset = 0;

%find nBest solutions
for k=1:nBest
    
    %find a non nan index
    if isnan(sorted(k+offset))
        while isnan(sorted(k+offset))
          offset = offset+1;
        end
    end
    ...
    a = find(sorted(k+offset) == lP_record);
