...
% 2015-01-16 starting to rethink to version_1  (no Hess Correct)
% Version 2 .. very different with respect to derivatives .. etc.

% Version  2 refix rk1 and rk2 with lcif_derivative ~= COV
% Version  2 finally correctly addressed the Hess Correction
% Versions 3_0 3_1 New functions to handle rk2-ConductanceBased 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% NEW ADDITION VERSION 2 2015-01-17 %%%%%%%%%%%%%%%%%%%%%%
%%% FOR CONVEX FUNCTIONS Covariate Vec lcif_derivative %%%%%
%%% NOT TRUE FOR NONCONVEX FUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lcif_derivative               = NaN(length(params) ,bins);
convex_ind                    = paramind.convParams_ind;
lcif_derivative(convex_ind,:) = convex_covariates;
...
if strcmp(stimfilter_mode, 'rk1') || strcmp(stimfilter_mode, 'rk2') || strcmp(stimfilter_mode, 'rk2-ConductanceBased')
    ...
    lcif_derivative(paramind.time1 ,:) =  temporal_covariatevec;
    lcif_derivative(paramind.space1,:) =  spatial_covariatevec;
end
...
if  strcmp(stimfilter_mode, 'rk2') || strcmp(stimfilter_mode, 'rk2-ConductanceBased')% same as rk1, just throwing in the rk2 term
    ...
    lcif_derivative(paramind.time2 ,:) =  temporal_covariatevec;
    lcif_derivative(paramind.space2,:) =  spatial_covariatevec;
end
%%
...
if exist('lcif_nonlinearity','var')
    ...
    [f grad Hess log_cif]= glm_opt_NonConvexfunction_withNL(params,covariate_vec,lcif_derivative,spikebins,bin_duration,X_frame,frame_shifts,bpf,paramind,lcif_nonlinearity);
else
    [f grad Hess_Base log_cif]= glm_opt_NonConvex_function(params,covariate_vec,lcif_derivative,spikebins,bin_duration);
