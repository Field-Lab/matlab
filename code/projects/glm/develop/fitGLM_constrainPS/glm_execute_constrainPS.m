...
function [fittedGLM] = glm_execute_constrainPS(GLMType,fitspikes,fitmovie,testspikes_raster,testmovie,inputstats,glm_cellinfo,troubleshoot)
...
if GLMType.PostSpikeFilter
    ...
    % constrainPS
    PS_balance = prep_convolvespikes_basis(home_spbins, ones(1,size(ps_basis,1)), bins);
end
if GLMType.CouplingFilters;
    basis = cp_basis';
...
if isfield(paramind, 'PS')
       ...
       % linearfilters.PostSpike.Filter     = ps_basis * pstar(paramind.PS);
        linearfilters.PostSpike.Filter     = ps_filter;
