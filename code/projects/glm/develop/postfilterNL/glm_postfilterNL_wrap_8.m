...
%{
clear; clc
exps = [1 2 3 4]; stimtypes = [2 1]; celltypes = [1 2]; 
...
clear; clc
exps = [1]; stimtypes = [2]; celltypes = [2]; 
        ...
        for i_celltype = celltypes            
            ...
            for i_cell = 1:length(cellgroup)
                ...
                elseif strcmp(postfilterNL.type, 'PosLogistic_fixedPS')
                    ...
                    SP0    = stim_pos;
                    K_vec  = [.25 .5 1 2];
                    X0_vec = [-4 -2 -1 -.5 0 .5 1];
                    
                    K_vec = [.5 1]
                    X0_vec = [-1 0]
                    
                    objvals   = zeros(length(K_vec), length(X0_vec) )
                    fitted_ps = cell(length(K_vec), length(X0_vec) )
                    for i_K = 1:length(K_vec)
                        for i_X0 = 1:length(X0_vec)
                            tic
                            K = K_vec(i_K);
                            X0 = X0_vec(i_X0);
                    ...
                    test_logistic_vals = 1./ (1 + exp(-K_opt * ( stim_pos - X0_opt) ) ) ...
                        + (  1- 1/(1+exp(K_opt*X0_opt)) );
                    stim_testLOGISTIC = log(test_logistic_vals);
                    lcif_teststim_NEW = new_p(2) * stim_testLOGISTIC + new_p(3)* stim_neg;
                    
                    rescale.MU = new_p(1);
                    rescale.stim_posLOG = new_p(2);
                    rescale.stim_neg = new_p(3);
                    rescale.PS = new_p(4);
                    rescale.poslogistic_K = K_opt;
                    rescale.poslogistic_X = X0_opt;
                end
                ...
                if runoptions.print
                    ...
                    %}
                    orient landscape
                    eval(sprintf('print -dpdf hello.pdf',printname))
