...
cell_subset = 'shortlist'; EM.debug = false;
base_glmsettings = {}
EM.type = 'PosLogistic_refitPS'
...
clear; clc
exps = [1 3]; stimtypes = [2]; celltypes = [1 2]; 
cell_subset = 'glmconv_4pct'; EM.debug = false;
base_glmsettings = {};
...
EM.type = 'saccade_NEGPOS_longfem_displacement_NEGPOS';
EM.fem_frameshifts = [-10:50];
EM.saccade_frameshifts = [-10:110];

if EM.debug
EM.fem_frameshifts = [-1:5];
EM.saccade_frameshifts = [-1:5];
end

runoptions.print = true;
glm_em_wrap(exps,celltypes,cell_subset,base_glmsettings,EM,runoptions)
%}
function glm_em_wrap(exps,celltypes,cell_subset,base_glmsettings,EM,runoptions)
...
if isfield(EM,'debug') && EM.debug
    BD.GLM_output_raw = sprintf('%s/EM/dbug_%s', BD.GLM_output_raw,EM.type);
else 
    BD.GLM_output_raw = sprintf('%s/EM/%s', BD.GLM_output_raw,EM.type);
end
currentdir = pwd;
       ...
       % error('make this into a subroutine')  
        ...
        for i_celltype = celltypes            
            ...
            for i_cell = 1:length(cellgroup)
                ...
                shifts = EM.saccade_frameshifts;
                for i_type = 1:2
                    if i_type == 1, raw_sac = zeros(1,length(displace_perframe.fit)); end
                    if i_type == 2, raw_sac = zeros(1,length(displace_perframe.test)); end
                    
                    offset  = 0;
                    indices = offset + [1:120:length(raw_sac)];
                    raw_sac(indices) = 1;
                    
                    bins = length(raw_sac) * fittedGLM.bins_per_frame; 
                    COV_sacshift = NaN(length(shifts),bins);
                    for i_shift = shifts
                        fit_sac0 = circshift(raw_sac, [0 i_shift]);
                        fit_sac = repmat(fit_sac0,[fittedGLM.bins_per_frame,1]);
                        shift_ind = find(shifts == i_shift);
                        COV_sacshift(shift_ind,:) = reshape(fit_sac,[1,bins]);
                    end
                    if i_type == 1, COV_sacshift_fit  = COV_sacshift; end
                    if i_type == 2, COV_sacshift_test = COV_sacshift; end
                end
                
                % Solve for saccade first
                p0 = [1, 1, 1, zeros(1,length(EM.saccade_frameshifts))]';
                COV = [lcif.mu; lcif.stim; lcif.ps; COV_sacshift_fit];
                [new_p new_objval eflag output] = fminunc(@(p) subr_optimizationfunction...
                        (p,COV,home_spbins,t_bin),p0,optim_struct);     
                saccade_filter0 = new_p(4:end);
                lcif_saccade0   = (saccade_filter0') * COV_sacshift_fit;
               
                % Then fem
                p0 = [new_p(1), new_p(2), new_p(3), 1, zeros(1,length( EM.fem_frameshifts))]';
                COV = [lcif.mu; lcif.stim; lcif.ps; lcif_saccade0; COV_femshift_fit];
                [new_p new_objval eflag output] = fminunc(@(p) subr_optimizationfunction...
                        (p,COV,home_spbins,t_bin),p0,optim_struct);   
                saccade_filter = new_p(4)*saccade_filter0;
                fem_filter = new_p(5:end);   
                
                MU_NEW = new_p(1) * MU_OLD;
                lcif_teststim_NEW = new_p(2) * lcif_teststim_OLD;
                PS_NEW  = new_p(3)*PS_OLD;
                lcif_saccade = (saccade_filter') * COV_sacshift_test;
                lcif_fem = (fem_filter') * COV_femshift_test;
                lcif_other_NEW = lcif_fem + lcif_saccade;
                
                EM.fitted_femfilter = fem_filter;
                EM.fitted_sacfilter = saccade_filter;
                ...
                xvalperformance_NEW.type = EM.type;
                
                xvalperformance_NEW.EM = EM;
                ...
                if runoptions.print
                    ...
                    c = 0;
                    offset = 0;
                    text(-offset, 1-0.1*c,sprintf('%s: %s %d: Fit by %s',...
                        info.exp_nm, info.celltype,info.cid, GLMType.fit_type, xvalperformance_NEW.type), 'interpreter','none','FontSize',10)
                    c = c + delta_c;
                    text(-offset, 1-0.1*c,sprintf('Original Fit (RED) : %s',GLMType.fitname), 'interpreter','none')
                    c = c + delta_c;
                    text(-offset, 1-0.1*c,sprintf('Refit (BLUE) with Eye Movements: %s',...
                         xvalperformance_NEW.type), 'interpreter','none')
                    c = c + delta_c;
                    ...
                    plot(EM.saccade_frameshifts, zeros(1,length(EM.saccade_frameshifts)),'k');
                    plot(EM.saccade_frameshifts, EM.fitted_sacfilter,'b', 'linewidth',LW);
                    xlabel('Frames ( ~8 millisec per frame )');
                    ylabel('LCIF');
                    title('Saccade Filter');
                    
                    subplot(4,5,[9 10])
                    set(gca,'fontsize',10); hold on
                    plot(EM.fem_frameshifts, zeros(1,length(EM.fem_frameshifts)),'k');
                    plot(EM.fem_frameshifts, EM.fitted_femfilter,'b', 'linewidth',LW);
                    xlabel('Frames ( ~8 millisec per frame )');
                    ylabel('LCIF/FEM');
                    title('FEM filter');
                    ...
                    glm_rast = xvalperformance_NEW.rasters.glm_OLD(:,1:bins); 
                    NL_rast  = xvalperformance_NEW.rasters.glm_NEW(:,1:bins); 
                    ...
                    for i_trial = 1:trials
                        ...
                        plot(rec1, i_trial, 'k.','markersize',MS)
                        if length(glm1) < 4*length(rec1) 
                            if length(glm1) > 0
                                plot(glm1, i_trial + trials, 'r.','markersize', MS)
                            end
                        end
                        if length(NL1) < 4*length(rec1) 
                            if length(NL1) > 0
                                plot(NL1, i_trial + 2*trials, 'b.','markersize',MS)
                            end
                        end
                    end
                    %}
...
xvalperformance_NEW.rasters.glm_NEW        = logical_sim;
xvalperformance_NEW.rasters.glm_OLD        = fittedGLM.xvalperformance.rasters.glm_sim;
