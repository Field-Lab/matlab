function [uop_bps, uop_logprob_persec, cm_bps, cm_logprob_persec, opt_params] = rastbps_comp_findPS_modsignal(mod_factor,inp_rast,t_bin,ps_basis,options)
...
COV_rate(find(COV_rate>0))  = mod_factor*(COV_rate(find(COV_rate>0)));
%COV_rate(find(COV_rate<0)) =0;pl mod_factor*(COV_rate(find(COV_rate<0)));
...
%%
lcif            = pstar' * COV;
cm_ratepersec_concat  = exp(lcif);
cm_ratepersec         = NaN(reps,bins);
for i_rep = 1:reps
	index_start = (i_rep-1)*bins + 1;
	index_end   =  i_rep * bins;
	cm_ratepersec(i_rep,:) = cm_ratepersec_concat(index_start:index_end);
end

cm_rateperbin  = t_bin*cm_ratepersec;
cm_model       = cm_rateperbin;
uop_model      = repmat(uop,[reps,1]);
null_model     = repmat( (1/(reps*bins)*sum(inp_rast(:)))*ones(1,bins) , [reps,1]);

% Evaluate Logarthmic Probability
cm_logprob          = sum(rastbps_comp_logprob( inp_rast, cm_model,  'binary')) ;
