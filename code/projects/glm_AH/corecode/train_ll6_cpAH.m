...
function [log_prob lcifs cifs kx] = train_ll6_cpAH(p,Basepars,Stimpars,Trainpars,kxopt, rec_kx_STA)
...
if (~exist('kxopt','var')) || isempty(kxopt)
...
if exist('rec_kx_STA','var')
    scale_rec = p(Basepars.paramind.SR); 
    rec_STA = scale_rec * rec_kx_STA;
    recSTA_microbin = reprows(rec_STA ,Basepars.spikebins_perstimframe); 
end

if isfield(Basepars, 'rect_full')
    spatialfilter = p(Basepars.paramind.FR); 
    rect_full = (Basepars.rect_full') * spatialfilter;
    rect_full_microbin = reprows(rect_full ,Basepars.spikebins_perstimframe); 
end
...
if exist('rec_kx_STA','var')
    lcifs = lcifs + recSTA_microbin;
end

if isfield(Basepars, 'rect_full')
    lcifs = lcifs + rect_full_microbin;
end
