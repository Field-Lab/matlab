...
function [ll_grad ll_hess] = train_ll_grad5AH(p,Basepars,Stimpars,Trainpars,lcifs,cifs)
...
if ~strcmp( Basepars.k_filtermode ,'STA' )
    if (~filtermode)
        ...
        Trainpars.lGrad = linearfilt_grad3AH(p,Basepars,Stimpars,Trainpars,1);
...
if (nargout == 1 || (nargout>1 && ~hessFlag))
    [ll_grad(idx)] = multisample_gradHess_structsAH(Basepars,Stimpars,Trainpars,kernel_grad,kernel_hess,frozen_idx);
elseif (hessFlag) 
	[ll_grad(idx) ll_hess(idx,idx)] = multisample_gradHess_structsAH(Basepars,Stimpars,Trainpars,kernel_grad,kernel_hess,frozen_idx);
end
