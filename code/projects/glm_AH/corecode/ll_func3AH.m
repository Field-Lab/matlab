...
if (exist('kx_opt','var') && ~isempty(kx_opt))
   ...
   if exist('rec_kx_STA','var') 
       [logprob lcifs cifs] = train_ll6_cpAH(p,Basepars,Stimpars,Trainpars,kx_opt,rec_kx_STA);
   else
    [logprob lcifs cifs] = train_ll6_cpAH(p,Basepars,Stimpars,Trainpars,kx_opt);
   end
else
    
    if exist('rec_kx_STA','var') 
       [logprob lcifs cifs] = train_ll6_cpAH(p,Basepars,Stimpars,Trainpars,[],rec_kx_STA);
    else
        [logprob lcifs cifs] = train_ll6_cpAH(p,Basepars,Stimpars,Trainpars);
   end
...
if (nargout < 3)
   [g] = train_ll_grad6AH(p,Basepars,Stimpars,Trainpars,lcifs,cifs);
else
   [g H] = train_ll_grad6AH(p,Basepars,Stimpars,Trainpars,lcifs,cifs);
   if (hessFlag)
      H = -H;
   end
