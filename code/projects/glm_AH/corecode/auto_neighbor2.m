function [ON_Neighbors , OFF_Neighbors ] = auto_neighbor2 (cid , datarun, neighbornumber)

% New auth_neighbor on 2013-1-26, make neighbornumber an input param 

idx = find(datarun.cell_ids == cid);
center = datarun.vision.sta_fits{idx}.mean;
center_x = center(1);
center_y = center(2);
...
%if strcmp ( init_pars.celltype , 'OFF-Parasol')
    ...
    plot(center_x , center_y, 'y*', 'markersize', MSHome);  
...
for type = 1:2
    ...
    ids       = datarun.cell_types{ type }.cell_ids;
    masteridx = zeros(size(ids));
    for i= 1:length(ids) 
        masteridx(i) = find (datarun.cell_ids == ids(i) ) ;
    end
    RF      = datarun.vision.sta_fits ( masteridx );
