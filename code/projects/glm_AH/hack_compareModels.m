...
comparison_name = 'PSFilter10_vsStandardPSFilter20';
changes_cell_A = {};
changes_cell_B{1}.type = 'specialchange';
changes_cell_B{1}.name = 'psfilter_10';
fit1 = 'standardGLM_psfilter20';
fit2 = 'psfilter10';
...
for i_exp = exps
    ...
    for i_celltype = celltypes
        ...
        xval_WN     = cell(length(cellgroup),2);
        xval_NSEM   = cell(length(cellgroup),2);
        ...
        for i_cell = 1:length(cellgroup)
            ...
            for i_model = 1:2
                ...
                rawXVALWN(i_model)  = fittedGLM.xvalperformance.logprob_glm_bpspike;
                rawfitWN(i_model) = fittedGLM.rawfit.objective_val;
                clear fittedGLM
                eval(sprintf('load %s/%s.mat fittedGLM', GLM_fitdir{i_model}.NSEM, cell_savename))
                rawXVALNSEM(i_model) = fittedGLM.xvalperformance.logprob_glm_bpspike;
                rawfitNSEM(i_model) = fittedGLM.rawfit.objective_val;
                clear fittedGLM
            end
            xval_WN(i_cell,:)   = rawXVALWN / norm_WN(norm_index);
            xval_NSEM(i_cell,:) = rawXVALNSEM / norm_NSEM(norm_index);
            
            fit_WN(i_cell,:)   = rawfitWN;
            fit_NSEM(i_cell,:) = rawfitNSEM;
        end
        
        model_comparison.byexpnm{i_exp}.celltype{i_celltype}.xval_WN.bps = xval_WN;
        model_comparison.byexpnm{i_exp}.celltype{i_celltype}.xval_NSEM.bps = xval_NSEM;
...
if plot_diagnostics
    ...
    for i_exp = exps
       ...
       % eval(sprintf('load  %s/%s_%s.mat raster_scores',savedir,crm_type,exp_nm)); 
        ...
        plot(unity_line,unity_line,'k')
        xlim([-1,1]); xlabel(fit1,'interpreter','none')
        ylim([-1,1]); ylabel(fit2,'interpreter','none')
...
if plot_deltaWNNSEM
    ...
    for i_exp = exps
        ...
        for i_celltype = celltypes
            scores_WN   = model_comparison.byexpnm{i_exp}.celltype{i_celltype}.xval_WN;
            scores_NSEM = model_comparison.byexpnm{i_exp}.celltype{i_celltype}.xval_NSEM;
            scores_NSEM(find(scores_NSEM<0)) = 0;
    ...
    eval(sprintf('print -dpdf %s/%s_deltaWNNSEM_%s.pdf', savedir, comparison_name, cellselection_type))
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i_plot = 1:2 
        clf; hold on; axis square
        ...
        set(gca,'xtick',[0:.25:1]); set(gca,'ytick',[0:.25:1]);
        if i_plot == 1
            eval(sprintf('print -dpdf %s/%s_oldWNNSEM_%s.pdf', savedir, comparison_name, cellselection_type))
        elseif i_plot == 2
            eval(sprintf('print -dpdf %s/%s_newWNNSEM_%s.pdf', savedir, comparison_name, cellselection_type))
        end
    end
end
