...
for i_exp = exps
    for i_stimtype = 1:2    
        for i_celltype = 1:2
            ...
            for i_cell = 1:length(cells)
                ...
                if exist('expanded_metrics','var') && expanded_metrics
                    ind_raster_scores = raster_metrics_expanded_old(logicalspike,metricparams);
                else
                    tic
    ...
    if i_exp == 1,expanded_raster_scoresA = raster_scores{i_exp}; eval(sprintf('save %s/expanded_raster_scoresA_old.mat expanded_raster_scoresA',rastmetdir)); end
    if i_exp == 2,expanded_raster_scoresB = raster_scores{i_exp}; eval(sprintf('save %s/expanded_raster_scoresB_old.mat expanded_raster_scoresB',rastmetdir)); end
    if i_exp == 3,expanded_raster_scoresC = raster_scores{i_exp}; eval(sprintf('save %s/expanded_raster_scoresC_old.mat expanded_raster_scoresC',rastmetdir)); end
    if i_exp == 4,expanded_raster_scoresD = raster_scores{i_exp}; eval(sprintf('save %s/expanded_raster_scoresD_old.mat expanded_raster_scoresD',rastmetdir)); end
end
