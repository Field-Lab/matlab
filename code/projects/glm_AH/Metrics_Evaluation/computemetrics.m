...
basedir = '/netapp/snle/lab/Experiments/Array/Analysis/akheitman/NSEM_Projects/GLM/Rasters';

%exp_nm = '2012-09-27-3'; expname = 'expB'; fit_type = 'NSEM';  %cmodeltype = '8pix_Identity_8pix';
test_type = 'BW'; 
shortlist = true;
...
%version = 'BWBW_vs_NSEMNSEM';
%version = 'BWNSEM_vs_NSEMNSEM';
...
for i_exp = 1:4
    %%
    if shortlist
        if i_exp == 1, exp_nm = '2012-08-09-3' ; expname = 'expA'; head_cellID = [1471  3676 5086 5161];   end  %ONPar doesn't converge for NSEM  [841 1426 1772 2101 1276];
    ...
    compareWNNSEM.note      = 'Row 1 is WN Row 2 is NSEM';
   ...
   %% 
    for i_test = 1:2
        if i_test  == 1
            fit_type = 'BW';
            if strcmp(version , 'BWBW_vs_NSEMNSEM')
                test_type = 'BW';
            end
            if strcmp(version ,'BWNSEM_vs_NSEMNSEM')
                test_type = 'NSEM';
            end
...
%
        ...
        if strcmp(test_type, 'NSEM')  
            ...
            eval(sprintf('load %s/NSEM_%s/testmovie_%s_%s', stimulidir,moviename,scheme, cmodel)); 
        ...
        if strcmp(test_type, 'BW') && strcmp(cmodel ,'8pix_Identity_8pix')
            load('/netapp/snle/lab/Experiments/Array/Analysis/akheitman/NSEM_Projects/Stimuli/BW-8-1-0.48-11111_RNG_16807/rastermovie_only.mat')
            testmovie.matrix = round(raster_movie);
            inputstats.mu_avgIperpix = .5;
        end

        if strcmp(test_type, 'BW') && strcmp(cmodel ,'8pix_Model1_1e4_8pix')
            load /netapp/snle/lab/Experiments/Array/Analysis/akheitman/NSEM_Projects/Stimuli/BW-8-1-0.48-11111_RNG_16807/testmovie_8pix_Model1_1e4_8pix.mat
            testmovie = testmovie;
            inputstats.mu_avgIperpix = .5
        end

        %
        fittype = sprintf('%s_%s/%s_Fit' , fit_type, map_type, fit_type);
