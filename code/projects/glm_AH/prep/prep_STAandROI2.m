...
STAPars.K_slen     = 15;
STAPars.STA_Frames = 120;

if STAPars.resfactor == 1 && STAPars.frameshift == 1
    d_save = sprintf('%s/STAandROI_new_%dframes',DirPars.output_dir, STAPars.STA_Frames);
else
...
for i_blk = FitBlocks
	...
	movie_singleform = double(movie.fitmovie.movie_byblock{fitmovieind}.matrix);
    if strcmp(fit_type, 'NSEM')
