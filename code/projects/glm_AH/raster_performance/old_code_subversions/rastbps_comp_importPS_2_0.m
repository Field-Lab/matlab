...
prob_fulluop_bin   = sum(inp_rast,1) / (size(inp_rast,1));
...
%
% GAIN_SQUARED AND GAIN_ROOT TERMS
if exist('higher_powers','var') && higher_powers
    gain_p2     = gainchange.*2;
    gain_n2     = gainchange.*(2);
    log_gain_p2 = log(gain_p2);
    log_gain_n2 = log(gain_n2);
    
    gain_p3     = gainchange.*3;
    gain_n3     = gainchange.*(3);
    log_gain_p3 = log(gain_p3);
    log_gain_n3 = log(gain_n3);
end
%

% COVARIATES VWHICH ARE REP INDEPENDENT (RATE, MEAN)
COV_constant   = repmat(ones(1,bins),[1, reps]);
COV_rate       = repmat(log_gain    ,[1, reps]);
if exist('higher_powers','var') && higher_powers
    COV_rate_p2    = repmat(log_gain_p2 ,[1, reps]);
    COV_rate_n2    = repmat(log_gain_n2 ,[1, reps]);
    
    COV_rate_p3    = repmat(log_gain_p3 ,[1, reps]);
    COV_rate_n3    = repmat(log_gain_n3 ,[1, reps]);
end
