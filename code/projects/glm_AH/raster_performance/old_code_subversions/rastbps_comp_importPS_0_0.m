function [uop_bits_perspike crm_bits_perspike] = rastbps_compute(raster,t_bin,psfilter,options)
 %%% PURPOSE %%%
...
%%% OUTSIDE CALLS %%%
% GLM_CONVEX_OPTIMAZATIONFUNCTION
% fminunc(@(p) glm_convex_optimizationfunction(p,COV,home_spbins,t_bin),p_init,optim_struct);

% AKHEITMAN 2015-03-01
% Version 0 confirmed to work 2015-03-01
...
[pstar fstar eflag output] = fminunc(@(p) glm_convex_optimizationfunction_local(p,COV,home_spbins,t_bin),p_init,optim_struct);
lcif            = pstar' * COV;
...
for i_rep = 1:reps
	...
	crm_ratepersec(i_rep,:) = crm_ratepersec_concat(index_start:index_end);
                ...
                crm_logprob          = sum(eval_rasterlogprob( raster, crm_model,  'binary', 'conditioned')) ;
                uop_logprob          = sum(eval_rasterlogprob( raster, uop_model,  'binary', 'conditioned')) ;
                null_logprob         = sum(eval_rasterlogprob( raster, null_model,  'binary', 'conditioned')) ;

                uop_bits          = uop_logprob - null_logprob;
                uop_bits_perspike = uop_bits / (sum(null_model(1,:)));

                crm_bits          = crm_logprob - null_logprob;
                crm_bits_perspike = crm_bits / (sum(null_model(1,:)));
end


function  [f grad Hess log_cif]= glm_convex_optimizationfunction_local(linear_params,covariates,spikebins,bin_duration)
