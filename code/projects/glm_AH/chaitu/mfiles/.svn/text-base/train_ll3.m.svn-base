...
function [lg_p input_term cifs kx lg_p_breakdown] = train_ll3(p,basepars,stimpars,trainpars,kxopt)
...
if (~exist('kxopt','var'))
    ...
    if (isfield(basepars,'XsqK') && basepars.XsqK)
        ...
        1;
        [kx kxsq] = filterstimulus_train(p,basepars,stimpars,trainpars); % T x N
        kx = kx + kxsq;
    else
...
if (Neff > 1 && basepars.nofilters_coupling > 0)
    cpidx = get_pars_idx(basepars,1,Neff,'cp');
    offset = cpidx(1)-1;
...
if (size(input_term,1) ~= (size(trainpars.negSpikes,1)+size(trainpars.D,1)))
    error('input term has length %d and does not match concatenated spike train length %d',size(input_term,1),(size(trainpars.negSpikes,1)+size(trainpars.D,1)));
end
