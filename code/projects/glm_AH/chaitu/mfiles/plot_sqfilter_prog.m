function out = plot_sqfilter_prog(x,j,basepars,stimpars,trainpars)
...
kidx = get_pars_idx(basepars,j,size(trainpars.D,2),'ksq');

K = reshape(x(kidx),basepars.n,basepars.Mk);
cla;
if(basepars.n > 1)
    imagesc(K',[-1 1].*max(eps,max(abs(K(:))))), axis tight, axis image;
else
