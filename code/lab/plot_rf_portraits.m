...
%
% arguments:      datarun - datarun struct with cell array of spatial RFs
...
% rfs_field_name    'rfs'       name of the field in datarun.stas in which RFs are stored
...
for cc = 1:length(cell_nums)
    ...
    % make title cell ID
    title(plot_axes{cc},num2str(cell_id))
    ...
    if ~isempty(params.plot_color)
        ...
        rf = rf(:,:,params.plot_color);
    end

    % get plot radius
    rad = params.plot_radius;

    % get plot range, clipping edges if it goes beyond the boundaries of the rf itself
    xrng = max(ctr(1)-rad,1):min(ctr(1)+rad,size(rf,2));
    yrng = max(ctr(2)-rad,1):min(ctr(2)+rad,size(rf,1));
    ...
    if params.scale_factor == 1
        image(norm_image(rf(yrng,xrng,:)))
    else
        image(matrix_scaled_up(norm_image(rf(yrng,xrng,:)),params.scale_factor))
    end
