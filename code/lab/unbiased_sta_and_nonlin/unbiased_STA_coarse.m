...
% on it. Then calculates nonlinearity on remaining inputs to avoid bias. Repeats several
% times choosing different snippets of inputs for STA and NL. Takes average
% of STAs and average of NLs for each cone/input, resulting in an unbiased STA
% and NL. 
% Input args:
%       - inputs (e.g. raw WN movie; works best if subset of the full movie (only ROI) is provided)
%       - spikes have to start from time 0 as inputs do.
%       - fraction - what fraction of inputs to take for each sta (0.5, 0.9, etc)
%       - for the first STA, last portion of inputs will be taken for nonlinearity
%           then, next adjacent portion. So for fraction 0.9, 90% of inputs
%           will be taken for STA, 10% for nonlinearity, and 10 STA/NL
%           pairs will be computed
...
% this can be saved if desired? just to compare biased (this) and unbiased
% (down below) NLs.
