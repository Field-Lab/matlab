...
cell_type_names = {'ON parasol', 'OFF parasol', 'ON midget', 'OFF midget'};



for i=180:length(pathfile)%59
    clear datarun
    ...
    if isempty(t)
        ...
        % find cell types in cell_list that are in datarun.cell_types
        class_list = [];
        for cnt = 1:length(cell_type_names)
            for ii = 1:length(datarun.cell_types)
                if strcmp(cell_type_names{cnt}, datarun.cell_types{ii}.name)
