...
%
% gauthier 2008-10
% phli 2013-11: Hacked up to workaround Matlab R2013b inputParser segfault.  
% MathWorks gave us a simpler workaround: set PartialMatching to false.  May only work on more recent Matlab versions though.
%
%
...
p.addParamValue('sparse', []);
        
p.parse(varargin{:});
params = p.Results;

% Handle defaults in funny way to workaround the inputParser segfault bug
switch params.dim
    case 1 % 1D gaussian
        if isempty(params.center), params.center = 0; end
        
    case 2 % 2D gaussian
        if isempty(params.center), params.center = [2 2]; end
        
        if isempty(params.sparse)
            % add defaults based on effective radius
            switch params.effective_radius
